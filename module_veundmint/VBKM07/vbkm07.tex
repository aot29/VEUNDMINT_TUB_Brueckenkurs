...
Die Ableitung wurde weiter oben mittels einer Tangente an den Graphen einer Funktion eingef"uhrt, die die Änderungsrate dieser Funktion
{\glqq}n"aherungsweise{\grqq} beschreibt. Aus den Eigenschaften dieser Geraden können wir auch auf Eigenschaften der
...
Die zweite und weitere {\glqq}h"ohere{\grqq} Ableitungen werden oft mit hochgestellten nat"urlichen 
...
mit Zuordnungsvorschrift $x \mapsto y = f(x)$ f"ur $x \in (a\MIntvlSep b)$.
...
Notwendige Bedingung f"ur Wendestellen (sofern die zweite Ableitung ${f'}'$ existiert): ${f'}'(w) = 0$\newline
...
Da $f$ auf ganz $\R$ definiert ist und sich für $x \rightarrow \pm \infty$ der $x$-Achse annähert,
kann das Monotonieverhalten mit Hilfe der Extremstellen abgelesen werden: $f$ ist monoton fallend auf
...
$O = 2 \cdot \pi r^2 + 2 \pi r h$ hat. Hier ist die Oberfläche $O$ eine Funktion in Abhängigkeit vom Radius $r$ und von der Höhe $h$ der Dose.
