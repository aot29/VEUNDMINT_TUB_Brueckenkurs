...
Stellen Sie sich vor, Sie sind mit Ihrem Auto unterwegs in den Urlaub. Sie fahren mit einer Geschwindigkeit von $60\MEinheit{km}/\MEinheit[]{h}$ durch eine Baustelle. Am Ende der Baustelle sehen Sie ein Schild, das es Ihnen erlaubt, ab sofort wieder mit einer Geschwindigkeit von $120\MEinheit{km}/\MEinheit[]{h}$ zu fahren. Auch wenn Sie so kräftig, wie Sie nur können, auf das Gaspedal treten, Ihre Geschwindigkeit wird sich nicht sprunghaft ändern, sondern in Abhängigkeit von der Zeit steigen. Schaffen Sie es, Ihre Geschwindigkeit innerhalb von 5 Sekunden von $60\MEinheit{km}/\MEinheit[]{h}$ auf $120\MEinheit{km}/\MEinheit[]{h}$ mit einer konstanten Änderungsrate zu erhöhen, dann ist die \emph{Beschleunigung} (= Geschwindigkeitsänderung pro Zeit) im vorliegenden Fall diese konstante Änderungsrate der Geschwindigkeit: Die Beschleunigung ergibt sich als Quotient aus der Geschwindigkeitsänderung und der dafür benötigten Zeit. Ihr Wert ist hier also $12$ Kilometer pro Stunde pro Sekunde. In der Realität werden Sie Ihre Geschwindigkeit jedoch nicht mit einer konstanten Änderungsrate erhöhen, sondern mit einer 
\emph{zeitabhängigen} Änderungsrate. Beschreiben Sie Ihre Geschwindigkeit als Funktion $v$ der Zeit $t$, erhalten Sie die Beschleunigung als Steigung dieser Funktion, unabhängig davon, ob diese Steigung (zeitlich) konstant ist oder nicht. Mit anderen Worten: Die Beschleunigung ist die \emph{Ableitung} der Geschwindigkeits\emph{funktion} $v$ nach der Zeit $t$.
...
In diesem Kapitel wiederholen wir die grundlegenden Ideen, die hinter diesen Berechnungen stecken. Wir sehen uns die Differentialrechnung an. Mit anderen Worten: Wir bilden Ableitungen von Funktionen und bestimmen so deren Steigungen bzw. Änderungsraten. Auch wenn wir diese Berechnungen hier streng mathematisch durchführen, ist unsere Motivation nicht rein mathematischer Natur. Ableitungen nehmen in vielen wissenschaftlichen Bereichen in der Interpretation als Änderungsraten verschiedener Funktionen eine wichtige Rolle ein und werden oft als herausragende Größen untersucht.
\end{MIntro}
...
Betrachten wir eine Funktion $f: [a\MIntvlSep  b] \rightarrow \R$ sowie eine Skizze des Graphen von $f$ (siehe unten).
Unser Ziel ist die Beschreibung der Änderungsrate dieser Funktion an einer beliebigen Stelle $x_0$ zwischen $a$ und $b$.
Dies wird uns auf den Begriff der Ableitung einer Funktion führen. Generell wollen wir möglichst einfache Rechenregeln verwenden.
...
Halten wir $x_0$ und den entsprechenden Funktionswert $f\left(x_0\right)$ fest und wählen eine weitere beliebige, aber variable Stelle $x$ zwischen $a$ und $b$ sowie ihren Funktionswert $f\left(x\right)$ aus. 
Durch diese beiden Punkte, also durch $\left(x_0\MIntvlSep f(x_0)\right)$ und $\left(x\MIntvlSep f(x)\right)$, können wir eine Gerade legen, die durch ihre Steigung und ihren $y$-Achsenabschnitt festgelegt ist. Als Steigung dieser Geraden erhalten wir den sogenannten Differenzenquotienten
...
Wir haben also eine mittlere Änderungsrate der Funktion $f$ im Intervall $[x_0\MIntvlSep  x]$ gefunden. Dieser Quotient wird auch als \textbf{relative Änderung} bezeichnet.

Lassen wir nun die variable Stelle $x$ gegen die Stelle $x_0$ streben, so stellen wir fest, dass die Gerade, die den Graphen der Funktion
...
an den Graphen im Punkt $\left(x_0\MIntvlSep f\left(x_0\right)\right)$ wird. Auf diese Weise können wir die Änderungsrate
der Funktion $f$ - oder die \textbf{Steigung} des Graphen von $f$ - an der Stelle $x_0$ \emph{selbst} bestimmen. Führt der geschilderte Prozess der Annäherung von $x$ an $x_0$ bildlich gesprochen
...
dass der \MEntry{Grenzwert}{Grenzwert} des Differenzenquotienten \emph{existiert}. Beschrieben wird dieser Grenzwertprozess, dass $x$ gegen $x_0$ strebt,
hier und im Folgenden mit dem
\[
\mbox{Symbol } \lim_{x \rightarrow x_0} \MDFPSpace,
\]
wobei $\lim$ abkürzend für \emph{Limes}, das lateinische Wort für Grenze, steht.
Existiert der Grenzwert des Differenzenquotienten, so bezeichnen wir mit
...
den Wert der Ableitung von $f$ in $x_0$. Die Funktion $f$ ist dann an der Stelle $x_0$ \textbf{ableitbar} bzw. \textbf{differenzierbar}.
...
\begin{MExercise}
  ...
  Bewegt sich $x$ auf $x_0=1$ zu, so erhalten wir die Steigung \MLParsedQuestion{5}{2}{5}{DG10} des Graphen von $f$ an der Stelle $x_0=1$.
...
auch nur für sehr einfache Funktionen ausrechnen. Typischerweise rechnet man die Ableitung
durch Anwendung von Rechenregeln und durch Einsetzen bekannter Ableitungswerte für bestimmte
Bausteine aus.
...
können wir den Differenzenquotienten mit $x = x_0 + h$ umschreiben:
\[
\frac{f(x) - f(x_0)}{x - x_0} = \frac{f(x_0 + h) - f(x_0)}{h} \MDFPeriod
\]
Beachten Sie, dass wir keine Voraussetzung darüber getroffen haben, ob $x$ größer oder kleiner als $x_0$ ist. Die Größe $h$ kann daher positive oder negative Werte annehmen. Um die Ableitung der Funktion $f$ zu bestimmen, muss nun der Grenzwert für $h \rightarrow 0$ berechnet werden: 
\[
...
\textbf{differenzierbar}. Viele der häufig benutzten Funktionen sind differenzierbar. Ein einfaches Beispiel dafür, dass eine Funktion nicht unbedingt differenzierbar ist, ist die Betragsfunktion $f(x) := |x|$.
...
Sehen wir uns den Differenzenquotienten für $f$ an der Stelle $x_0 = 0$ an:
\[
\frac{f(0+h) - f(0)}{h} = \frac{|h| - |0|}{h} = \frac{|h|}{h} \MDFPeriod
\]
Da $h$ größer oder kleiner als $0$ sein kann, unterscheiden wir zwei Fälle: Beginnen wir mit dem Fall $h > 0$. Dann ist $\frac{|h|}{h} = \frac{h}{h} = 1$. Im Fall $h < 0$ erhalten wir $\frac{|h|}{h} = \frac{-h}{h} = -1$. Der Grenzwertprozess, dass $h$ sich $0$ nähert, führt in den beiden Fällen also auf zwei verschiedene Ergebnisse ($1$ und $-1$). Daher existiert \textbf{der} Grenzwert des Differenzenquotienten an der Stelle $x_0 = 0$ nicht. Als Folge davon ist die Betragsfunktion an der Stelle $x_0 = 0$ nicht differenzierbar.
...
Die meisten Funktionen, die Ihnen bekannt sind, wie z.B. Polynome, trigonometrische Funktionen und Exponentialfunktionen (siehe Modul \MRef{VBKM06}),
sind differenzierbar. Im Folgenden wollen wir die Ableitungsregeln für diese Funktionen wiederholen.
\end{MIntro}
...
Aus der Einf"uhrung der Ableitung mithilfe des Differenzenquotienten ergibt sich f"ur eine linear-affine Funktion (siehe Modul \MRef{VBKM06}, Abschnitt \MRef{VBKM06_sec:linear-affin}) $f\left(x\right) = m x + b$, wobei $m$ und $b$ gegebene Zahlen sind, dass die Ableitung von $f$ an der Stelle $x_0$ gleich $f'(x_0) = m$ ist. (Überprüfen Sie dies gerne selbst!) F"ur Monome $x^n$ mit $n \geq 1$ ist es am einfachsten, die Ableitung "uber den Differenzenquotienten zu bestimmen. Ohne detaillierte Rechnung oder Beweis ergeben sich die folgenden Aussagen:
...
\[
f'(x) = r \cdot n \cdot x^{n-1} \MDFPeriod %%
\]
Diese Ableitungsregel gilt im Übrigen auch für $n\in\mathbb{R}\setminus\{0\}$.
\end{MXInfo}

Überprüfen Sie auch diese Aussagen selbst!
...
$f(x) := x^{\frac{1}{n}}$ f"ur $x > 0$ differenzierbar, und es gilt
\[
f'(x) = \frac{1}{n} \cdot x^{\frac{1}{n}-1} \text{ f"ur } x > 0 \MDFPeriod%%
\]%
\end{MXInfo}

F"ur $n\in\mathbb{N}$ werden durch $f(x) = x^{\frac{1}{n}}$ Wurzelfunktionen beschrieben. Die hier beschriebene Ableitungsregel gilt natürlich auch für $n = 1$ oder $n = -1$.
\begin{MExample}
...
Die bisher getroffenen Aussagen können f"ur Exponenten $p \in \R$ mit 
...
\begin{MInfo}
Die Exponentialfunktion $f(x) := \MEU^{x} = \exp(x)$ hat die besondere Eigenschaft, 
...
\begin{MXContent}{Vielfache und Summen von Funktionen}{Vielfache und Summen}{STD}
...
Auch das $r$-fache einer Funktion, also $f(x) := r \cdot u(x)$ ist differenzierbar,
und es gilt
$$
...
Mit diesen beiden Rechenregeln und der Ableitungsregel für Monome lässt sich z.B. jedes beliebige Polynom ableiten.
...
\[
g'(x) = 3 x^2 + \frac{1}{x} = \frac{3 x^3 + 1}{x} \MDFPeriod%%
\]
...
$h(x) = 4^{-1} \cdot x^2 - \sqrt{x} %
 = \frac{1}{4} x^2 + (-1) \cdot x^{\frac{1}{2}}$ f"ur $x > 0$ ab, dann erhalten wir
\[
...
Auch das Produkt von Funktionen, $f(x) := u(x) \cdot v(x)$, ist differenzierbar, und es gilt die \textbf{Produktregel}
$$
f'(x) = u'(x) \cdot v(x) +  u(x) \cdot v'(x) \MDFPeriod%%
$$

Der Quotient von Funktionen, $f(x) := \frac{u(x)}{v(x)}$, ist f"ur alle $x$ mit $v(x) \neq 0$ definiert und differenzierbar, und es gilt die \textbf{Quotientenregel}
\ifttm
$$
...
\begin{MExample}
Bestimmen wir die Ableitung von $f(x) = x^2 \cdot \MEU^x$. Wenden wir die Produktregel an, können wir $u(x) = x^2$ und $v(x) = \MEU^x$ ablesen und die Ableitungen dieser Funktionen bestimmen. Dann gilt $u'(x) = 2x$ und $v'(x) = \MEU^x$. Setzen wir diese Teilergebnisse mit der Produktregel zusammen, dann erhalten wir die Ableitung der Funktion $f$:
\[
f'(x) = 2 x \MEU^x + x^2 \MEU^x = (x^2 + 2x) \MEU^x \MDFPeriod%%
\]
Untersuchen wir die Funktion $g(x) = \tan(x) = \frac{\sin(x)}{\cos(x)}$. Hier können wir durch Vergleich $u(x) = \sin(x)$ und $v(x) = \cos(x)$ ablesen. Mit $u'(x) = \cos(x)$ und $v'(x) = -\sin(x)$ können wir auch hier die Teilergebnisse mit der Quotientenregel zur Ableitung der Funktion $g$ zusammensetzen:
\[
g'(x) = \frac{\cos(x) \cdot \cos(x) - \sin(x) \cdot (-\sin(x))}{\cos^2(x)} \MDFPeriod%
\]
Mit dem in Modul \MNRef{VBKM05} (Abschnitt \MRef{Abschnitt:TrigonometrieAmDreieck}) abgeleiteten Zusammenhang
$\sin^2(x) + \cos^2(x) = 1$ können wir dieses Ergebnis zu einem der folgenden Ausdrücke zusammenfassen:
\[
...
\begin{MExercise}
Zerlegen Sie dieses Produkt in zwei Faktoren, bilden Sie die Ableitungen und setzen Sie diese mit Hilfe der Produktregel zusammen,
um die Ableitung von $f(x)=\sin(x)\cdot x^3$ zu berechnen:
\begin{MExerciseItems}
\item{Linker Faktor \MEquationItem{$u(x)$}{\MLFunctionQuestion{10}{sin(x)}{5}{x}{5}{DS2}} hat Ableitung \MEquationItem{$u'(x)$}{\MLFunctionQuestion{10}{cos(x)}{5}{x}{5}{DS5}}.}
\item{Rechter Faktor \MEquationItem{$v(x)$}{\MLFunctionQuestion{10}{x^3}{5}{x}{5}{DS3}} hat Ableitung \MEquationItem{$v'(x)$}{\MLFunctionQuestion{10}{3*x^2}{5}{x}{5}{DS4}}.}
\item{Das Produkt hat die Ableitung \MEquationItem{$f'(x)$}{\MLFunctionQuestion{30}{(cos(x)*x^3)+(sin(x)*3*x^2)}{5}{x}{5}{DS6}}.}
\end{MExerciseItems}
\begin{MHint}{Lösung}
...
\begin{MExercise}
Zerlegen Sie diesen Quotienten in Zähler und Nenner, bilden Sie die Ableitungen
und setzen Sie diese mit Hilfe der Quotientenregel zusammen,
um die Ableitung von $f(x)=\frac{\ln(x)}{x^2}$ zu berechnen:
\begin{MExerciseItems}
\item{Zähler \MEquationItem{$u(x)$}{\MLSimplifyQuestion{10}{ln(x)}{5}{x}{5}{512}{DS7}} hat Ableitung \MEquationItem{$u'(x)$}{\MLSimplifyQuestion{10}{1/x}{5}{x}{5}{512}{DS8}}.}
\item{Nenner \MEquationItem{$v(x)$}{\MLFunctionQuestion{10}{x^2}{5}{x}{5}{DZ2}} hat Ableitung \MEquationItem{$v'(x)$}{\MLFunctionQuestion{10}{2*x}{5}{x}{5}{DZ1}}.}
\item{Der Quotient hat die Ableitung \MEquationItem{$f'(x)$}{\MLSimplifyQuestion{30}{(1/x*x^2-ln(x)*2*x)/(x^4)}{5}{x}{5}{512}{DS9}}.}
\end{MExerciseItems}
\begin{MHint}{Lösung}
Die vier Bausteine sind
$$
...
Zum Abschluss wollen wir die Verkettung (Modul \MRef{VBKM06}, Abschnitt \MRef{Verkettung}) von Funktionen untersuchen. Wir wollen also wissen, was passiert, wenn wir eine Funktion $u$ (innere Funktion) in eine andere Funktion $v$ (äußere Funktion) einsetzen. Eine solche Verkettung wird in der Mathematik durch die Schreibweise $f(x) := (v \circ u)(x) = v(u(x))$ dargestellt. Diese bedeutet, dass wir den Wert einer Funktion $u$ in Abhängigkeit von der Variable $x$ bestimmen. Diesen so berechneten Wert $u$ verwenden wir als neue Variable, die wir in $v$ einsetzen. Aus dieser Kombination berechnen wir schließlich den endgültigen Funktionswert $v(u(x))$.

\begin{MXInfo}{Kettenregel}
Die Ableitung der Funktion $f(x) := (v \circ u)(x) = v(u(x))$ kann mit der \textbf{Kettenregel} bestimmt werden:
$$
f'(x) = v'(u) \cdot u'(x) \MDFPeriod%%
$$
Hierbei ist $v'(u)$ als Ableitung der Funktion $v$ nach $u$ zu verstehen. Bei diesem Teil der Ableitung wird die Abhängigkeit der Funktion $u$ von der Variable $x$ nicht berücksichtigt. Es gilt der Merksatz: Innere Ableitung $\times$ äußere Ableitung.
\end{MXInfo}

Machen wir uns diese Ableitungsregel an einigen Beispielen klar:

\begin{MExample}
Bestimmen wir die Ableitung der Funktion $f(x) = (3 - 2 x)^5$. Im Vergleich mit der Kettenregel müssen wir eine innere und eine äußere Funktion ablesen. Verwenden wir $u(x) = 3 - 2x$ als innere Funktion, dann können wir $v(u) = u^5$ als äußere Funktion ablesen. Setzen wir $u(x)$ für $u$ in $v(u)$ ein, erhalten wir die ursprüngliche Funktion $f$.

Um die Ableitung von $f$ zu bestimmen, leiten wir zunächst $u$ nach $x$ ab und erhalten $u'(x) = -2$ als innere Ableitung. Für die äußere Ableitung leiten wir $v$ nach $u$ ab und finden $v'(u) = 5 u^4$. Setzen wir diese Zwischenergebnisse in die Kettenregel ein, so bekommen wir die Ableitung $f'$ der Funktion $f$:
\[
f'(x) = 5 u^4 \cdot (-2) = 5 (3 - 2 x)^4 \cdot (-2) = -10 (3 - 2 x)^4 \MDFPeriod %%
\]

Sehen wir uns ein zweites Beispiel an und bestimmen die Ableitung von $g(x) = \MEU^{x^3}$ und verwenden die Zuordnung $u(x) = x^3$ für die innere Funktion und $v(u) = \MEU^u$ für die äußere Funktion. Bestimmen wir die innere und äußere Ableitung, dann erhalten wir $u'(x) = 3 x^2$ und $v'(u) = \MEU^u$. Setzen wir beides in die Kettenregel ein, dann finden wir die Ableitung der Funktion $g$:
\[
g'(x) = \MEU^{u} \cdot 3 x^2 = \MEU^{x^3} \cdot 3 x^2 = 3 x^2 \MEU^{x^3} \MDFPeriod %%
\]
\end{MExample}
...
\begin{MExerciseItems}
\item $f(x) := 3 + 5 x$
zu $f'(x) = $\MLParsedQuestion{30}{5}{4}{DG19}
%
\item $g(x) := \frac{1}{4 x} - x^3$
zu $g'(x) = $\MLSimplifyQuestion{30}{-1/(4*x^2) - 3*x^2}{1}{x}{20}{512}{SIMPLE10}
%
\item $h(x) := 2 \sqrt{x} + 4 x^{-3}$
zu $h'(x) = $\MLSimplifyQuestion{30}{1/sqrt(x) - 12/(x^4)}{1}{x}{20}{512}{SIMPLE11}
\end{MExerciseItems}
\end{MExercise}
...
zu $f'(x) = $\MLSimplifyQuestion{30}{-1/(sin(x) * sin(x))}{10}{x}{4}{512}{DG20}
%
\item $g(x) := \sin(3 x) \cdot \cos(3 x)$
zu $g'(x) = $\MLFunctionQuestion{30}{3*cos(6*x)}{10}{x}{4}{DG21}
%
\item $h(x) := \frac{\sin(3 x)}{\sin(6 x)}$
zu $h'(x) = $\MLSimplifyQuestion{30}{3/2 * tan(3*x)/cos(3*x)}{10}{x}{4}{512}{SIMPLE12}
\end{MExerciseItems}
\end{MExercise}
...
\begin{MExerciseItems}
\item $f(x) := \MEU^{5 x}$
zu $f'(x) = $\MLFunctionQuestion{30}{5*exp(5*x)}{10}{x}{4}{DG22}
%
\item $g(x) := x \cdot \MEU^{6 x}$
zu $g'(x) = $\MLFunctionQuestion{30}{(6*x+1)*exp(6*x)}{10}{x}{4}{DG23}
%
\item $h(x) := (x^2 - x) \cdot \MEU^{-2 x}$
zu $h'(x) = $\MLFunctionQuestion{30}{-(2*x^2 - 4*x + 1) * exp(-2*x)}{10}{x}{4}{DG24}
\end{MExerciseItems}
\end{MExercise}
...
Antwort: 
Wir bezeichnen $f^{(k)}$ als die $k$-te Ableitung von $f$.
Damit ist
\begin{itemize}
\item
...
Die Ableitung wurde weiter oben mittels einer Tangenten an den Graphen einer Funktion eingef"uhrt, die die Änderungsrate bzw. Ableitung einer Funktion {\glqq}n"aherungsweise{\grqq} beschreibt. Aus den 
Eigenschaften dieser Geraden können wir auch auf Eigenschaften der Funktion schließen.
\end{MIntro}
...
kleiner werden.
Dazu wird eine Funktion $f: D \rightarrow \R$ betrachtet, die auf $(a\MIntvlSep b) \subseteq D$ 
differenzierbar ist.
\begin{center}
\MTikzAuto{%
\begin{small}
...
$g'$ jeweils ein anderes Vorzeichen hat.
...
Auch wenn wir für die beiden unterschiedenen Teilbereiche dasselbe Monotonieverhalten finden,
...
Seien $a,b \in D$ mit $a<b$. Wir untersuchen eine Funktion $f: D \rightarrow \R$, die auf dem Intervall $(a\MIntvlSep b) \subseteq D$
...
$f'(x) = 3 x^2 \geq 0$ für alle $x$ ist $f$ auf dem gesamten Definitionsbereich monoton wachsend.
...
\begin{MExercise}
In welchen m"oglichst gro"sen offenen Intervallen $(c\MIntvlSep d)$ ist die Funktion
...
%
\item $f$ ist auf \MLParsedQuestion{35}{(1/sqrt(3),infty)}{4}{DEG2} konkav.
\end{itemize}
...
\end{itemize}
Die Maximalstelle von $f$ ist \MLParsedQuestion{8}{-4}{4}{DEG7}.
Die Minimalstelle von $f$ ist \MLParsedQuestion{8}{0}{4}{DEG8}.
\end{MExercise}
\end{MExercises}
...
mit Zuordnungsvorschrift $y = f(x)$ f"ur $x \in (a\MIntvlSep b)$.
...
\end{itemize}
Die Funktion $f$ ist auf den Intervallen des Definitionsbereichs monoton
...
Da $f$ auf ganz $\R$ definiert ist, kann das Monotonieverhalten an den Extremstellen abgelesen werden: $f$ ist monoton fallend auf $(-\infty\MIntvlSep -\sqrt2)$, monoton wachsend auf $(-\sqrt2\MIntvlSep \sqrt2)$
...
Da das Polynom im Nenner von ${f'}'$ stets positiv ist, genügt es, das Vorzeichen des Polynoms $p(x)=8x(x-\sqrt6)(x+\sqrt6)$ im Zähler zu untersuchen. Für $0<x<\sqrt6$ ist es negativ (dort ist $f$ konkav). Für $x>\sqrt6$ ist es positiv (dort ist $f$ konvex). Da $f$ punktsymmetrisch ist folgt, dass $f$ auf den Intervallen $(-\sqrt 6\MIntvlSep 0)$ und $(\sqrt6\MIntvlSep \infty)$ konvex sowie auf $(-\infty\MIntvlSep -\sqrt6)$ und $(0\MIntvlSep \sqrt6)$ konkav ist.
\ \\ \ \\
\textbf{Skizze des Graphen}\\
\MUGraphics{BildKurvendiskussion1.png}{width=0.5\linewidth}{Die Funktion $f(x)$ skizziert auf dem Intervall $[-8\MIntvlSep 8]$.}{width:600px}
\end{MXContent}
...
\ \\ \ \\
Definitionsbereich: \MLIntervalQuestion{25}{(-infty,infty)}{5}{DXG18} \MInputHint{(in Intervallschreibweise \texttt{(a;b)})}.\\
\ \\
...
\ \\
Einziger Schnittpunkt mit der $y$-Achse ist bei \MEquationItem{$y$}{\MLParsedQuestion{5}{0}{5}{DXG2}}.\\
\ \\
Symmetrie: Die Funktion ist\\
\begin{MQuestionGroup}
\begin{tabular}{lll}
\MCheckbox{0}{JCA1} & \ \ & $y$-Achsensymmtrisch,\\
\MCheckbox{0}{JCA2} & \ \ & Punktsymmetrisch zum Ursprung.
\end{tabular}
\end{MQuestionGroup}
\MGroupButton{Auswahl überprüfen}
\ \\
Grenzverhalten: Die Asymptoten am Rand sind \MLFunctionQuestion{20}{-infty}{4}{infty}{4}{DXG3} am für $x\rightarrow \infty$ und \MLParsedQuestion{15}{0}{3}{DXG5} für $x\rightarrow-\infty$.\\
\ \\
Ableitungen: Es ist $f'(x)$ = \MLFunctionQuestion{20}{-(x^2-2)*exp(x)}{4}{x}{4}{DXG6} sowie ${f'}'(x)$ = \MLFunctionQuestion{30}{-(x^2+2*x-2)*exp(x)}{5}{x}{5}{DXG7} .\\
\ \\
...
\ \\
Extremwerte: Die Stelle $x_1$ = \MLParsedQuestion{13}{-sqrt(2)}{3}{DXG9} ist ein Minimum und $x_2$ = \MLParsedQuestion{13}{sqrt(2)}{3}{DXG10} ist ein Maximum.\\
\ \\
Wendepunkte: Die Menge der Wendepunkte (nur $x$-Komponente) ist \MLParsedQuestion{30}{-1-sqrt(3),-1+sqrt(3)}{3}{DXG11}\\\MInputHint{(in Mengenschreibweise, Wurzeln dürfen stehenbleiben)}.\\
\ \\
...
\ \\ \ \\
\textbf{Skizze des Graphen}\\
\MUGraphics{BildKurvendiskussion3.png}{width=0.5\linewidth}{Die Funktion $f(x)$ skizziert auf dem Intervall $(-\MZahl{6}{2}\MIntvlSep 3)$.}{width:400px}
\end{MHint}
...
\end{eqnarray*}
%
Setzen wir diese Formel in die Funktion $O(r,h)$ ein, reduziert $O$ sich auf eine Funktion in nur einer Variablen:
%
\begin{eqnarray*}
...
\begin{MExercise}
In einem Beh"ahlter wird um $9$ Uhr eine Temperatur von $-10^{\circ}{\MEinheit[]{C}}$ 
...
\begin{MExercise}
Zu einer Funktion $f: [-3\MIntvlSep 2] \rightarrow \R$ geh"ort die Ableitung $f'$, deren 
...
$f'(x) = $\MLSimplifyQuestion{30}{(3 x + 2)/(x^2 + x)}{1}{x}{20}{0}{SIMPLE13}
\item $g(x) := x \cdot \MEU^{-x}$ die zweite Ableitung
${g'}'(x) = $\MLSimplifyQuestion{30}{(x - 2) * exp(-x)}{10}{x}{4}{0}{SIMPLE14}
\end{MExerciseItems}
...
In welchen Bereichen ist $f: (0\MIntvlSep  \infty) \rightarrow \R$ monoton fallend, 
