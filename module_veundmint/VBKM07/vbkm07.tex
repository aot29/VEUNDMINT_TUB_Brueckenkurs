...
an der Stelle $x_0$ gleich $f'(x_0) = m$ ist. (Überprüfen Sie dies gerne selbst!)
...
\begin{MExample}
Wir untersuchen die Funktion $f: \R \rightarrow \R$ mit $x \mapsto f(x) = 5 x^3$.
Vergleichen wir die gegebene Funktion mit den oben verwendeten Bezeichnungen, dann sind $r = 5$ und $n = 3$. Damit erh"alt man für den Wert der 
...
dass ihre Ableitung $f'$ wiederum die Exponentialfunktion ist, also $f'(x) = \exp(x)$ gilt.
\end{MInfo}
...
Wir geben hier die Ableitung der Logarithmusfunktion ohne Beweis an. Für $f: (0 \MIntvlSep \infty) \rightarrow \R$ mit $x \mapsto f(x) = \ln(x)$
...
\begin{MXContent}{Vielfaches und Summe von Funktionen}{Vielfache und Summen}{STD}
...
Sind $u$ und $v$ als differenzierbare Funktionen vorgegeben, dann ist auch die Summe der Funktionen
...
können wir hier die Teilergebnisse mit der Quotientenregel zur Ableitung der Funktion $g$ zusammensetzen; es folgt:
\[
...
Dazu verwenden wir die Zuordnung $x \mapsto u(x) = x^3$ für die innere Funktion $u$ und $u \mapsto v(u) = \MEU^u$ für die äußere Funktion $v$.
Bestimmen wir die innere und äußere Ableitung, dann erhalten wir $u'(x) = 3 x^2$ und $v'(u) = \MEU^u$.
