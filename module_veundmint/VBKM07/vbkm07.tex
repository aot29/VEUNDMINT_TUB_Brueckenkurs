...
Der Verlauf des Graphen "andert seine Richtung im Punkt $\MPointTwo{0}{0}$ sprunghaft: Salopp ausgedr"uckt, weist der Funktionsgraph im Punkt $\MPointTwo{0}{0}$ einen Knick auf.
%Bild:
\begin{center}
\MTikzAuto{%
...
Denn die Tangente an den Funktionsgraphen durch den Punkt $\MPointTwo{0}{0}$ verl"auft 
...
\[
g': \R \rightarrow \R \MDFPSpace , \MDFPaSpace g'(x) = -\sin(x) \MDFPeriod %%
\]
F"ur die Tangensfunktion $h: \R \setminus \{ \frac{\pi}{2} + k \pi \MCondSetSep k \in \Z \} \rightarrow \R$,
...
erkennen, ob eine Funktion auf dem Intervall $\MoIl a \MIntvlSep b\MoIr$ monoton wachsend oder monoton fallend ist.
%\end{MXContent}
...
auf $\MoIl -\infty\MIntvlSep 0\MoIr$ monoton fallend ist, bedeutet also, die Einschr"ankung von $h$ auf dieses Intervall
...
\begin{MXInfo}{Kr"ummungseigenschaften}
 Ist ${f'}'(x) \geq 0$ f"ur alle $x$ zwischen $a$ und $b$, dann hei{\ss}t $f$ auf dem Intervall $\MoIl a\MIntvlSep b\MoIr$ \textbf{konvex (linksgekrümmt)}.

 Ist ${f'}'(x) \leq 0$ f"ur alle $x$ zwischen $a$ und $b$, dann hei{\ss}t $f$ auf dem Intervall $\MoIl a\MIntvlSep b\MoIr$ \textbf{konkav (rechtsgekrümmt)}.
\end{MXInfo}
...
\begin{MExercise}
In welchen m"oglichst gro"sen offenen Intervallen $\MoIl c\MIntvlSep d\MoIr$ ist die Funktion $f$ mit
...
\begin{MHint}{Lösung}
 ...
 echt größer $0$ und $f$ daher dort linksgekrümmt (konvex). Auf $\MoIl[\left] \frac{1}{\sqrt{3}} \MIntvlSep \infty \MoIr[\right]$ gilt ${f'}'(x) < 0$; somit ist $f$ dort
...
Gesucht sind jeweils m"oglichst gro"se offene Intervalle $\MoIl c\MIntvlSep d\MoIr$, auf 
...
mit Abbildungsvorschrift $x \mapsto y = f(x)$ f"ur $x \in \MoIl a\MIntvlSep b\MoIr$.
...
\begin{MHint}{Lösung}
\textbf{Maximaler Definitionsbereich}\\
Es gilt $f(x) = - x (x - 2) \MEU^x$ und $\MEU^x > 0$ f"ur alle  $x \in \R$, damit ist $D_f = \R = \MoIl -\infty \MIntvlSep \infty\MoIr$
...
Man betrachte die Funktion $f: \MoIl 0\MIntvlSep  \infty\MoIr \rightarrow \R$, $x \mapsto f(x)$ mit $f'(x) = x \cdot \ln x$.
In welchen Bereichen ist $f$ monoton fallend, in welchen Bereichen ist $f$ konkav?
%
Geben Sie als Bereiche m"oglichst gro"se offene Intervalle $\MoIl c\MIntvlSep  d\MoIr$ an:
\begin{MExerciseItems}
