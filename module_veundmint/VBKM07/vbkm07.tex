...
in einem gewissen Bereich {\glqq}n"aherungsweise{\grqq}. Aus den Eigenschaften dieser Geraden kann auch auf Eigenschaften der
...
Gegenstand der Untersuchung ist eine Funktion $f: D \rightarrow \R$, die auf dem Intervall $(a\MIntvlSep b) \subseteq D$
differenzierbar ist. Ist deren Ableitung $f'$ ebenfalls auf dem Intervall $(a\MIntvlSep b) \subseteq D$ differenzierbar, so hei{\ss}t $f$ \textbf{zweimal differenzierbar}.
...
\begin{MXInfo}{Kr"ummungseigenschaften}
 Ist ${f'}'(x) \geq 0$ f"ur alle $x$ zwischen $a$ und $b$, dann hei{\ss}t $f$ auf dem Intervall $(a\MIntvlSep b)$ \textbf{konvex (linksgekrümmt)}.

 Ist ${f'}'(x) \leq 0$ f"ur alle $x$ zwischen $a$ und $b$, dann hei{\ss}t $f$ auf dem Intervall $(a\MIntvlSep b)$ \textbf{konkav (rechtsgekrümmt)}.
\end{MXInfo}
...
und monoton fallend auf $(\sqrt2\MIntvlSep \infty)$. Monotonieintervalle werden stets in offener Form angegeben.
\ \\ \ \\
\textbf{Wendestellen}\\
...
Als Beispiel werde die Aufgabe betrachtet, eine zylinderf"ormige Dose zu konstruieren. Die Dose soll zus"atzlich die Bedingung erf"ullen, ein 
Fassungsverm"ogen (Volumen) $V$ von einem Liter, also einem Kubikdezimeter ($1 \MEinheit{dm}^3$), zu haben. Wählt man für $V$ die
...
Um Arbeitsmaterial zu sparen, wird nach derjenigen Dose gesucht, die eine m"oglichst kleine Oberfl"ache
...
Das obige Beispiel soll etwas genauer betrachtet werden: Es geht also um die Minimierung der Oberfläche einer zylinderförmigen Dose bei einem vorgegebenen Volumen
...
\end{eqnarray*}
%
Nach dem Einsetzen dieser Formel in die Funktion $O(r,h)$ resultiert eine Funktion, die nur noch von einer Variablen abhängt und die der
...
Einsetzen dieses Ergebnises in die zweite Ableitung von $O$  dient der Überprüfung, ob wirklich ein Minimum gefunden wurde
(${O'}'(r) = 4\pi + 4/r^3$):
\begin{eqnarray*}
