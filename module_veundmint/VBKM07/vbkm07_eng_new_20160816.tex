...
A family is travelling on holiday by car. The car is moving with a velocity of 
$60\MEinheit{km}/\MEinheit[]{h}$ through a roadworks site. The sign
at the end of the roadworks site says that the speed limit is as of now  
...
the \emph{acceleration} (= change of velocity per time) equals in the present case 
this constant rate of velocity change: The acceleration is the quotient of 
...
at a constant rate of change but at a \emph{time-dependent} rate of change.
...
Similar relations can also be found in other technical fields, as for example, in the calculation of 
internal forces acting in steel frames of buildings, in the forecast of atmospheric and ocean 
currents, or also in the nowadays so relevant modelling of financial markets.
...
to find their slopes or rates of change, respectively. Even thought these calculations 
...
play an important role in many scientific fields and are often investigated as outstanding
quantities.
...
the graph of $f$ shown in the figure below. We like to describe the rate of change of this
...
\]
that describes how the function values of $f$ between $x_0$ and $x$ change \textbf{in average}.
...
This limiting process, i.e. if $x$ approaches $x_0$, is described here and in the following 
by the symbol 
\[
...
\begin{MExercise}
  Let the function $f: \R \rightarrow \R$ with $x \mapsto f(x)=x^2$ and a point $x_0=1$ be given. 
...
Using the formula for the relative rate of change, the derivative can only be 
calculated very cumbersome and also only for very simple functions. Typically, 
...
In mathematics as well as in the sciences and engineering, different notations for 
derivatives are used equivalently:
\[
...
\]
where $x = x_0 + h$. There is no statement whether $x$ has to be greater or less than $x_0$. 
...
\]
If this limit exists \textbf{for all} points $x_0$ in a functions domain, then the
function is said to be \textbf{differentiable} (everywhere). Many of the common functions
are differentiable. However, a simple example for a function that is not differentiable everywhere 
...
i.e. if $h$ approaches $0$, results in two different values ($1$ and $-1$).
...
Likewise, also in the case that a function has a jump at a certain point, a unique tangent line to the graph at 
...
Most of the common functions, as for example, polynomials, trigonometric functions, and exponential functions
...
\]
This differentiation rule is also true for $n\in\mathbb{R}\setminus\{0\}$.
\end{MXInfo}
...
According to the notation above, this is a function with $r = 5$ and $n = 3$. Thus, 
we have for the value of the derivative at the point~$x$
\[
...
The latter also results form the calculation rules explained in the following and
...
special property that its derivative  $f'$ is in turn the exponential function, i.e. 
$f'(x) = \MEU^{x} = \exp(x)$.
\end{MInfo}
...
applying your knowledge on the differentiation of common functions
($x > 0$):
\begin{MExerciseItems}
...
\begin{MHint}{Solution}
 \begin{MExerciseItems}
  \item It is $f(x) = x^6 \cdot x^\frac72 = x^{6 + \frac72} = x^\frac{19}{2}$.
  Thus, we have $f'(x) = \frac{19}{2} x^{\frac{19}{2} - 1} = \frac{19}{2} x^\frac{17}{2}$.
  \item It is $g(x) = \frac{x^{-\frac32}}{\sqrt{x}} = x^{- \frac32} \cdot x^{- \frac12} = x^{- \frac32 - \frac12} = x^{-2}$.
  Thus, we have $g'(x) = (-2) \cdot x^{-2 -1} = -2 \cdot x^{-3} = - \frac{2}{x^3}$.
 \end{MExerciseItems}
\end{MHint}
\end{MExercise}
...
($\cos(x) \neq 0$). With respect to the quotient rule we set  $u(x) = \sin(x)$ and $v(x) = \cos(x)$.
The corresponding derivatives are  $u'(x) = \cos(x)$ and $v'(x) = -\sin(x)$. Combining these terms 
according to the quotient rule results in 
the derivative of the function~$g$:
\[
g'(x) = \frac{\cos(x) \cdot \cos(x) - \sin(x) \cdot (-\sin(x))}{\cos^2(x)} \MDFPeriod%
\]
This result can be transformed into one of the following expressions:
\[
...
described in Module~\MNRef{VBKM05} (see Section~\MRef{Abschnitt:TrigonometrieAmDreieck}).
\end{MExample}
...
$$
where the last transformation step (cancelling $x$) is only for simplifying the expression.
\end{MHint}
\end{MExercise}
\end{MXContent}
...
Last, we investigate the composition (see Module~\MNRef{VBKM06}, Section~\MRef{Verkettung}) of functions:
...
calculated applying the \textbf{chain rule}; we have:
$$
...
The following mnemonic phrase is useful: The derivative of a composite function is the product 
...
For applying the chain rule, inner and outer functions are to be identified. If we 
take for the inner function $u$ the function $u(x) = 3 - 2x$, then the outer function 
$v$ is given by $v(u) = u^5$. With this, we have $v(u(x)) = f(x)$, as required.
...
As a second example, the derivative of $g: \R \rightarrow \R$ with $g(x) = \MEU^{x^3}$ is calculated.
...
\begin{MHint}{Solution}
 \begin{MExerciseItems}
  ...
  \]
  \item Form the product rule and the chain rule, we find
  \[
...
whether for increasing values of $x$ the corresponding function values 
increase or decrease. For this purpose, we consider a function $f: D \rightarrow \R$ 
...
$f'(x) \geq 0$ and hence, $f$ is monotonically increasing.
...
$f$ is called \textbf{twice-differentiable}. If the derivative of the first derivative of $f$ is taken, then 
$(f')' = {f'}'$ is called the \textbf{second derivative} of the function $f$.
...
this notation is used in generally written formulas also for the (first) derivative ($k=1$)
...
\]
Since the denominator of $f'(x)$ is always positive, the sign of $f'(x)$ is determined only by the numerator:
...
\begin{MHint}{Solution}
 ...
 The simple roots of ${f'}'(x)$ are at $x_0 = \pm \frac{1}{\sqrt{3}}$. Thus, for $x>0$, the derivative ${f'}'(x)$
 is greater than $0$ on the open interval $\MoIl[\left] 0 \MIntvlSep \frac{1}{\sqrt{3}} \MoIr[\right]$, and hence,
 the function $f$ is convex (left curved) on this region. On the interval 
 $\MoIl[\left] \frac{1}{\sqrt{3}} \MIntvlSep \infty \MoIr[\right]$, we have ${f'}'(x) < 0$; hence, the function
...
\begin{MExercise}
Let a function $f: [-\MZahl{4}{5} \MIntvlSep 4] \rightarrow \R$ with $f(0) := 2$ be given; its derivative 
...
\begin{MHint}{Solution}
 ...
 At an extremum point $x_e$ (maximum or minimum point) of a function $f$ (which does not lie on the boundary of the 
 domain) the first derivative is zero: $f'(x_e) = 0$. Graphically this means, that the tangent line to the graph of 
 $f$ is a horizontal line. According to the exercise text, the zeros of $f'(x)$ are at $x_ 1 = -4$, $x_2 = 0$, and $x_3 = 3$.
 Since $f$ is monotonically increasing on $\MoIl -\MZahl{4}{5} \MIntvlSep -4\MoIr$ and monotonically decreasing on
 $\MoIl -4 \MIntvlSep 0\MoIr$, $x_1 = - 4$ is a maximum point. Accordingly, it is reasoned that the function 
...
\begin{MXContent}{Curve Sketching}{Curve Sketching}{STD}
...
\begin{description}
\item[Maximum Domain:]
All real numbers $x$ are determined for which $f(x)$ exists. The set $D$
of these numbers is called maximum domain.
...
$x \in D$. Then the function $f$ is also called \textbf{even}. If $f(-x) = -f(x)$
...
is also called \textbf{odd}.


\item[Asymptotic Behaviour at the Domains Boundary:]
...
\item[Extremum Values and Monotony:]

The necessary condition, for $x$ to be an extremum point (if $x \in D$ is not a boundary point of $D$), is $f'(x) = 0$.\newline
...
The maximum domain of this function is $D_f=\R$ since the denominator of the function is $x^2+2\geq 2$, i.e. it is always 
non-zero, and hence no points have to be excluded.
\ \\ \ \\
\textbf{$x$- and $y$-Intercepts}\\
The zeros of the function are the zeros of the numerator. Hence, the graph of $f$ intersects the $x$-axis only in the origin 
$\MPointTwo{0}{0}$ since the numerator is only zero for $x=0$. At the same time, this is the only intersection point 
...
The function is defined on the entire set of real numbers $\R$, and thus, only the limiting behaviour
...
\end{eqnarray*}
\ \\
\textbf{Extremum Values}\\
The necessary condition for $x$ to be an extremum point, is $f'(x)=0$, in this case $-x^2+2=0$.
...
$$
Hence, $x_1$ is a maximum point and $x_2$ is a minimum point of $f$. Inserting these values in $f$ results
...
the position of the extremum points and their types:
...
\begin{MExercise}
Carry out a complete curve sketching for a function $f$ with $f(x) = (2 x - x^2) \MEU^x$ and enter 
...
\ \\
Extremum values: The point $x_1$ = \MLParsedQuestion{13}{-sqrt(2)}{3}{DXG9} is a minimum point and 
...
\ \\
Sketch the graph, and subsequently, compare your result to the sample solution.
...
\end{eqnarray*}
\ \\
\textbf{Monotony Behaviour and Extremum Values}\\
The solutions of $f'(x) = 0$ are $x_1 = -\sqrt{2}$ and $x_2 = \sqrt{2}$. 
Furthermore, we have $x_1 < x_2$ and 
$$
f'(x)\; =\; - (x + \sqrt{2}) (x - \sqrt{2}) \MEU^x \MDFPeriod
$$
On $\MoIl[\left]-\infty\MIntvlSep  -\sqrt{2}\MoIr[\right]$ the first derivative $f'$ is negative 
and hence, $f$ is monotonically decreasing there.
On $\MoIl[\left]-\sqrt{2}\MIntvlSep  \sqrt{2}\MoIr[\right]$ the first derivative $f'$ is positive
and hence, $f$ is monotonically increasing there.
On $\MoIl[\left]\sqrt{2}\MIntvlSep  \infty\MoIr[\right]$ the first derivative $f'$ is negative and hence,
...
In many applications in engineering and business, problem solutions can be found that are not unique. 
Often they depend on variable conditions. To find an ideal solution, additional properties (constraints)
are defined that are to be satisfied by the solution. This very often results in so called 
\textbf{optimisation problems}, in which from a family of solutions that one solution has to be selected 
that satisfies a previously specified property the best.

As an example, we consider the problem of constructing a cylindrical can. This can shall satisfy the 
additional condition to have a capacity (volume) $V$ of one litre, i.e. one cubic decimetre ($1 \MEinheit{dm}^3$).
Thus, if $V$ is specified in $\MEinheit[]{dm}^3$ and $r$ is the radius and $h$ the height of the can, 
each measured in decimetre ($\MEinheit[]{dm}$), then the volume shall be $V = \pi r^2 \cdot h = 1$.
...
additional condition for the volume: $V = \pi r^2 \cdot h = 1$. In the context of finding extremum points, 
...
extremum point $x_{\text{ext}}$ of a function $f$ satisfying a given equation 
$g(x_{\text{ext}}) = b$.

If we search for a minimum point, this problem is also called a \MEntry{minimisation problem}{minimisation problem}. 
If we search for a maximum point, this problem is also called a \MEntry{maximisation problem}{maximisation problem}.  

The function $f$ is called \MEntry{target function}{target function}, and the equation $g(x) = b$
is also called \MEntry{constraint}{constraint} of the optimisation problem.
\end{MXInfo}
\end{MXContent}
...
to normal extremum value problems of functions. Thus, we take the first derivative of the function $O$ with respect to the 
variable $r$ and let this derivative be equal to zero:
%
\begin{eqnarray*}
...
Substituting this result into the second derivative of $O$ shows whether actually a minimum was found 
(${O'}'(r) = 4\pi + 4/r^3$):
\begin{eqnarray*}
...
is fallen to $-140^{\circ}{\MEinheit[]{C}}$.

\begin{MExerciseItems}
\item What is the average rate of temperature change due to the first and the second measurement?
...
%Temperatur f"allt?
%
\item In the (average) rate of change, the property of a falling temperature shows in the fact
...
$f'(x) = x \cdot \ln x$. On what regions is $f$ monotonically decreasing, on what regions is $f$ concave? 
%
