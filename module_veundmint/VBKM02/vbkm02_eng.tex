...
applying transformations and solution formulas from a formulary. For a more sophisticated equation, a case analysis may be required. This module consists of
...
\MEntry{proportionality}{proportionality}. Examples include 
...
\MEntry{rule of three}{rule of three} and is taught in great detail in the schools of some countries such as Germany and France.
...
$$
\MInputHint{In an input field you would type this as \texttt{falls(x>=2;2*x-4;-2*x+4)}. Or alternatively as \texttt{falls(x<2;-2*x+4;2*x-4)}.}
...
\end{center}
\MInputHint{Enter the case analysis in the form \texttt{falls(CONDITION;W1;W2)}, where \texttt{W1} is the value of the expression if the corresponding condition is satisfied. Do not use the absolute value function.}
\begin{MHint}{Solution}
\begin{eqnarray*}
...
\end{eqnarray*}
\end{MHint}\\
\MInputHint{Enter the case analysis in the form \texttt{falls(CONDITION;W1;W2)}, where \texttt{W1} is the value of the expression if the corresponding condition is satisfied.
