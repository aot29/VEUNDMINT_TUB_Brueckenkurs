...
%sonst unter dem Bildschirmrand landet.
\begin{MExercise}
%Aufgabentext ueberarbeitet (jgl):
Angenommen, es ist $F$ eine Stammfunktion von $f$ mit $f(x)=1+x^2$ und besitzt 
den Wert $F(0)=1$, dann ist $F(3)$ = \MLParsedQuestion{10}{13}{10}{IG22}.
%Loesungshinweis erstellt (jgl):
\begin{MHint}{L"osung}
...
Intervall $[a\MIntvlSep b]$. Dann nennt man
%
\begin{equation}
...
Untergrenze $a$ und der Obergrenze $b$ (wenn der Grenzwert existiert), und die 
Funktion $f$ hei"st \MEntry{integrierbar}{integrierbar}.
\end{MXInfo}
%
%\begin{MXInfo}{Integral}
...
$f: [0\MIntvlSep 1] \rightarrow \R,\, x \mapsto x$ 
berechnet, wobei die Berechnung des Grenzwertes im Vordergrund steht.

\begin{MExample}
Es soll das Integral von $f: [0\MIntvlSep 1] \rightarrow \R,\, x \mapsto x$ 
berechnet werden.
Dazu teilt man das Intervall $[0\MIntvlSep 1]$ in Teilintervalle 
$[x_{k}\MIntvlSep x_{k+1}]$ mit 
...
$x_k = \frac{k}{n}$ und damit auch $f(x_k) = x_k = \frac{k}{n}$.

Setzt man diese Ergebnisse in die Summenformel ein, dann erhält man unter der 
Verwendung von $\sum_{k=1}^{n-1} k = \frac{1}{2}\,n(n-1)$ ("`kleiner Gauß"') die
Gleichung
%
\begin{eqnarray*}
...
\begin{MHint}{L"osung}
Die Fl"ache wird mit Geraden durch $x_0 = -3$, $x_1 = -1$, $x_2 = 0$, 
$x_3 = \frac{3}{2}$, $x_4 = 2$, $x_5 = 3$ und $x_6 = 4$ in Teilfl"achen zerlegt,
die jeweils durch den Graphen von $f$, die $x$-Achse und durch die Geraden 
$x_{k-1}$ und $x_k$ f"ur $1 \leq k \leq 6$ begrenzt werden.
...
beispielsweise durch $z_0 = -3$, $z_1 = -1$, $z_2 = \frac{1}{2}$ und $z_3 = 4$ 
...
\item{\MEquationItem{$\displaystyle \int_{0}^{4} \left(4 - x\right) \MDwSp x$}{\MLParsedQuestion{10}{8}{4}{IG27}}.}
\end{MExerciseItems}
%Loesungshinweise erstellt (jgl):
\begin{MHint}{L"osung}
...
%
\item $\displaystyle \int_{0}^{4} \left(4 - x\right) \MDwSp x %
...
$\displaystyle \int_{-\pi}^{\pi} \left(5 x^3 - 4 \sin(x)\right) \MDwSp x$
...
\]
den Wert $0$ ergibt: %
\MEquationItem{$z$}{\MLParsedQuestion{10}{-14/6}{4}{ICONSTFIND}}.
...
$$
also muss man $z=-\frac{14}{6} = -\frac{7}{3}$ einsetzen.
\end{MHint}
\end{MExercise}
...
$F$ mit $F(x) = 2 x^3 - 2 x^2 + x$ eine Stammfunktion ist. Mit dem Hauptsatz folgt
\[
...
$F$ mit $F(x) = \frac{1}{2} \ln|x|$ eine Stammfunktion von $f$. Mit dem Hauptsatz 
folgt
\[
...
\[
 ...
 = 6 \MDFPeriod %
\]
Im zweiten Aufgabenteil ist der Integrand $f(x) = 1 - 2 x$ eine Polynomfunktion.
Damit ist $F(x) = x - x^2$ eine Stammfunktion von $f$. Mit dem Hauptsatz folgt
\[
...
\begin{MExample}
Zu berechnen ist der Flächeninhalt $I_A$ der Fl"ache, den die Funktion 
$f(x) = \frac{1}{2} x^3$ im Bereich $[-2\MIntvlSep 2]$ mit der $x$-Ache 
...
x $[-2\MIntvlSep 0]$ und $[0\MIntvlSep 2]$ auf und berechnet mit
%
\begin{eqnarray*}
...
In der Rechnung wurde nach dem Ziehen der ersten Wurzel auf eine nähere 
...
Die Werte von $f(x)$ sind kleiner als die von $g(x)$ auf diesem Intervall, also
...
\end{eqnarray*}
%
Dies sollen nur zwei Beispiele aus dem physikalisch-technischen Bereich sein. 
