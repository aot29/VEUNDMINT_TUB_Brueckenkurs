...
Im letzten Kapitel wurden Ableitungen von Funktionen behandelt. Natürlich stellt sich,
...
sehr einfach erklärt. Kann man einer Funktion $f$ eine Ableitung $f'$ zuordnen und fasst auch $f'$ als Funktion auf,
so kann man auch der Funktion $f'$ eine Funktion $f$ zuordnen, indem man die Operation "`Ableitung"' rückgängig macht.
Man dreht in diesem Kapitel also die Fragestellung um: Kann man zu einer Funktion $f$ eine andere Funktion finden,
deren Ableitung wieder die Funktion $f$ ist?

Die Anwendungen der Integralrechnung sind genauso vielfältig wie die Anwendungen der Differentialrechnung. Untersucht man z.B. in der Physik die Kraft $F$, die auf einen Körper wirkt, dann kann man unter Verwendung des bekannten Zusammenhangs $F = m a$ ($m$: Masse des Körpers, $a$: Beschleunigung des Körpers) zunächst aus der Kraft die Beschleunigung $a = F/m$ berechnen. Interpretiert man die Beschleunigung als Änderungsrate der Geschwindigkeit, $a = \frac{\MD v}{\MD t}$, dann kann man anschließend die Geschwindigkeit über die Umkehrung der Ableitung -- also durch die Integralrechnung -- bestimmen. Ähnliche Zusammenhänge lassen sich in vielen Bereichen aus Naturwissenschaften, Technik und auch Wirtschaftswissenschaften finden. So benötigt man die Integralrechnung zur Bestimmung von Flächen, von Schwerpunkten, Biegeeigenschaften von Balken oder zur Lösung sogenannter Differentialgleichungen, mit denen die meisten Probleme im naturwissenschaftlich-technischen Umfeld beschrieben werden.
\end{MIntro}
...
%\[
%\int \frac{1}{x} \MDwSp x = \ln(-x) = \ln|x|, %%
%\]
%wenn $x < 0$ gilt (hier ist $|x| = -x$).
%\end{MExample}

Notiert man die Beziehung zwischen Ableitung $f=F'$ und Stammfunktion $F$ in der eben besprochenen umgekehrten Sichtweise f"ur die bisher betrachteten Funktionsklassen, ergibt sich die folgende Tabelle:
...
Aber woher kennt man den Wert dieser Konstanten $C$? Schließlich ist die Ableitung jeder beliebigen
...
Haben die Funktionen $F$ und $G$ dieselbe Ableitung $f = F' = G'$, dann ist $G'(x) - F'(x) = 0$. Bildet man nun
auf beiden Seiten der Gleichung die Stammfunktion, dann erhält man den Zusammenhang $G(x) - F(x) = C$. Somit
ist $G(x) = F(x) + C$. Hat man also mit $F$ eine Stammfunktion von $f$ gefunden, dann ist auch $G$ mit $G(x) = F(x) + C$
...
Somit wird durch 
\[
\int (10 x - 6) \MDwSp x = 5 x^2 - 6 x + C
\]
...
f(x) = 1 + \tan^2(x) = \frac{1}{\cos^2(x)}    & F(x) = \int (1 + \tan^2(x)) \MDwSp x = \tan(x) + C \\
f(x) = \MEU^x                                 & F(x) = \int \MEU^x \MDwSp x = \MEU^x + C \\
f(x) = \MEU^{k x}                             & F(x) = \frac{1}{k}\,\MEU^{k x} + C \\
f(x) = \frac{1}{x}                            & F(x) = \int \frac{1}{x} \MDwSp x = \ln|x| + C
%f(x) = \frac{1}{1 + x^2} & F(x) = \int \frac{1}{1 + x^2} \MDwSp x = \arctan(x) + C %
\end{array}
\]
...
Die Überprüfung, ob man die Stammfunktion einer vorgegebenen Funktion $f$ richtig gebildet hat, ist sehr einfach. Man bestimmt die Ableitung der gefundenen Stammfunktion und vergleicht diese mit der ursprünglich vorgegebenen Funktion $f$. Stimmen beide überein, dann war die Rechnung richtig. Stimmt das Ergebnis der Probe nicht mit der Funktion $f$ überein, so muss die Stammfunktion noch einmal überprüft werden.
\end{MXInfo}
\end{MContent}
...
\item{\MEquationItem{$\displaystyle\int |x| \MDwSp x$}{\MLSimplifyQuestion{60}{1/2*x*abs(x)}{10}{x}{4}{32}{LSTNF3}}.
\ \\ \ \\
...
$x$-Intervalls beschrieben wird. Man bestimmt nun die Flächen dieser Rechtecke und summiert diese Teilflächen auf.
...
$$
%
bestimmt. Im betrachteten Beispiel wird das Intervall $[0\MIntvlSep 8]$ in acht gleich große Teile eingeteilt.
Dabei sind $x_0 = 0$, $x_1 = 1$, $x_2 = 2$, $x_3 = 3$, $x_4 = 4$, $x_5 = 5$, $x_6 = 6$, $x_7 = 7$ und $x_8 = 8$. Wendet man darauf diese Summenformel an, so erhält man
%
\begin{eqnarray*}
S_8 &=& f(x_0) \cdot (x_1-x_0) + f(x_1) \cdot (x_2-x_1) + f(x_2) \cdot (x_3-x_2) + f(x_3) \cdot (x_4-x_3)\\
    ...
    &=& f(x_0) \cdot 1 + f(x_1) \cdot 1 + f(x_2) \cdot 1 + f(x_3) \cdot 1 + f(x_4) \cdot 1 + f(x_5) \cdot 1 + f(x_6) \cdot 1 + f(x_7) \cdot 1 \MDFPeriod
\end{eqnarray*}
%
Leider ist dieser Vorgang nicht immer so einfach wie in diesem Beispiel und die Intervalllänge nicht immer $1$. Die Intervalllänge soll schließlich gegen Null gehen, um einen möglichst genauen Wert der Fläche zu berechnen.
% TODO? TS: "..., um einen -- falls möglich -- exakten Wert der Fläche zu berechnen."
\begin{MXInfo}{Integral}
Gegeben ist eine Funktion $f: [a\MIntvlSep b] \rightarrow \R$ auf einem reellen Intervall $[a\MIntvlSep b]$. Dann nennt man
%
\begin{equation}
...
\begin{MExample}
Es soll das Integral von $f: [0\MIntvlSep 1] \rightarrow \R,\, x \mapsto x$ berechnet werden.
Dazu teilt man das Intervall $[0\MIntvlSep 1]$ in Teilintervalle $[x_{k}\MIntvlSep x_{k+1}]$ mit 
$x_0 := 0$ und $x_k := x_{k-1} + \frac{1}{n}$ ein. Die Intervalllänge ist also
$\Delta(x_k) = x_{k+1} - x_{k} = \frac{1}{n}$.

Untersucht man die Intervalllänge auf ihr Verhalten für $n$ gegen unendlich, dann
sieht man, dass $\Delta(x_k)$ immer kleiner wird und gegen Null strebt. Die Voraussetzung
für die Berechnung eines bestimmten Integrals ist also gegeben.

Für die Werte $x_k$ findet man unter Zuhilfenahme der Intervalllänge außerdem $x_k = \frac{k}{n}$ und damit auch $f(x_k) = x_k = \frac{k}{n}$.

Setzt man diese Ergebnisse in die Summenformel ein, dann erhält man unter der Verwendung von $\sum_{k=1}^{n-1} k = \frac{1}{2}\,n(n-1)$ ("`kleiner Gauß"')
%
\begin{eqnarray*}
...
Regeln zur Integration von Funktionen nötig. Ein wichtiges Ergebnis liefert der
...
$f(x) = x^2$ zwischen $a = 1$ und $b = 2$ berechnet. Mit den Regeln für die Bestimmung von
Stammfunktionen und dem Hauptsatz der Differential- und Integralrechnung kann
man diese Aufgabe sehr leicht lösen.
...
Damit ist
\[
\int_1^2 x^2 \MDwSp x = \left[\frac{1}{3} x^{3}\,+\,C\right]_1^2 %
 = \left(\frac{1}{3}\, 2^{3}\,+\,C\right) - \left(\frac{1}{3}\, 1^{3}\,+\,C\right) = \frac{7}{3} \MDFPeriod %%
\]
Wie man sieht, fällt die Konstante nach dem Einsetzen der Grenzen weg, sodass man sie in der Praxis bei der Berechnung von bestimmten Integralen bereits bei der Bildung der Stammfunktion "`unterschlagen"' kann.
...
%
$$
\int_a^b f'(x) \cdot g(x) \MDwSp x = \left[f(x)\cdot g(x)\right]_a^b - \int_a^b f(x)\cdot g'(x) \MDwSp x \MDFPSpace, %%
$$
%
...
$$
%
unter Verwendung der partiellen Integration. Dazu werden die Funktionen
%
\begin{eqnarray*}
f'(x) = \sin x \quad\text{und}\quad g(x) = x
\end{eqnarray*}
%
gewählt; man erhält damit die für die partielle Integration nötigen Funktionen
%
\begin{eqnarray*}
f(x) = -\cos x \quad\text{und}\quad g'(x) = 1 \MDFPSpace.
\end{eqnarray*}
%
So kann man das gesuchte Integral berechnen:
%
\begin{eqnarray*}
\int_{0}^{\pi} x\sin x \MDwSp x &=& \left[(-\cos x)\cdot x\right]_0^\pi - \int_{0}^{\pi} (-\cos x)\cdot 1 \MDwSp x \\
&=& (-\cos\pi)\cdot\pi - 0 + \int_{0}^{\pi} \cos x \MDwSp x \\
&=& -(-1)\cdot\pi + \left[\sin x\right]_0^\pi = \pi \MDFPeriod
\end{eqnarray*}
%
Die Zuordnung der Funktionen $f'$ und $g$ muss zielführend erfolgen. Die Leserin bzw. der Leser mag probieren, dieses Beispiel zu lösen, indem $f'$ und $g$ anders herum gewählt werden!
\end{MExample}
...
Dies soll am Beispiel der Funktion $f$ auf $[-2\MIntvlSep 2]$ mit $f(x) = x^3$ erläutert werden:
\MUGraphics{BildFlaechePolynomxdrei.png}{scale=0.4}{Ungerade Funktion $f(x) = x^3$ auf einem Intervall $[-2\MIntvlSep 2]$.}{}

Man teilt den Graphen von $f$ in zwei Teile zwischen $-2$ und $0$ bzw. $0$ und $2$ ein und untersucht die Teilflächen,
die der Graph in beiden Bereichen mit der $x$-Achse einschließt. Man kann die beiden Teilflächen durch eine Punktspiegelung
ineinander überführen. Beide Teilflächen sind gleich groß. Bildet man jeweils die Riemann-Summen, dann stellt man fest, dass Flächen,
die unterhalb der $x$-Achse liegen, im Integral einen negativen Wert annehmen. Wenn man also die beiden hier abgebildeten Teilflächen addiert,
...
In sehr vielen Situationen wird die Berechung eines Integrals einfacher, wenn man den Integranden vor der Integration in eine bekannte Form bringt. Entsprechende Umformungen sollen im Folgenden an einigen Beispielen betrachtet werden.
Im ersten Beispiel werden Potzenfunktionen untersucht.

\begin{MExample}
Es soll versucht werden, das Integral
\[
\int_{1}^{4} (x - 2) \cdot \sqrt{x} \MDwSp x %%
\]
zu berechnen. Um die Rechnung zu vereinfachen, formt man den Integranden um:
\[
...
Im n"achsten Beispiel wird die Umformung eines Integranden mit trigonometrischen Funktionen durchgeführt.

\begin{MExample}
Bestimmt werden soll das Integral
\[
\int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} \frac{\sin(x)}{\tan(x)} \MDwSp x \MDFPSpace, %%
\]
indem die Definition der Tangensfunktion ausgenutzt wird. Damit erhält man die Umformung
\[
\frac{\sin(x)}{\tan(x)} %
...
\[
\int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} \frac{\sin(x)}{\tan(x)} \MDwSp x %%
= 
\int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} \cos(x) \MDwSp x %%
= 
...
dann führt man zunächst eine Polynomdivision (siehe Modul \MRef{VBKM06}) durch. Je nach Situation können sich auch noch weitere Umformungen (z.B. Partialbruchzerlegung)
anbieten, die man in weiterführender Literatur und Formelsammlungen finden kann. Im folgenden Beispiel wird eine
Polynomdivision durchgeführt, um eine rationale Funktion zu integrieren.

\begin{MExample}
Zu berechnen ist Integral
\[
\int_{-1}^{1} \frac{4 x^2 - x + 4}{x^2 + 1} \MDwSp x \MDFPeriod %%
\]
Dazu formt man zun"achst den Integranden mittels Polynomdivision zu
\[
...
das Integral $\displaystyle \int_{-3}^{4} f(x) \MDwSp x$ mit Methoden aus der Geometrie.
...
Beispielhaft soll hier zunächst die Berechnung von Flächen erörtert werden, deren Ränder durch mathematische Funktionen
...
Zum Abschluss werden einige weitere physikalisch-technische Anwendungen betrachtet.
\end{MIntro}
...
Zur Veranschaulichung ist in der folgenden Abbildung (linkes Bild) die Funktion $f(x) = \frac{1}{2} x^3$ auf dem Intervall $[-2\MIntvlSep 2]$ dargestellt. Das Ziel ist
die Berechnung des Flächeninhalts, der vom Graphen der Funktion und der $x$-Achse eingeschlossen wird. Die bisherigen Untersuchungen ergeben, dass das
...
Spiegelt man jedoch die "`negative"' Fläche an der $x$-Achse, gibt man der Funktion also ein positives Vorzeichen (rechtes Bild), dann kann man den Flächeninhalt
richtig bestimmen. Mathematisch bedeutet das, dass man nicht das Integral der Funktion $f$ berechnet, sondern das Integral des Betrags $\left|f\right|$.
...
Durch die Bildung des Betrags der Funktion benötigt man eine Aufteilung des Integrals in die Bereiche mit positivem und negativem Vorzeichen.
F"ur die Berechnung heißt dies, dass man das Integrationsintervall in Abschnitte zu unterteilt, in denen die Funktionswerte dasselbe Vorzeichen haben.
...
\begin{MExample}
Zu berechnen ist der Flächeninhalt $A$, den die Funktion $f(x) = \frac{1}{2} x^3$ im Bereich $[-2\MIntvlSep 2]$ mit der $x$-Ache einschließt. Die einzige Nullstelle der gegebenen Funktion befindet sich bei $x_0 = 0$. Man teilt den Integrationsbereich also in die beiden Teilintervalle $[-2\MIntvlSep 0]$ und $[0\MIntvlSep 2]$ auf und berechnet mit
%
\begin{eqnarray*}
A &=& \int_{-2}^{2} \left|\frac{1}{2} x^3\right| \MD x = \left|\int_{-2}^{0} \frac{1}{2} x^3 \MDwSp x\right| + \left|\int_{0}^{2} \frac{1}{2} x^3 \MDwSp x\right| \\
...
Man kann nicht nur Flächeninhalte zwischen einer Kurve und der $x$-Achse bestimmen, sondern auch den Inhalt einer Fläche, die von zwei Kurven eingeschlossen wird, wie in der folgenden Abbildung veranschaulicht.
...
In der Rechnung wurde nach dem Ziehen der ersten Wurzel auf eine nähere Betrachtung des Falls $x^2 + 2 = -6$ verzichtet, da man
aus der daraus folgenden Gleichung $x^2 = -8$ keine reellen Nullstellen erhält. Die reellen Nullstellen von $f-g$ sind $-2$ und $2$.
...
ergibt sich der Fl"acheninhalt $I_A$ zu
...
zur Zeit $t$. Es gilt also $v = \frac{\MD s}{\MD t}$, wenn man $v = v(t)$ und $s = s(t)$
als Funktionen der Zeit auffasst.
Der aktuelle Aufenthaltsort $s(T)$ ergibt sich durch die Umkehrung der Ableitung, also durch die Integration der
Geschwindigkeit über die Zeit. Mit dem Anfangswert $s(t = 0) = s_0$ zur Zeit $t = 0$ erhält man damit
%
\begin{eqnarray*}
...
\end{eqnarray*}
%
Ein weiteres Beispiel aus der Physik, das der Leserin oder dem Leser bekannt sein könnte, ist die Bestimmung der Arbeit als Produkt aus Kraft und Weg: $W = F_s \cdot s$. Dabei ist
...
\end{eqnarray*}
%
Dies sollen nur zwei Beispiele aus dem physikalisch-technischen Bereich sein. Im Verlauf des Studiums wird ggf. eine ganze Reihe weiterer Anwendungen der Integration auftauchen.
\end{MXContent}
...
\begin{MHint}{Lösung}
Man betrachtet $f(x) - g(x) = 12 x - 12 x^2 + \sin\left(\pi x\right)$ auf dem Intervall
$[0\MIntvlSep  1]$. Die Funktion $f - g$ wird nur an den R"andern des 
Intervalls $[0\MIntvlSep  1]$ gleich Null. Dies kann man mit einer einfachen Überlegung verstehen:

Auf dem Intervall $\MoIl 0\MIntvlSep  1\MoIr$ ist die Funktion $g\left(x\right) = 12x^2 - 12x = 12x\left(x-1\right)$
immer kleiner als Null, da $x-1$ in diesem Intervall kleiner als Null ist. Nimmt man
nun die Randwerte des Intervalls hinzu, so sieht man, dass die Funktion $g$ dort ihre Nullstellen
hat.

Untersucht man hingegen die Funktion $f\left(x\right) = \sin\left(\pi x\right)$ auf demselben
Intervall, dann findet man, dass diese Funktion dort immer positiv ist und ebenfalls an den
Rändern ihre Nullstellen besitzt.

Berechnet man also $f-g$, dann zieht man auf dem gesamten offenen
Intervall $\MoIl 0\MIntvlSep  1\MoIr$ eine negative Zahl von einer positiven Zahl ab, erhält also durchgehend positive
Werte für $f -g$. Nur an den Rändern rechnet man $0 - 0$ und erhält dort auch für die Differenz
...
I_A & = & \left|\int_0^1 \left(12 x^2 - 12 x + \sin\left(\pi x\right)\right) \MD x \right| \\
 ...
 & = & |-2 + \frac{2}{\pi}| = 2 - \frac{2}{\pi} \MDFPeriod %%
\end{eqnarray*}
\end{MHint}
\end{MExercise}
...
$F: [r_1\MIntvlSep  \infty\MoIr \rightarrow \R, r \mapsto F(r) := -\gamma \cdot \frac{m \cdot M}{r^2}$ 
...
%\item $\displaystyle\int \left(2 x - \frac{1}{x+\pi}\right) \MDwSp x = $%
...
\qquad \text{und} \qquad
\int_{\pi}^{\frac{3\pi}{4}} 5 \sin(3 \pi - 4 x) \MDwSp x = %
