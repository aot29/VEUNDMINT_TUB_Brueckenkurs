...
\end{MExerciseItems}
f"ur $x > 0$, nachdem Sie die Funktionsterme vereinfacht haben:
...
\begin{MHint}{Lösung}
Die Funktion lässt sich zu $f(x)=\frac{8}{x} - \frac{6}{x^2}$ vereinfachen, was 
auf 
\[
...
\item{Mit der Vereinfachung \MEquationItem{$h(x)$}{\MLSimplifyQuestion{30}{1/4+1/(2*sqrt(x))}{10}{x}{4}{0}{ISF3}}\\
ergibt sich \MEquationItem{$H(x)$}{\MLSimplifyQuestion{30}{x/4+sqrt(x)}{10}{x}{4}{32}{ISF3b}} f"ur $x > 0$.\\
\begin{MHint}{Lösung}
Die Funktion lässt sich zu %
$h(x)=\frac{1}{4} + \frac{1}{2 \cdot \sqrt{x}}$ vereinfachen, was auf 
\[
...
\end{eqnarray*}
%
Leider ist dieser Vorgang nicht immer so einfach wie in diesem Beispiel und die 
Intervalllänge nicht immer $1$. Die Intervalllänge soll schließlich gegen Null 
gehen, um einen möglichst genauen Wert der Fläche zu berechnen. 
...
\end{equation}
%
das \MEntry{bestimmte Integral}{Integral (bestimmt)} von $f$ mit der Untergrenze $a$ und der Obergrenze $b$ (wenn der Grenzwert existiert).
\end{MXInfo}
%
%\begin{MXInfo}{Integral}
...
Weiterf"uhrende "Uberlegungen zeigen, dass f"ur jede stetige Funktion das 
Integral existiert.
