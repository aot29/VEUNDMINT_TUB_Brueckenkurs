...
$K(x) = 18$ gegeben. Dann hat $H$ die Ableitung 
\[
H'(x) = 0 \MDFPeriod
\]
...
Zahl $C$ gibt, sodass $F(x) = C$ f"ur alle $x$-Werte des Intervalls gilt.
\end{MXInfo}
...
%Die Sprechweise ist hier: "`$F$ ist das Integral über $f$."' 
Den Zusammenhang zum (bestimmten) Integral beschreibt der
\MEntry{Hauptsatz der Differential- und Integralrechnung}{Hauptsatz (Integral)}
in \MRef{HSDDIR}. Durch die Schreibweise des unbestimmten Integrals wird 
betont, dass zur gegebenen Funktion $f$ eine Funktion $F$ mit $F' = f$ gesucht
wird.
...
f(x) = \sin(k x)                            & F(x) = -\frac{1}{k}\,\cos(k x) + C \\[1ex]
f(x) = \cos(x)                              & F(x) = \int \cos(x) \MDwSp x = \sin(x) + C \\[1ex]
f(x) = \cos(k x)                            & F(x) = \frac{1}{k}\,\sin(k x) + C \\[1ex]
%f(x) = 1 + \tan^2(x) = \frac{1}{\cos^2(x)}  & F(x) = \int (1 + \tan^2(x) \MDwSp x = \tan(x) + C \\[1ex]
f(x) = \MEU^x                               & F(x) = \int \MEU^x \MDwSp x = \MEU^x + C \\[1ex]
f(x) = \MEU^{k x}                           & F(x) = \frac{1}{k}\,\MEU^{k x} + C \\[1ex]
f(x) = x^{-1} = \frac{1}{x}                 & F(x) = \int \frac{1}{x} \MDwSp x = \ln|x| + C %%
%f(x) = \frac{1}{1 + x^2} & F(x) = \int \frac{1}{1 + x^2} \MDwSp x = \arctan(x) + C %%
\end{array}
\]
...
\begin{MExample}
Zur Funktion $f$ mit $f(x) = 10 x - 6 = 10 x^2 - 6 x^0$ wird das unbestimmte 
Integral gesucht.
...
Stammfunktion von $g$ und $H$ mit $H(x) = \frac{1}{1} \cdot x^{0+1} = x$ eine
...
Seite durch den Funktionswert am linken Rand des dazu gehörenden $x$-Intervalls 
beschrieben wird. Man bestimmt nun die Flächen dieser Rechtecke und summiert 
diese Teilflächen auf.
...
%
\[
S_n = \sum_{k=0}^{n-1} f(x_k) \cdot \Delta(x_k) \qquad \text{mit } %
 \Delta(x_k) = x_{k+1} - x_{k} %%
\]
%
bestimmt. Im betrachteten Beispiel wird das Intervall $[0\MIntvlSep 8]$ in 
acht gleich große Teile eingeteilt. Dabei sind $x_0 = 0$, $x_1 = 1$, $x_2 = 2$, 
$x_3 = 3$, $x_4 = 4$, $x_5 = 5$, $x_6 = 6$, $x_7 = 7$ und $x_8 = 8$. Wendet 
man darauf diese Summenformel an, so erhält man
%
\begin{eqnarray*}
...
des Intervalls nennt man
%
\begin{equation}
\int_a^b f(x) \MDwSp x = \lim_{n \rightarrow \infty} S_n %
 = \lim_{n \rightarrow \infty} \sum_{k=0}^{n-1} f(z_k) \cdot (x_{k} - x_{k-1}) %
\quad \text{mit } x_{k-1} \leq z_k \leq x_k %%
\end{equation}
%
