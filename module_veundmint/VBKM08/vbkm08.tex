...
Die Überprüfung, ob man die Stammfunktion einer vorgegebenen Funktion $f$ richtig
gebildet hat, ist sehr einfach. Man bestimmt die Ableitung der gefundenen 
...
\item{\MEquationItem{$\displaystyle\int \left(12 x^2 - 4 x^7\right) \MDwSp x $}
{\MLSimplifyQuestion{30}{4*x^3 - 1/2*x^8}{10}{x}{4}{32}{LSTFX1}}.
\begin{MHint}{Lösung}{%
Es ist
\[
\int \left(12 x^2 - 4 x^7\right) \MDwSp x
...
\end{MHint}}
%
\item{\MEquationItem{$\displaystyle\int \left(\sin(x) + \cos(x)\right) \MDwSp x$}
{\MLSimplifyQuestion{30}{sin(x)-cos(x)}{10}{x}{4}{32}{LSTFX2}}.
\begin{MHint}{Lösung}{%
Es ist
\[
\int \left(\sin x + \cos x\right) \MDwSp x %
= -\cos(x) + \sin(x) + C %
...
%}\end{MHint}}
\item{\MEquationItem{$\displaystyle\int \MEU^{6+x} \MDwSp x$}
{\MLSimplifyQuestion{30}{exp(6+x)}{10}{x}{4}{32}{LSTFX3}}. 
\begin{MHint}{Lösung}{%
Es ist $\MEU^{6+x} = \MEU^6 \cdot \MEU^x$. Somit ist
\[
\int \MEU^{6+x} \MDwSp x = \int \MEU^6 \cdot \MEU^x \MDwSp x %
= \MEU^6 \cdot \int \MEU^x \MDwSp x = \MEU^6 \cdot \MEU^x + C %
= \MEU^{6+x} + C\MDFPeriod
\]
}
\end{MHint}}
...
Fallunterscheidung und bestimmen Sie damit 
\MEquationItem{$\displaystyle\int |x| \MDwSp x$}.

Antwort: \MEquationItem{$\displaystyle\int |x| \MDwSp x$} %
...
\begin{MExerciseItems}
%Teilaufgabe ersetzt, da die Aufgabe als zu umfangreich bezeichnet wurde (jgl):
%\item $f(x) := \frac{1+x+x^2+\sqrt{x}}{x}$,
\item $f(x) := \frac{8 x - 6 x^2}{x}$,
%Teilaufgabe ersetzt, da die Aufgabe als zu umfangreich bezeichnet wurde (jgl):
%\item $g(x) := 4 + \left(\frac{4 \cos^2 x}{(2 \sin x)^2}\right)^{-1}$,
\item $g(x) := 4 + \frac{5 x}{x + 2 x^2}$,
%Teilaufgabe ersetzt, da arctan nicht (mehr) in der Liste der Stammfunktionen
%aufgefuehrt werden soll (jgl):
%\item $h(x) := \frac{2}{4 + (2x)^2}$,
\item $h(x) := \frac{x + 2 \sqrt{x}}{4x}$,
\end{MExerciseItems}
...
\end{eqnarray*}
%
Die Zuordnung der Funktionen $f'$ und $g$ muss zielführend erfolgen. Dies wird 
am obigen Beispiel deutlich, wenn die Rollen von $f'$ und $g$ vertauscht werden. 
Die Leserin bzw. der Leser mag probieren, dieses Beispiel zu lösen, indem $f'$ 
und $g$ anders herum gewählt werden!
\end{MExample}
\end{MCOSHZusatz}
...
symmetrisch bez"uglich der $y$-Achse. Die Fl"ache zwischen dem Graphen von $g$ 
und der $x$-Achse ist hier symmetrisch bez"uglich der $y$-Achse. Die Teilfl"ache 
links davon ist also das Spiegelbild der rechts liegenden Fl"ache. Beide zusammen 
...
In sehr vielen Situationen wird die Berechung eines Integrals einfacher, wenn man den Integranden vor der Integration in eine bekannte Form bringt. Entsprechende 
Umformungen sollen im Folgenden an einigen Beispielen betrachtet werden.
...
\]
indem die Definition der Tangensfunktion ausgenutzt wird. Damit erhält man die 
Umformung
\[
\frac{\sin(x)}{\tan(x)} %
...
weitere Umformungen (z.B. Partialbruchzerlegung) anbieten, die man in 
weiterführender Literatur und Formelsammlungen finden kann. Im folgenden Beispiel
...
Graph von $f$ punktsymmetrisch zum Ursprung ist.
\end{MHint}
\end{MExercise}
...
der Mathematik formuliert. Dies soll exemplarisch verdeutlichen, dass die 
...
\begin{MExercise}
Berechnen Sie die Arbeit $W$, die n"otig ist, um einen kugelf"ormigen homogenen
K"orper mit der Masse $m$ \emph{gegen} die Gravitationskraft
$F: [r_1\MIntvlSep  \infty\MoIr \rightarrow \R, r \mapsto F(r) := -\gamma \cdot \frac{m \cdot M}{r^2}$ 
von der Oberfl"ache eines kugelf"ormigen homogenen K"orpers mit Radius $r_1 = 1$ 
und Masse $M = 2$ bis zu einer Entfernung $r_2 = 4$ zu bewegen (alle L"angen 
...
\MLParsedQuestion{10}{1/2}{4}{IG38}
und
$\displaystyle\int_{5}^{8} \frac{6}{x-4} \MDwSp x = $%
\MLParsedQuestion{10}{12*ln(2)}{4}{IG37}
\end{center}

\else
\[
\int_{1}^{\MEU} \frac{1}{2 x} \MDwSp x = %
\MLParsedQuestion{10}{1/2}{4}{IG40}
\qquad \text{und} \qquad
\int_{5}^{8} \frac{6}{x-4} \MDwSp x = %
\MLParsedQuestion{10}{12*ln(2)}{4}{IG39}
\]
\fi
\end{MExercise}
...
\begin{MExercise} %Hauptsatz: Gemeinsame Aufgabe mit OMB+
