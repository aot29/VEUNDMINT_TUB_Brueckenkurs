...
Anwendungen der Differentialrechnung. Untersucht man z.B. in der Physik die 
Kraft $F$, die auf einen Körper wirkt, dann kann man unter Verwendung des 
bekannten Zusammenhangs $F = m a$ ($m$: Masse des Körpers, $a$: Beschleunigung 
des Körpers) zunächst aus der Kraft die Beschleunigung $a = F/m$ berechnen. 
...
\int f(x) \MDwSp x = F(x) %%
$$
notiert. Die Sprechweise ist hier: "`$F$ ist das Integral über $f$."' 
Der Zusammenhang zum Integral wird in der Infobox \MRef{HSDDIR} 
...
f(x) = \sin(x)                             & F(x) = -\cos(x) \\
f(x) = \sin(k x)                           & F(x) = -\frac{1}{k}\,\cos(k x) \quad\mbox{\ f"ur eine Zahl \ } k \in \R, k \neq 0 \\
f(x) = \cos(x)                             & F(x) = \sin(x) \\
f(x) = \cos(k x)                           & F(x) = \frac{1}{k}\,\sin(k x) \quad\mbox{\ f"ur eine Zahl \ } k \in \R, k \neq 0 \\
%f(x) = 1 + \tan^2(x) = \frac{1}{\cos^2(x)} & F(x) = \tan(x) \\
f(x) = \MEU^x                              & F(x) = \MEU^x \\
f(x) = \MEU^{k x}                          & F(x) = \frac{1}{k}\,\MEU^{k x} \quad\mbox{\ f"ur eine Zahl \ } k \in \R, k \neq 0 \\
f(x) = \frac{1}{x}                         & F(x) = \ln|x|
%f(x) = \frac{1}{1 + x^2} & F(x) = \arctan(x) %
\end{array}
\]
%Hier bezeichnet $k$ eine beliebige reelle Zahlen mit $k \neq 0$.
\end{MXInfo}
\fi

In der ersten Zeile in obiger Tabelle steht, dass $F(x) = C$ eine Stammfunktion 
zu $f(x) = 0$ ist. Klar, denn die Ableitung einer konstanten Funktion ist die 
Nullfunktion.
Aber woher kennt man den Wert dieser Konstanten $C$? Schließlich ist die 
Ableitung jeder beliebigen konstanten Funktion die Null. So gilt z.B. für 
$F(x) = 3$ und für $G(x) = 5$, dass $F' = G' = 0$ ist.
Ist nur nach einer Stammfunktion von $f(x) = 0$ gefragt, ohne dass weitere 
Forderungen getroffen werden, ist die Stammfunktion eine ganz beliebige 
Konstante $C$. Andere M"oglichkeiten, als dass es sich um
irgendeine \emph{konstante} Funktion handelt, gibt es nicht.

Haben die Funktionen $F$ und $G$ dieselbe Ableitung $f = F' = G'$, dann ist 
$G'(x) - F'(x) = 0$. Bildet man nun auf beiden Seiten der Gleichung die 
...
$D$ sind, dann gibt es eine Zahl $C$, sodass
\[
F(x) = G(x) + C %
...
\begin{MExample}
Die Funktion $F: \R \to \R$ mit $F(x) = 5 x^2 - 6 x$ hat die 
Ableitung $F'(x) = 10 x - 6$. Somit wird durch 
\[
...
Beispielsweise ist auch $G: \R \to \R$ mit $G(x) := 5 x^2 - 6 x - 7$ eine 
Stammfunktion von $f$, denn es ist $G'(x) = 5 \cdot 2 x - 6 = f(x)$ f"ur alle 
$x \in \R$.
\end{MExample}
...
Stammfunktionen dann jeweils durch die Addition einer Konstanten:
...
$F_2'(x) = \frac{x + 7} \cdot 1 = \frac{1}{x + 7} \neq \frac{1}{x}$. 
Deshalb ist $F_2$ keine Stammfunktion von $f$.
%
...
Anhand der folgenden beiden "Ubungsausfgaben kann man die Regel zur 
...
\begin{MExercise}
Es soll das Integral $I = \displaystyle \int_{4}^{8} x \cdot \ln(x) \MDwSp x$
berechnet werden: $I = $\MLParsedQuestion{10}{24*ln(2)-16+1/4}{4}{M08ZEx02}.
\begin{MHint}{L"osung}
Der Integrand $f$ mit $f(x) = x \cdot \ln(x) = \ln(x) \cdot x$ f"ur 
$1 \leq x \leq 8$ ist ein Produkt einer Polynomfunktion und einer 
Logarithmusfunktion. Die Ableitung der Logarithmumsfunktion $u$ mit 
...
n"amlich $v$ mit $v'(x) = \frac{1}{2} \cdot x^2$. Das Produkt
$u' \cdot v$ mit 
$u'(x) \cdot v(x) = \frac{1}{x} \cdot \frac{1}{2} x^2 = \frac{1}{2} x$ 
f"ur $1 \leq x \leq 8$ eine stetige Funktion, zu der eine Stammfunktion 
bekannt ist. Damit kann das gesuchte Integral mittels partieller Integration
gem"a"s
\begin{eqnarray*}
\int_{1}^{8} x \cdot \ln(x) \MDwSp x %
 = \int_{1}^{8} \ln(x) \cdot x \MDwSp x %
& = & \left[ \ln(x) \cdot x \right]_{1}^{8} - \int_{4}^{8} \frac{1}{x} \cdot \frac{1}{2} x^2 \MDwSp x \\
& = & \left[ \ln(x) \cdot x \right]_{1}^{8} - \int_{4}^{8} \frac{1}{2} x \MDwSp x \\
%& = & \left[ \ln(x) \cdot x \right]_{1}^{8} - \left[ \frac{1}{2} \cdot \frac{1}{2} x^2 \right]_{1}^{8} \\
& = & \left[ x \cdot \ln(x) - \frac{1}{4} x^2 \right]_{1}^{8} %
%& = & \left(8 \ln(8) - 16\right) - \left(1 \cdot \ln(1) - \frac{1}{4}\right) \\
 =  24 \ln(2) - 16 + \frac{1}{4} %%
\end{eqnarray*}
berechnet werden, wobei $\ln(8) = \ln(2^3) = 3 \cdot \ln(2)$ verwendet wurde.
\end{MHint}
\end{MExercise}
\end{MCOSHZusatz}
...
Die Werte von $f$ sind kleiner als die von $g$ auf diesem Intervall. Also
...
homogenen K"orper, der die Masse $m$ hat, \emph{gegen} die Gravitationskraft
...
als gegebene Werte anzunehmen, und der kleine K"orper wird im Vergleich zum 
...
\ \\
\begin{MHint}{Lösung}
Die Kraft $F_s$ l"angs des Weges, durch die der kleine K"orper mit der 
