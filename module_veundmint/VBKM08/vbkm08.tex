...
\begin{MExercise}
Gegeben ist $f$ mit $f(x) = \frac{1}{x}$ f"ur $x > 0$.
...
Das Integral einer Funktion kann als "`Fläche unter der Kurve"' interpretiert 
werden. In dem nach Riemann benannten Integral wird der Funktionsverlauf durch 
eine Treppenfunktion angen"ahert, und die Funktionswerte werden, gewichtet 
mit der jeweiligen Intervalll"ange bzw. "`Breite einer Treppenstufe"', 
aufsummiert. Dies ist in der unten gezeigten Abbildung dargestellt.
...
%\node at (-3,-2) {unterteilt in acht Teilintervalle.};
\end{tikzpicture}
}

Zur Definition des Integrals: Funktion angenähert durch %
eine Treppenfunktion,\\
unterteilt in acht Teilintervalle.
...
gehen, um einen möglichst genauen Wert der Fläche zu berechnen. Details zu den
Voraussetzungen der nachfolgenden Definiton findet man in der Literatur.
...
$$
%
wobei $f$ die Stammfuktion von $f'$ ist und $g'$ die Ableitung der Funktion $g$.
Diese Rechenregel ist als \MEntry{partielle Integration}{Partielle Integration} 
bekannt.
\end{MXInfo}
...
\[
\int_{-2}^{3} \frac{8 \MEU^{3 + x} - 12 \MEU^{2 x}}{2 \MEU^{x}} \MDwSp x \MDFPSpace, %%
\]
...
punktsymmetrisch zum Nullpunkt ist, ergibt sich der Fl"acheninhalt mit der
...
Funktionen
$f: [1\MIntvlSep  3] \rightarrow \R, x \mapsto 3 - (x - 2)^2$ und 
$g: [1\MIntvlSep  3] \rightarrow \R, x \mapsto 2 \cdot (x - 2)^4$ 
eingeschlossen wird.
Zeichnen Sie dazu zun"achst die Graphen der Funktionen, bevor Sie den 
Fl"acheninhalt berechnen.

Antwort: \MEquationItem{$I_A$}{\MLParsedQuestion{30}{30}{22+8/15}{IG36}}.

\begin{MHint}{Lösung}
Zur Berechnung der Fl"ache zwischen den Funktionsgraphen wird $f - g$ mit 
...
%$g: [0\MIntvlSep  1] \rightarrow \R, x \mapsto 12 x^2 - 12 x$ 
%eingeschlossen wird.
