...
Spezialfälle von Geraden, die als unendliche Punktmengen im $\R^2$ gegeben sind, sind bereits aus \MSRef{VBKM06}{Kapitel 6} bekannt. Dies sind nämlich die Graphen linear-affiner Funktionen aus dem Abschnitt \MSRef{VBKM06_linear}{Lineare Funktionen und Polynome} in Kapitel 6. Anhand des folgenden Beispiels können nochmals die wichtigsten Begriffe aus diesem Abschnitt rekapituliert werden, die im Folgenden wieder benutzt werden:
...
\]
In dieser Form beschreibt die Gerade den Graphen einer \MSRef{VBKM06_sec:linear-affin}{linear-affinen Funktion} $f$ mit Steigung $-\frac{p}{q}$ und Achsenabschnitt $\frac{c}{q}$:
\[
 \function{f}{\R}{\R}{x}{y=f(x)=-\frac{p}{q}x + \frac{c}{q}\MDFPeriod}
\]
\end{MInfo}

Da man in der Normalform einer Geraden ihre Steigung und ihren Achsenabschnitt ablesen kann, kann man diese so wie die Graphen \MSRef{VBKM06_sec:linear-affin}{linear-affiner Funktionen} zeichnen.
...
Dieser trägt die besondere Bezeichnung \MEntry{Einheitskreis}{Einheitskreis} und spielt eine besondere Rolle in der Trigonometrie (vgl.: \MNRef{Mathematik_ElementareGeometrie_Sec:Trigonometrie} und \MNRef{VBKM06_triogonometrisch}).
