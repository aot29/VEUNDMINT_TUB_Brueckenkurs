...
\begin{MInfo}
Sind ein Kreis $K$ und eine Gerade $g$ in der Ebene (mit Hilfe von Koordinatengleichungen bezüglich eines festen Koordinatensystems) gegeben, so gibt es für ihre relative Lage genau drei Fälle:
\begin{enumerate}
 ...
 \item Der Kreis $K$ und die Gerade $g$ haben genau einen Punkt gemeinsam. Dies ist für die blaue Gerade in der Abbildung unten der Fall. Eine solche Gerade heißt \MEntry{Tangente}{Tangente (eines Kreises)} des Kreises.
 \item Der Kreis $K$ und die Gerade $g$ haben zwei gemeinsame Punkte. Dies ist für die grüne Gerade in der Abbildung unten der Fall. Eine solche Gerade heißt \MEntry{Sekante}{Sekante (eines Kreises)} des Kreises.
\end{enumerate}
\begin{center}
 \MTikzAuto{
\begin{tikzpicture}
...
Im Fall einer Sekante oder Tangente haben Kreis oder Gerade also zwei Punkte oder einen Punkt gemeinsam. Wie kann man diesen Punkt oder diese Punkte nun ausrechnen? Da die Punkte jeweils auf Kreis und Gerade liegen müssen, müssen sie sowohl die Koordinatengleichung des Kreises als auch die Koordinatengleichung der Geraden erfüllen. Man hat dann also zwei Gleichungen für die beiden unbekannten Koordinaten der Schnittpunkte, die man aus diesen beiden Gleichungen ausrechnen kann. Dabei ist allerdings folgendes zu beachten: Da in der Koordinatengleichung für Kreise die Unbekannten quadriert vorkommen handelt es sich \textit{nicht} um zwei lineare Gleichungen. Es sind also leider \textit{nicht} die Methoden zum Lösen linearer Gleichungssysteme aus Kapitel \MNRef{VBKM05} anwendbar. Die folgende Infobox fasst die Methode zur Berechnung der Schnittunkte zusammen.
...
\[
 (x-3)^2+(2x+b)^2=2\MDFPaSpace\Leftrightarrow\MDFPaSpace x^2-6x+9+4x^2+4bx+b^2=2\MDFPaSpace\Leftrightarrow\MDFPaSpace 5x^2+(4b-6)x+b^2+7=0.
\]
...
\begin{MInfo}
Sind zwei (verschiedene) Kreise $K_1$ mit dem Mittelpunkt $M_1$ und dem Radius $r_1$ sowie $K_2$ mit dem Mittelpunkt $M_2$ und dem Radius $r_2$ in der Ebene (mit Hilfe von Koordinatengleichungen bezüglich eines festen Koordinatensystems) gegeben, so gibt für ihre relative Lage genau drei Fälle:
\begin{enumerate}
...
\draw[color=blue] (3,1.8) node[anchor=south] {\scriptsize $K_2$};
...
\]
und eine Kreis $L$ mit noch unbekanntem Radius $r>0$ durch die Kreisgleichung
\[
...
\[
 r+r_K=r+\sqrt{2}.
\]
Somit berühren sich die Kreise, falls 
\end{MExample}
