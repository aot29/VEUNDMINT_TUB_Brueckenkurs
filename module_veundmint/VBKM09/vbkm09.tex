...
\begin{MExample}
Gegeben ist der Kreis $K$ mit Mittelpunkt $\MPointTwo{2}{2}$ und Radius $1$ durch
\[
 K\colon (x-2)^2 + (y-2)^2=1
\]
sowie die Geraden $g_1$, $g_2$ und $g_3$ durch
\[
 g_1\colon y=x-\sqrt{2},
\]
\[
 g_2\colon x+1
\]
...
\begin{itemize}
 ...
 \[
  ...
  \MDFPaSpace\Leftrightarrow\MDFPaSpace x = 2+\frac{1}{\sqrt{2}}.
 \]
 ...
 \]
 \[
  \Leftrightarrow\MDFPaSpace x_{1,2}=\frac{3\pm\sqrt{9-8}}{2}=\frac{3\pm 1}{2}\MDFPaSpace\Leftrightarrow\MDFPaSpace x_1=2\ \wedge\ x_2=1.
 \]
 ...
 \]
 \[
  \Leftrightarrow\MDFPaSpace 5x^2-4x+3=0.
 \]
...
\begin{MExercise}
Gegeben ist der Kreis $K$ durch die Gleichung
\[
 K\colon x^2+(y-1)^2=2
\]
...
$x_1=$\MLFunctionQuestion{10}{-1/10}{5}{x}{5}{SEK01}\\
$y_1=$\MLFunctionQuestion{10}{9/5}{5}{x}{5}{SEK02}\\
$x_2=$\MLFunctionQuestion{10}{-7/10}{5}{x}{5}{SEK03}\\
$y_2=$\MLFunctionQuestion{10}{3/5}{5}{x}{5}{SEK04}
} 
\end{MExerciseItems}
...
\]
\[
 \Leftrightarrow\MDFPaSpace 8x^2+8\sqrt{7}x+14=0\MDFPaSpace\Leftrightarrow\MDFPaSpace 4x^2+4\sqrt{7}x+7=0\MDFPaSpace\Leftrightarrow\MDFPaSpace (2x+\sqrt{7})^2=0.
\]
...
\]
\[
 \Leftrightarrow\MDFPaSpace 5x^2+4x-1=0\MDFPaSpace\Leftrightarrow\MDFPaSpace x_{1,2}=\frac{-4\pm\sqrt{25-16}}{10}=\frac{-4\pm 3}{10}\MDFPaSpace\Leftrightarrow\MDFPaSpace
 x_1=-\frac{1}{10}\ \wedge\ x_2=-\frac{7}{10}.
\]
...
\[
 y_1=2\left(-\frac{1}{10}\right)+2=-\frac{2}{10}+2=\frac{9}{5}
\]
und 
\[
 y_2=2\left(-\frac{7}{10}\right)+2=-\frac{14}{10}+2=\frac{3}{5}\MDFPeriod
\]
...
\begin{MExercise}
Gegeben ist der Kreis $K$ durch die Kreisgleichung
\[
 K\colon (x-3)^2+y^2=2
\]
...
$b\in]$\MLFunctionQuestion{15}{-6-4*sqrt(5)}{5}{x}{5}{SEK1}$;$\MLFunctionQuestion{15}{-6+4*sqrt(5)}{5}{x}{5}{SEK2}$[$.
...
\[
 (x-3)^2+(2x+b)^2=2\MDFPaSpace\Leftrightarrow\MDFPaSpace x^2-6x+9+4x^2+4b x+b^2=2\MDFPaSpace\Leftrightarrow\MDFPaSpace 5x^2+(4b-6)x+b^2+7=0.
\]
...
\[
 (4b-6)^2-4\cdot 5(b^2+7)>0\MDFPaSpace\Leftrightarrow\MDFPaSpace 16b^2-48b+36-20b^2+140>0\MDFPaSpace\Leftrightarrow\MDFPaSpace -4b^2-48b+176>0
\]
\[
 \Leftrightarrow\MDFPaSpace b^2+12b-44<0\MDFPeriod
\]
Die Lösungen der quadratischen Gleichung $b^2+12b-44=0$ (in der Variablen $b$) ergeben sich zu 
\[
 b_{1,2}=\frac{-12\pm\sqrt{144+176}}{2}=\frac{-12\pm\sqrt{320}}{2}=\frac{-12\pm8\sqrt{5}}{5}=-6\pm4\sqrt{5}.
\]
Damit ist das gesuchte Intervall $]-6-4\sqrt{5};-6+4\sqrt{5}[$.
\end{MHint}
\end{MExercise}
...
\begin{enumerate}
 ...
 \[
  [\overline{M_1 M_2}]=r_1+r_2\MDFPaSpace\textrm{oder}\MDFPaSpace[\overline{M_1 M_2}]=|r_1-r_2|\MDFPeriod
 \]
 \item Die Kreise $K_1$ und $K_2$ haben zwei gemeinsamen Punkte, das heißt sie schneiden sich. Dies ist genau dann der Fall wenn für die Radien $r_1$ und $r_2$ sowie für den Abstand $[\overline{M_1 M_2}]$ der beiden Mittelpunkte gilt:
 \[
...
\begin{MExample}
Gegeben ist ein Kreis $K$ mit Mittelpunkt $M_K$ und Radius $r_K$ durch die Gleichung
\[
...
\]
Es sind diejenigen Werte für $r$ zu bestimmen, für die die Kreise $K$ und $L$ schneiden, berühren bzw. nicht schneiden.
...
\]
Somit ist $M_K=\MPointTwo{0}{1}$ und $r_K=\sqrt{2}$. Somit beträgt der Abstand der beiden Mittelpunkte
\[
 [\overline{M_K M_L}]=\sqrt{(0+1)^2+(1-1)^2}=1
\]
...
Da $r>0$ gilt folgt
\[
 r+\sqrt{2}>\sqrt{2}>1=[\overline{M_K M_L}].
\]
...
\]
gilt. Also sind die beiden Radien $r$, für die der Kreis $L$ den Kreis $K$ berührt:
\[
...
\[
 |r-\sqrt{2}|=-(r-\sqrt{2})=\sqrt{2}-r>1=[\overline{M_K M_L}],
\]
...
\[
 |r-\sqrt{2}|=r-\sqrt{2}<1=[\overline{M_K M_L}].
\]
Somit haben $K$ und $L$ für $\sqrt{2}-1<r<\sqrt{2}+1$ zwei gemeinsame Punkte, die Kreise schneiden sich also. Zuletzt gilt im Fall $r>\sqrt{2}+1$:
\[
...
\begin{MHint}{Lösung}
Der Mittelpunkt $M_1$ und der Radius $r_1$ des Kreises $K_1$ ist direkt abzulesen. Es gilt $M_1=\MPointTwo{-6}{-4}$ und $r_1=8$. Für $K_2$ ist eine quadratische Ergänzung durchzuführen:
\[
...
\]
Nun kann der Mittelpunkt $M_2$ und der Radius $r_2$ von $K_2$ abgelesen werden. Es gilt $M_2=\MPointTwo{-1}{8}$ und $r_2=5$. Damit berechnet man
\[
...
\end{center}
 \item Bereich oberhalb der Gerade $y=x$ und innerhalb des Einheitskreises $y^2+x^2=1$, inklusive der Punkte auf dem Kreis, aber ohne die Punkte auf der Geraden:
\begin{center}
\MTikzAuto{
\begin{tikzpicture}
%Koordinatensystem
...
\end{center}
\end{itemize}
Hier uns im Folgenden wird die übliche Konvention genutzt, dass Abbildungen diejenigen Kurven, welche zur Fläche gehören durchgezogen und diejenigen, welche \textit{nicht} zur Fläche gehören, gestrichelt dargestellt werden.
...
Die folgenden Infobox listet die möglichen entstehenden Bereiche auf, wenn man in einer Geradengleichung das Gleichheitszeichen durch ein Ungleichheitszeichen ersetzt.
...
\[
 B=\{\MPointTwo{x}{y}\in\R^2\MCondSetSep x\geq-1\},
\]
...
\end{center}
Der Schnittpunkt $\MPointTwo{-1}{2}$ der beiden Geraden gehört dabei nicht zum Bereich $A\cap B$ (im Bild angedeutet durch einen leeren Kringel), da Punkte auf $g$ nicht zum Bereich $A\cap B$ gehören.
\end{MExample}

Dieses Beispiel zeigt bereits Folgendes: Bereiche, welche durch nur eine Koordinatenungleichung, die aus einer Geradengleichung stammt, gegeben sind, sind relativ einfach anzugeben. Es wird schwieriger, sobald Schnittmengen solcher Bereiche gesucht sind. Das folgende, etwas schwierigere, Beispiel zeigt, dass man nun auch noch Beträge mit ins Spiel bringen kann. 
...
\begin{enumerate}
 ...
 \[
  |x-y|<1\MDFPSpace\Leftrightarrow\MDFPSpace x-y<1\MDFPSpace\Leftrightarrow\MDFPSpace y>x-1
 \]
...
\end{enumerate}
Aus allen zwei Fällen ergibt sich also die folgende Beschreibung für die Menge $M$:
\[
...
\item Die Menge $A_2:=\{\MPointTwo{x}{y}\in\R^2\MCondSetSep (x-2)^2+y^2<1\}\cap\{\MPointTwo{x}{y}\in\R^2\MCondSetSep x^2+y^2<4\}$ besteht aus allen Punkten, die sowohl innerhalb des Kreises $K_1$, als auch innerhalb Kreises $K_2$ liegen, also im Schnitt der beiden Kreisscheiben, aber nicht auf den Kreisen selbst:
...
\begin{MHint}{Lösung}
\begin{MExerciseItems}
\item{Die Menge $A$ wird gebildet von allen Punkten, die sowohl außerhalb des Kreises um $\MPointTwo{1}{1}$ mit Radius $1$ oder auf diesem, als auch oberhalb der Gerade $y=-\frac{1}{2}x+1$ liegen:
\begin{center}
\MTikzAuto{
\begin{tikzpicture}
%Koordinatensystem
...
\item{Die Punkte in der Ebene, welche die Ungleichung $|x|<1$ erfüllen, sind diejenigen für die $-1<x<1$ gilt. Dazu (Vereinigungmenge!) kommen noch die 
die Punkte auf oder innerhalb des Kreises $(x+3)^2+(y-1)^2=4$:
\begin{center}
\MTikzAuto{
\begin{tikzpicture}
%Koordinatensystem
