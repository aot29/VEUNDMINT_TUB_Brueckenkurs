...
If two numbers are related by one of the \MEntry{comparators}{comparators} 
...
equations described in Module~\MNRef{VBKM02}, now also the comparator can be inverted when
...
As in Module~\MNRef{VBKM02} the following statement holds for the solution set.
...
\begin{MExerciseItems}
\item{On the interval \MLIntervalQuestion{15}{(-infty,2)}{4}{GOM1} the term is positive, the comparator 
...
As in the approach in Module~\MNRef{VBKM02} and in the previous section, 
...
\MSRef{VBKM02_FallBetrag}{previous module} and merged to form the solution set of the initial inequality. 
\end{MInfo}
...
Always note the calculation rule $\sqrt{x^2}=|x|$ described in Module~\MNRef{VBKM01}.
...
\begin{MTest}{Final Test Module 3}
