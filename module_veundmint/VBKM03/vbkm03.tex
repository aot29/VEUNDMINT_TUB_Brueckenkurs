...
\begin{itemize}
\item{$x< a$ besitzt die Lösungsmenge $\MoIl -\infty\MIntvlSep a\MoIr$, alle $x$, die kleiner als $a$ sind.}
\item{$x\leq a$ besitzt die Lösungsmenge $\MoIl -\infty\MIntvlSep a]$, alle $x$, die kleiner oder gleich $a$ sind.}
\item{$x> a$ besitzt die Lösungsmenge $\MoIl a\MIntvlSep \infty\MoIr$, alle $x$, die größer als $a$ sind.}
\item{$x\geq a$ besitzt die Lösungsmenge $[a\MIntvlSep \infty\MoIr$, alle $x$, die größer oder gleich $a$ sind.}
\end{itemize}
...
entweder $\R=\MoIl -\infty\MIntvlSep \infty\MoIr$, falls die Ungleichung erfüllt ist, oder die leere Menge $\lbrace \rbrace$, falls die Ungleichung nicht erfüllt ist.
\end{MInfo}
...
&\Leftrightarrow&2 > x
\end{eqnarray*}
und damit das Lösungsintervall $\ML=\MoIl -\infty\MIntvlSep 2\MoIr$. Umformen der zweiten Ungleichung ergibt
\begin{eqnarray*}
...
\end{eqnarray*}
Diese Aussage ist unabhängig von $x\in\R$ immer erfüllt, also ist $\ML=\R=\MoIl -\infty\MIntvlSep \infty\MoIr$ die Lösungsmenge.
\end{MHint}
\end{MExercise}
...
\begin{MExample}
Beispielsweise ist die Ungleichung $\frac1{2x}\leq \frac1{3x}$ äquivalent zu $2x\geq 3x$ (Vergleichssymbol wurde gedreht) sofern $x\not=0$ ist. Die neue Ungleichung hat die Lösungsmenge $\MoIl -\infty\MIntvlSep 0]$, aber da
der Fall $x=0$ ausgeschlossen wurde (und er auch nicht zur Definitionsmenge der ursprünglichen Ungleichung gehört) ist $\ML=\MoIl -\infty\MIntvlSep 0\MoIr$ die Lösungsmenge von $\frac1{2x}\leq \frac1{3x}$.
\end{MExample}
...
\item{Unter der Bedingung $x<0$ erhält man die neue Ungleichung $1\geq 2x$. Sie hat die Lösungsmenge $\MoIl[\left] -\infty\MIntvlSep \frac12\right]$. Wegen der zusätzlichen Bedingung $x<0$ sind in diesem Fall aber nur die Elemente von $\ML_2=\MoIl -\infty\MIntvlSep 0\MoIr$ Lösungen.}
...
\ \\ \ \\
\begin{MHint}{Lösung}
Auf dem Intervall $\MoIl -\infty\MIntvlSep 2\MoIr$ ist der Term positiv mit Lösungsmenge $\MoIl[\left] -\infty\MIntvlSep \frac{11}{6}\MoIr[\right]$.
...
\item{Im Fall $x<2$ multipliziert man mit $x-2$ und erhält $x-1\geq x-2$, was äquivalent zur wahren Aussage $-1\geq -2$ ist. Wegen der Vorbedingung ist das Lösungsintervall für diesen Fall aber nur $\ML_2=\MoIl -\infty\MIntvlSep 2\MoIr$.}
\item{Der Einzelwert $x=2$ ist keine Lösung.}
\end{itemize}
Die Lösungsmenge ist also insgesamt $\ML=\MoIl -\infty\MIntvlSep 2\MoIr$ ohne die Randpunkte (auch wenn die Ursprungsungleichung mit $\leq$ aufgebaut war).
\end{MHint}
\end{MExercise}
...
\begin{itemize}
\item{Im Fall $0\leq x<1$ multipliziert man mit $1-\sqrt{x}$ und erhält $1<(1+\sqrt{x})(1-\sqrt{x})$, was äquivalent zur Aussage $1<1-x$ ist. Diese ist für $x<0$ efüllt, aber diese $x$ verletzen die Fallbedingung und kommen daher nicht in die Lösungsmenge.}
...
Für $x\in\MoIl -\infty\MIntvlSep 1\MoIr$ sind beide Terme in den Beträgen negativ und man erhält $-(x-1)<-2(x-1)+x$. Diese Ungleichung ist äquivalent zur immer richtigen Ungleichung $x-1<x$. Damit ist $\ML_2=\MoIl -\infty\MIntvlSep 1\MoIr$ die Lösungsmenge des zweiten Falls.
\ \\ \ \\
Wegen $\ML=\ML_1\cup \ML_2=\R=\MoIl -\infty\MIntvlSep \infty\MoIr$ ist die Ungleichung immer erfüllt.
\end{MHint}
\end{MExercise}
...
\item{zwei Äste $\MoIl -\infty\MIntvlSep x_1\MoIr\cup \MoIl x_2\MIntvlSep \infty\MoIr$ (ggf. mit beiden Randpunkten enthalten bei $\leq$ und $\geq$),}
...
\ \\ \ \\
\begin{MHint}{Lösung}
Umformen ergibt $x^2+6x+5<0$. Mit der $p q$-Formel findet man die Nullstelle $x_{1}=-3\pm\sqrt{9-5}$, also $x_1=-1$ und $x_2=-5$.
Die linke Seite beschreibt eine nach oben geöffnete Parabel,
sie erfüllt die Ungleichung mit $<$, also nur auf dem Intervall $\MoIl -5\MIntvlSep -1\MoIr$ ohne die Randpunkte.
\end{MHint}
\end{MExercise}
