...
\]
im dreidimensionalen Fall. Geometrisch kann die Vektoraddition als \glqq Aneinanderhängen\grqq\ von Pfeilen oder als Ergänzen eines Parallelogramms interpretiert werden, je nachdem welche Repräsentanten der Vektoren man verwendet:
\begin{center}
\begin{tabular}{lr}
\MTikzAuto{%
\begin{tikzpicture}[>=stealth] 
...
\]
Und der Nullvektor $\MDVec{O}=\MVector{0\\0}$ bzw. $\MDVec{O}=\MVector{0\\0\\0}$ erfüllt die gleiche Funktion für Vektoren, die die $0$ für reelle Zahlen erfüllt:
\[
...
\end{center}
Weiterhin ist $\MVec{v}$ unter anderem der Verbindungsvektor vom Punkt $P=\MPointTwo{2}{0}$ zum Punkt $Q=\MPointTwo{1}{2}$, also $\MVec{v}=\MDVec{P Q}$ und $\MVec{w}$ ist unter anderem der Verbindungsvektor vom Punkt $Q=\MPointTwo{1}{2}$ zum Punkt $R=\MPointTwo{3}{1}$, also $\MVec{w}=\MDVec{Q R}$. Dann stellt man fest, dass
\[
 \MVec{v}+\MVec{w} = \MDVec{P Q} + \MDVec{Q R} = \MDVec{P R}
\]
gilt:
\begin{center}
\MTikzAuto{
...
%Pfeile
\draw[->, color=red] (2,0) -- (1,2);
\draw[color=red] (1.5,1) node[anchor=east] {\footnotesize $\MVec{v}=\MDVec{P Q}$};
\draw[->, color=blue] (1,2) -- (3,1);
\draw[color=blue] (2,1.5) node[anchor=south west] {\footnotesize $\MVec{w}=\MDVec{Q R}$};
\draw[->, color=violet] (2,0) -- (3,1);
\draw[color=violet] (2.5,0.5) node[anchor=west] {\footnotesize $\MVec{v}+\MVec{w} = \MDVec{P R}$};
\end{tikzpicture}
} 
\end{center}
\end{MExample}
...
\begin{MExerciseItems}
\item{Gegeben sind die Vektoren $\MVec{u}=\MVector{1\\0\\-8}$ und $\MVec{v}=\MVector{-3\\-4\\3}$. Berechnen Sie $\MVec{v}+\MVec{u}=$\MLFunctionQuestion{15}{(-2,-4,-5)}{5}{x}{5}{VEC21}.} 
