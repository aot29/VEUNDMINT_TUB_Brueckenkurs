...
which are generally denoted by lowercase Latin letters (see Table~\MRef{VBKM01_Griechisch} 
in module~\MNRef{VBKM01}). Further angles can be found if angles formed by intersecting lines are 
taken into account. 
...
Technical structures such as trusses and some bridges use triangles as its 
...
\MEntry{perpendicular foot}{perpendicular foot}. The altitudes $h_a$ and 
...
\MEntry{Pythagoras' theorem}{Pythagoras (theorem)}. The theorem is given here 
...
For the right and the left triangle in Example~\MRef{VBKM05_BeispielAehnlichkeit}
there is a special fact. The left triangle is transformed into the other by 
...
\item \MEntry{square}{square} if it is a rectangle having four sides  of equal length;
...
a vertex isn't such specifying anymore. Instead, there is a greater variety of shapes.
...
\MEntry{regelar $n$-gon}{regelar $n$-gon}.
\end{MXInfo}
...
are introduced in Chapter~\MRef{VBKM07}. The approach described here is
...
(see Chapter~\MRef{VBKM08}). For this purpose, it will be illustrated in the 
...
A tunnel drilling machine creates -- considered in a simplified manner -- 
a cylindrical tube.
...
have a distance from $M$ less or equal to $r$ (see also Chapter~\MRef{VBKM10}).
...
base face now only a point is given.
...
(as for the case of a pyramid) now a disk is used as base face. 
...
\MSubsection{Trigonometric Functions: Sine and et cetera}
...
The \MEntry{opposite (side)}{opposite (side)} is the side opposite to the angle $x$, 
and the other leg is called \MEntry{adjacent (side)}{adjacent (side)}. 
...
we can find different ways how to interpret them, and then identify 
some relations.
...
a reflection across the $x$-axis results in a change of sign of the sinus 
...
\begin{MTest}{Final Test Modul \arabic{section}}
\MLabel{M05_Abschlusstest_Test}
\MDeclareSiteUXID{VBKM05_Abschlusstest}

\begin{MExercise} %Testaufgabe
Identify the figures below as precise as possible by specifying the name of the type 
(preceded by an adjective if necessary) describing as many properties 
