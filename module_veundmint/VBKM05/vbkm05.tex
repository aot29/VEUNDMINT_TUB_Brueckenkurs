...
Richtung (entgegen des Uhrzeigersinns) mit lateinischen Gro"sbuchstaben. 
...
\begin{itemize}
 ...
 \item Ein Dreieck, das einen Winkel mit einem Ma"s von "uber~$\frac{\pi}{2}$
  besitzt, hei"st \MEntry{stumpfwinklig}{Dreieck (stumpfwinklig)}.
\end{itemize}
\end{MXInfo}
...
In diesem Abschnitt werden einige Spezialf"alle von Vielecken betrachtet. 
...
Neben den oben betrachteten Dreiecken kann man nat\"urlich auch Figuren mit 
mehr Ecken betrachten. 

\begin{MXInfo}{Vielecke}\MLabel{VBKM05_Vielecke}%
Es sei $n$ eine nat\"urliche Zahl, die mindestens $3$ betrage.
\begin{itemize}
 \item 
Ein \MEntry{$n$-Eck}{$n$-Eck} (auch als \MEntry{Vieleck}{Vieleck} oder 
\MEntry{Polygon}{Polygon} bezeichnet) entsteht,
...
\begin{itemize}
 ...
 \item \MEntry{Raute}{Raute} oder \MEntry{gleichseitiges Viereck}{gleichseitiges Viereck} oder 
...
\draw[color=blue] (3.0,-0.5) node {\large $F_4$};
...
Fall dieser Situation eine Formel %f"ur Volumen und 
...
%\item \MEquationItem{$V$}{\MLSimplifyQuestion{30}{ ? }{10}{a,b,c}{4}{32}{ExM05TPyr1}}
%\item 
\MEquationItem{$O$}{\MLSimplifyQuestion{30}{sqrt{3}*a*a}{10}{a,b,c}{4}{32}{ExM05TPyr2}}
%\end{MExerciseItems}
\end{MExercise}
...
\par
\MEquationItem{$l$}{\MLSimplifyQuestion{30}{sqrt(r*r + h*h)}{10}{r,h}{4}{32}{ExM05Zyl1}}

\item
Berechnen Sie das Volumen $V$ und die Oberfl"ache $O$ des Kegels:
\begin{MExerciseItems}
\item %
 \MEquationItem{$V$}{\MLSimplifyQuestion{30}{pi*r*r*h}{10}{r,h}{4}{32}{ExM05Zyl2}}
\item %
 \MEquationItem{$O$}{\MLSimplifyQuestion{30}{pi*r*(r + sqrt(r*r + h*h))}{10}{r,h}{4}{32}{ExM05Zyl3}}
\end{MExerciseItems}
\end{MExerciseItems}
\end{MExercise}
...
so einfach.
Vor der Zeit von Taschenrechnern und Computern wurden die aufw"andig 
bestimmten Werte in gro"sen Tafelwerken aufgeschrieben, um dann einfach
nachgeschlagen werden zu k"onnen.
...
\begin{MExample}
Von einem Dreieck ist bekannt, dass es einen rechten Winkel $\gamma=\frac{\pi}{2}=90\MGrad$ hat. Die Seite $c$ ist $5\MEinheit{cm}$, die Seite $a$ ist $\MZahl{2}{5}\MEinheit{cm}$ lang. Es sollen jeweils der Sinus, Kosinus und Tangens des Winkels $\alpha$ bestimmt werden:
...
die rechtwinkligen Dreiecke f\"ur die Winkel $\alpha\in\left\{0\MGrad\MElSetSep 10\MGrad\MElSetSep 20\MGrad\MElSetSep 30\MGrad\MElSetSep 40\MGrad\MElSetSep 45\MGrad\MElSetSep
50\MGrad\MElSetSep 60\MGrad\MElSetSep 70\MGrad\MElSetSep 80\MGrad\MElSetSep 90\MGrad\right\}$.
...
\begin{tabular}{r|r|r|r|r|r}\hline
	...
	$90\MGrad$		& $\MZahl{5}{0}$		& $\MZahl{0}{0}$	& $\MZahl{1}{0}$	& $\MZahl{0}{0}$	& $\rightarrow\infty$\\ \hline
\end{tabular}
\end{center}
...
$\sin\left(\alpha\right)=\cos\left(90\MGrad-\alpha\right)=\cos\left(\pi/2-\alpha\right)$
bzw. 

$\cos\left(\alpha\right)=\sin\left(90\MGrad-\alpha\right)=\sin\left(\pi/2-\alpha\right)$.
...
Winkel $\alpha$ von Null ins Unendliche. 
\end{itemize}

Das Diagramm sieht folgenderma"sen aus:
\begin{center}
\MTikzAuto{%
...
Das Dreieck ist nach dem Kongruenzsatz "`sss"' mit der Angabe einer Seite $a$ 
...
Es gilt nun: \[\sin\left(\alpha\right)=\sin\left(60\MGrad\right)=\frac{h}{a} \MDFPeriod\]
...
\end{tabular}
 \[\Rightarrow\quad\sin\left(60\MGrad\right)=\sin\left(\frac{\pi}{3}\right)=\frac{1}{2}\cdot \sqrt{3} \MDFPeriod\]
 \ \\
Aus diesem Dreieck kann man noch den Sinus eines weiteren Winkels berechnen: 
Die H\"ohe $h$ teilt den oberen Winkel in zwei gleiche Teile, sodass man in den beiden kleinen kongruenten Dreiecken jeweils den Winkel $30\MGrad = \frac{\pi}{6}$ erh\"alt.
Es ist nun \[\sin\left(30\MGrad\right)=\sin\left(\frac{\pi}{6}\right)=\frac{a/2}{a}=\frac{1}{2} \MDFPeriod\]
\end{MExample}
...
dass $\cos\left(\alpha\right)= \sin\left(90\MGrad-\alpha\right)$.
Daraus folgt
\[
\cos\left(30\MGrad\right)=\sin\left(90\MGrad-30\MGrad\right)=\sin\left(60\MGrad\right)=\frac{1}{2}\cdot\sqrt{3} \MDFPSpace, \]
\[\cos\left(45\MGrad\right)=\sin\left(90\MGrad-45\MGrad\right)=\sin\left(45\MGrad\right)=\frac{1}{2}\cdot\sqrt{2} \MDFPSpace, \]
\[\cos\left(60\MGrad\right)=\sin\left(90\MGrad-60\MGrad\right)=\sin\left(30\MGrad\right)=\frac{1}{2} \MDFPeriod\]
\end{MHint}
\end{MExercise}
...
Im Beispiel 
\MRef{Mathematik_ElementareGeometrie:Trigonometrie_Beispiel:gleichseitigesDreieck}
...
Im letzten Abschnitt wurden die trigonometrischen Funktionen anhand eines 
rechtwinkligen Dreiecks betrachtet. Die gefundenen Erkenntnisse gelten also 
f\"ur einen Winkelbereich von $0$ bis $90\MGrad=\frac{\pi}{2}$.
...
\begin{MExample}
Gesucht sind jeweils die Werte des Sinus, Kosinus und Tangens des Winkels $\alpha=315\MGrad$.\\

F\"ur $\alpha=315\MGrad$ liegt der Punkt $P_\alpha$ im 4. Quadranten, der zugeh\"orige Vektor bildet mit den zugeh\"origen Achsenabschnitten ein gleichschenkliges Dreieck. Es gilt: \[\left|x_\alpha\right|=\left|y_\alpha\right|\quad\Rightarrow\quad \left|x_\alpha\right|^2+\left|y_\alpha\right|^2=2\cdot\left|x_\alpha\right|^2=1\]
...
invers(sin(-\MZahl{0}{766})) bzw. $\sin^{-1}$(-\MZahl{0}{766}), so erhalten Sie ungef\"ahr $-50\MGrad$, und \\
invers(cos(-\MZahl{0}{643})) bzw. $\sin^{-1}$(-\MZahl{0}{643}), so erhalten Sie ungef\"ahr $130\MGrad$.\\
Au\ss erdem wissen Sie, dass der Punkt im 3. Quadranten ist, also ein Winkel im Bereich zwischen $180\MGrad$ und $270\MGrad$ herauskommen muss.
...
\begin{minipage}[b]{10cm}
Anhand des Bildes kann man erkennen, dass der negative Sinuswert zwar zum Winkel $-50\MGrad$, aber auch zu $\alpha=\left(180\MGrad+50\MGrad\right)=230\MGrad$ geh\"ort.\\
\par
Ebenso kann der negative Kosinuswert zu $130\MGrad$, aber auch zu $\alpha=-130\MGrad=\left(360\MGrad-130\MGrad\right)=230\MGrad$ geh\"oren.\\
\par
Der richtige Winkel ist also $\alpha=230\MGrad$ (rosa).
\vspace*{2cm}
\end{minipage}
\end{MHint}
...
Geben Sie $\sin \left( \alpha \right)$, $\sin \left( \beta \right)$ und $a$ an!
	...
	= \frac{\MZahl{2}{53}\MEinheit{cm}}{\MZahl{3}{88}\MEinheit{cm}}
       ...
       \end{MHint}
\item Berechnen Sie den Fl\"acheninhalt eines Dreiecks mit den Seiten
$a = 4\MEinheit{m}$ und $c = 60\MEinheit{cm}$
und dem Winkel $\beta = \Mmeasuredangle(a,c) = \frac{11 \pi}{36}$!

Ergebnis: \MLParsedQuestion{25}{4*sin(11*pi/36)*6/20}{3}{GEO8} $\MEinheit{m}^2$
