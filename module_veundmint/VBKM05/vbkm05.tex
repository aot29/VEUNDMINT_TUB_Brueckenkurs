...
\begin{MExample}
Der Winkel zwischen zwei Halbgeraden, die einen Halbkreis ausschneiden, beträgt $2\pi/2=\pi$.\\
Der Winkel zwischen zwei Halbgeraden, die einen Viertelskreis ausschneiden, beträgt $2\pi/4=\pi/2$.\\
Der Winkel zwischen zwei Halbgeraden, die einen Achtelskreis ausschneiden, beträgt $2\pi/8=\pi/4$.
\end{MExample}
...
Ein anderes sehr gebräuchliches Winkelmaß erhält man, indem man den Kreis in 360~gleich
...
Die Strahlensätze haben etwas mit der zentrischen Streckung zu tun (siehe \MRef{Mathematik_ElementareGeometrie_zentrischeStreckung}).
...
\begin{itemize}
	...
	\item \textbf{Höhensatz}\\
		Das Quadrat über der Höhe ist flächeninhaltsgleich dem Rechteck aus den beiden Hypotenusenabschnitten: \[h^2=p\cdot q\]	
\end{itemize}
\end{MInfo}
...
\begin{scope}[color=black!50, every node/.style={color=black}]
 ...
 \draw ($ (KF)!0.5!(BF) $) -- node[right]{$h_K = \MZahl{1}{4}\,\text{m}$} ($ (KK)!0.5!(BM) $) ++ (-0.15,0) -- ++(0.3,0);
 \draw (BF) -- node[right]{$h_B$} (BK) ++ (-0.15,0) -- ++(0.3,0);
 \draw ($ (HF) + (0.75,0) $) -- node[below right]{$h_H = \MZahl{2}{4}\,\text{m}$} ($ (HK) + (0.75,0) $) ++ (-0.15,0) -- ++(0.3,0);
...
Nun gelten $d_{KH} = \MZahl{2}{5}\,\text{m} + \frac{1\,\text{m}}{2} = 3\,\text{m}$
und $d_{KB} = \MZahl{2}{5}\,\text{m} + 1\,\text{m} + \MZahl{14}{5}\,\text{m} = 18\,\text{m}$.
Damit folgt
\[
   h_B
   = \left( \MZahl{2}{4}\,\text{m} - \MZahl{1}{4}\,\text{m} \right) \cdot \frac{18\,\text{m}}{3\,\text{m}} + \MZahl{1}{4}\,\text{m}
   = 1\,\text{m} \cdot 6 + \MZahl{1}{4}\,\text{m}
   = \MZahl{7}{4}\,\text{m}.
\]
\end{MHint}
\end{MExercise}
...
$, ... als \textbf{Gegenkatheten} und die am Winkel anliegenden Seiten $b$, $b'$, ... als \textbf{Ankatheten}.\\
...
die rechtwinkligen Dreiecke für die Winkel $\alpha\in\left\{0^\circ\MIntvlSep 10^\circ\MIntvlSep 20^\circ\MIntvlSep 30^\circ\MIntvlSep 40^\circ\MIntvlSep 45^\circ\MIntvlSep
50^\circ\MIntvlSep 60^\circ\MIntvlSep 70^\circ\MIntvlSep 80^\circ\MIntvlSep 90^\circ\right\}$.
...
\begin{MExercise}
Berechnen Sie, auf die vorher gezeigten Beispielen Bezug nehmend, den exakten Wert des Kosinus für die Winkel $\alpha_1=30^\circ, \alpha_2=45^\circ$ und $\alpha_3=60^\circ$. Verwenden Sie die Erkenntnisse aus Aufgabe \MRef{Mathematik_ElementareGeometrie:Trigonometrie_Aufgabe:Tabelle}.
...
\[\cos\left(60^\circ\right)=\sin\left(90^\circ-60^\circ\right)=\sin\left(30^\circ\right)=\frac{1}{2}\]
\end{MHint}
\end{MExercise}
...
Diese Definitionen gelten auch für die Winkel $\alpha>\pi/2$. Dabei können die Werte für $x_\alpha$ und $y_\alpha$ auch negativ werden und damit auch der Kosinus bzw. Sinus. Trägt man die $y$-Werte in Abhängigkeit vom Winkel $\alpha$ in ein Diagramm, so erhält man die rote Kurve, für die $x$-Werte erhält man die blaue Kurve.
\ \\
...
\begin{MExample}
Wir suchen die Werte jeweils des Sinus, Kosinus und Tangens des Winkels $\alpha=315^\circ$.\\
...
\[\cos\left(\alpha\right)=x_\alpha=\frac{1}{2}\sqrt 2,	\quad	\sin\left(\alpha\right)=y_\alpha=-\frac{1}{2}\sqrt 2,	\quad	\tan\left(\alpha\right)=\frac{y_\alpha}{x_\alpha}=-1\]
\end{MExample}
...
invers(sin(-\MZahl{0}{766})) bzw. $\sin^{-1}$(-\MZahl{0}{766}), so erhalten Sie ungefähr $-50^\circ$, und \\
invers(cos(-\MZahl{0}{643})) bzw. $\sin^{-1}$(-\MZahl{0}{643}), so erhalten Sie ungefähr $130^\circ$.\\
...
\begin{MExercise}
\begin{enumerate}
\item F\"ur ein bei~$C$ rechtwinkliges Dreieck seien $b = \MZahl{2}{53}~\text{cm}$ und
	$c = \MZahl{3}{88}~\text{cm}$ gegeben. Geben Sie $\sin \left( \alpha \right)$,
       ...
       \[
          a
        = \sqrt{c^2 - b^2}
	= \sqrt{\left( \MZahl{3}{88}~\text{cm} \right)^2 - \left( \MZahl{2}{53}~\text{cm} \right)^2}
	= \sqrt{\MZahl{15}{0544}~\text{cm}^2 - \MZahl{6}{4009}~\text{cm}^2}
	= \sqrt{\MZahl{8}{6535}}~\text{cm}.
       \]
       \[
          \sin \left( \alpha \right)
        = \frac{a}{c}
	= \frac{\sqrt{\MZahl{8}{6535}}~\text{cm}}{\MZahl{3}{88}~\text{cm}}
        = \frac{\sqrt{86535}}{388}
        \qquad \text{und} \qquad
          \sin \left( \beta \right)
        = \frac{b}{c}
	= \frac{\MZahl{2}{53}~\text{cm}}{\MZahl{3}{88}~\text{cm}}
        = \frac{253}{388}
       \]
       Numerisch ergibt sich
       $a \approx \MZahl{2}{9417}~\text{cm}$, $\sin \left( \alpha \right) \approx \MZahl{0}{7587}$
 ...
 \item Berechnen Sie den Fl\"acheninhalt eines Dreiecks mit $\beta = \frac{11 \pi}{36}$,
	...
	=    \sin \left( \Mtfrac{11 \pi}{36} \right) \cdot \MZahl{1}{2}~\text{m}^2
	\approx \MZahl{0}{98298}~\text{m}^2
       \]
       \end{MHint}
\end{enumerate}
\end{MExercise}
