...
%content: Strahlensatz und Pythagoras.
...
(siehe \MRef{Mathematik_ElementareGeometrie_zentrischeStreckung}).
...
\begin{MXInfo}{Strahlens\"atze}\MLabel{Mathematik_ElementareGeometrie_Satz:ErsterStrahlensatz}%
...
$S$ ausgehen, schlie"sen einen \MEntry{Winkel}{Winkel} $\alpha$ ein.
...
Wenn man den Winkel $\Mmeasuredangle \left( g, h \right)$ kennt, so kann man nun auch leicht den Winkel $\Mmeasuredangle \left( h, g \right)$ bestimmen, der ja nach dem Bild \MRef{VBKM05_Bogenmass} durch den anderen Kreisbogen bestimmt ist:
\[
...
bietet der \textbf{Satz des Pythagoras}. Dieser wird hier in einer oft 
verwendeten Formulierung angegeben.
...
Es gibt noch einen weiteren wichtigen Satz, der f\"ur rechtwinklige Dreiecke 
g\"ultig ist:
...
sind diese Dreiecke \textbf{kongruent} oder deckungsgleich, dabei spielt es keine 
...
zu bestimmen.
Diese F\"alle werden mit den \MEntry{Kongruenzs\"atzen}{Kongruenzs\"atze} 
beschrieben:
...
Grundfl"ache $G$ und des Mantels $M$. Wenn $U$ der Umfang des gegebenen Vielecks 
ist, gilt $O = 2 \cdot G + M = 2 \cdot G + U \cdot h$.
\end{MXInfo}

Im einf"uhrenden Beispiel wurde ein Quader beschrieben. Mit obiger Definition
kann er als spezielles Prisma angesehen werden, n"amlich ein Prisma mit einem 
