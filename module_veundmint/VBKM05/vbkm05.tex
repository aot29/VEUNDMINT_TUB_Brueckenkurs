...
\path (A) -- node[below] {$a = 100\MEinheit{m}$} (B) %
 -- node[right] {$b = 5\MEinheit{m}$} (C) -- node[above] {Steigung $5\%$} (A);
\draw (A) -- (B) -- (C) -- cycle;
\end{tikzpicture}
}
\end{center}
...
als Zuordnungen zwischen Winkel und Seitenverh"altnissen im Dreieck beschrieben.
Die Winkelfunktionen hei"sen auch 
...
\path (A) -- node[below] {Ankathete $b$} (B) %
 -- node[right] {Gegenkathete $a$} (C) %
 -- node[above left] {Hyptenuse $c$} (A);
\draw (A) -- (B) -- (C) -- cycle;
...
Die Hypotenuse $c=5$ ist vorgegeben. Zeichnen Sie mithilfe des Thaleskreises 
die rechtwinkligen Dreiecke f\"ur die Winkel 
$\alpha\in\left\{0\MGrad\MElSetSep 10\MGrad\MElSetSep 20\MGrad\MElSetSep %
30\MGrad\MElSetSep 40\MGrad\MElSetSep 45\MGrad\MElSetSep 50\MGrad\MElSetSep %
60\MGrad\MElSetSep 70\MGrad\MElSetSep 80\MGrad\MElSetSep 90\MGrad\right\}$
(Ma"sstab $1\MEinheit{L"angeneinheit} \hat{=} 2\MEinheit{cm}$).

Messen Sie die Seiten $a$ und $b$ und schreiben Sie diese in eine Tabelle. 
Berechnen Sie zu jedem Dreieck den Sinus, Kosinus und Tangens.
Tragen Sie anschlie"send die Werte von Sinus, Kosinus und Tangens in 
...
\sin\left(\alpha\right)=\cos\left(90\MGrad-\alpha\right) %
 =\cos\left(\pi/2-\alpha\right) %%
\]
bzw. 
\[
\cos\left(\alpha\right)=\sin\left(90\MGrad-\alpha\right)=\sin\left(\pi/2-\alpha\right) \MDFPeriod %%
\]
%
...
also f\"ur einen Winkelbereich von $0$ bis $90\MGrad=\frac{\pi}{2}$.
...
Nullpunkt rotiert. 
Dabei \"uberstreicht ihre Spitze den Einheitskreis, und sie bildet mit der 
positiven $x$-Achse den Winkel $\alpha$, der bei der Rotation von $0$ bis 
$2\pi$ bzw. $360\MGrad$ w\"achst. Zu jedem Winkel $\alpha$ geh\"ort also ein 
Punkt $P_\alpha$ mit den Koordinaten $x_\alpha$ und $y_\alpha$ auf dem 
Einheitskreis.

F\"ur $\alpha\in\left[0\MIntvlSep \pi/2\right]$ kann man die Strecke, den 
zugeh\"origen $x$-Achsenabschnitt und den zugeh\"origen $y$-Achsenabschnitt 
als rechtwinkliges Dreieck ansehen, wie es vom letzten Kapitel her bekannt ist.
...
Diese Definitionen gelten auch f\"ur die Winkel $\alpha>\pi/2$. Dabei k\"onnen 
die Werte f\"ur $x_\alpha$ und $y_\alpha$ auch negativ werden und damit auch 
der Kosinus bzw. Sinus. Tr\"agt man die $y$-Werte in Abh\"angigkeit vom 
Winkel $\alpha$ in ein Diagramm, so erh\"alt man die rote Kurve, f\"ur die 
$x$-Werte erh\"alt man die blaue Kurve.
\par
Mit dem Satz von Pythagoras gilt au"serdem
\[
x_{\alpha}^{2}+y_{\alpha}^{2}=1 \MDFPeriod
\]
Setzt man hier die Beziehungen f\"ur $x_{\alpha}$ und $y_{\alpha}$ mit den 
Winkelfunktionen ein, so ergibt sich f\"ur beliebige Winkel $\alpha$ die 
wichtige Beziehung
\[
\sin^2\left(\alpha\right)+\cos^2\left(\alpha\right)=1 \MDFPeriod
\]
Aus obiger Zeichnung wird zudem ersichtlich, dass sich die Kosinuswerte bei 
Spiegelung an der $y$-Achse nicht "andern.
Eine Spiegelung an der $x$-Achse f"uhrt zur "Anderung des Vorzeichens des 
Sinuswertes.  
Formelm"a"sig ausgedr"uckt, gilt
\[
\cos(-x) = \cos(x)
\qquad \text{und} \qquad
\sin(-x) = -\sin(x)
...
Ein Beispiel ist die Berechnung des Winkels zwischen $x$-Achse und der 
Verbindungsstrecke vom Nullpunkt zu einem gegebenen Punkt im kartesischen 
Koordinatensystem (siehe 
auch \MRef{VBKM05_TrigonometrieAufgabeArcsinArccos}).
...
\sin\left(\alpha\right)=-\MZahl{0}{766}
 \quad \text{und} \quad
\cos\left(\alpha\right)=-\MZahl{0}{643} \MDFPeriod %%
\]
Tippen Sie in den Taschenrechner:\\
invers(sin(-\MZahl{0}{766})) bzw. $\sin^{-1}$(-\MZahl{0}{766}), so erhalten 
Sie ungef\"ahr $-50\MGrad$, und \\
invers(cos(-\MZahl{0}{643})) bzw. $\cos^{-1}$(-\MZahl{0}{643}), so erhalten 
Sie ungef\"ahr $130\MGrad$.\\
Au\ss erdem wissen Sie, dass der Punkt im 3. Quadranten ist, also ein Winkel 
im Bereich zwischen $180\MGrad$ und $270\MGrad$ herauskommen muss.
