...
\draw (-1,-0.5) circle(1pt);
\node[below] at (-1,-0.5) {$P$};
\draw (1,0.5) circle(1pt);
\node[below] at (1,0.5) {$Q$};
\end{scope}
\end{tikzpicture}
}
\end{center}
...
\MEntry{Strahl}{Strahl}, der in einem Punkt $P$ beginnt und in Richtung eines
zweiten Punktes $Q$ gedanklich immer weiter geht. Ein Strahl wird auch
...
%\coordinate (GP) at ($(A) + (-1,-0.5)$);
%\coordinate (GQ) at ($(B) + (1,0.5)$);
%Strahl:
\begin{scope}[xshift=-3cm]
\draw (-1,-0.5) -- (2,1);
\draw (-1,-0.5) circle(1pt);
...
Wenn man den Weg auf einer Strecke $\MGeoStrecke{P}{Q}$ in beide Richtungen 
...
dieses vorab festgelegt wurde, wird in Rechnung auch kurz vom Winkel 
...
\ifttm%
(Ein~$\pi$ geben Sie als \glqq\texttt{pi}\grqq\ ein. Sie k"onnen Ihr Ergebnis
auch auf drei Nachkommastellen gerundet angeben.)
\fi
...
\ifttm%
Ein~$\pi$ geben Sie als {\glqq\}texttt{pi}{\grqq} ein. Sie k"onnen Ihr Ergebnis
auch auf drei Nachkommastellen gerundet angeben.
\fi
