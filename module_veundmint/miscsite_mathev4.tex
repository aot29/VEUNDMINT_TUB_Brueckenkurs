...
\color{red}
Sie müssen nicht alle Module vollständig durcharbeiten, Sie sollten aber die Abschlusstests erfolgreich bearbeitet haben.
\color{black}
...
\ \\ \ \\
Bei Aktivierung der Datenspeicherung werden die Kursdaten (insbesondere eingegebene Lösungen) in Ihrem Browser sowie auf einem Server des KIT gespeichert.
Die Testergebnisse gehen in keiner Weise in Ihr Studium ein, werden aber statistisch ausgewertet um Informationen über den Jahrgang und den Erfolg des Vorkurses zu erhalten.
Zudem wird Ihr Nutzungsverhalten anonym kommuniziert um den Kurs zu verbessern, beispielsweise welche mathematischen Symbole Sie bei der Lösung der Aufgaben einsetzen.

Ihre Testergebnisse werden den Tutoren Ihrer Gruppe kommuniziert, damit diese das Niveau des Tutoriums einstellen können.

Falls Sie die Speicherung Ihrer Daten nicht wünschen können Sie in den folgenden Einstellungsoptionen das Verhalten des Kurses anpassen.
\end{MInfo}
...
% \ \\ \ \\
% Einige Features sind noch nicht verfügbar und technische sowie inhaltiche Fehler können trotz sorgfältiger Prüfung des Materials nicht ausgeschlossen werden.
% \ \\ \ \\
% Über den Feedback-Button kann eine Mail an den Kursadministrator geschickt werden, bitte melden Sie inhaltliche Fehler oder technische Probleme. Beachten Sie dabei,
% dass der Kurs nur mit den neueren Versionen der auf der Startseite gelisteten Browser bearbeitet werden kann und für eine Nutzung an einem PC oder Notebook/Tablet mit
% einer genügend hohen Auflösung (mindestens 1024x1020) konzipiert ist. Für mobile Endgeräte mit kleinerer Auflösung ist der Kurs nicht geeignet.
% \ \\ \ \\
% Der endgültige Release sowie der Einsatz im Massenbetrieb an den Standorten des VE\&MINT-Projekts ist zum Juli 2015 geplant.
% \end{MSContent}
% 
% \fi
