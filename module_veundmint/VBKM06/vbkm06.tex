...
\[
 \R\setminus\{0\}=(-\infty\MIntvlSep 0)\cup(0\MIntvlSep \infty).
\]
...
\[
\N\ni1\longmapsto\MZahl{0}{5}\in\Q,\ \N\ni2\longmapsto1\in\Q,\text{ usw.} 
\]
...
\begin{itemize}
 ...
 \item Eine Funktion $\Mvarphi$ soll jeder reellen Zahl $y$ zwischen $0$ und $1$ ihren dreifachen Wert plus $1$ zuordnen. Dies ist ein Beispiel für eine sogenannte linear-affine Funktion (siehe \MNRef{sec:linear-affin}):
 \[
...
\[
 0<y<1\,|\cdot 3\quad\Leftrightarrow\quad 0<3y<3\,|+1\quad\Leftrightarrow\quad 1<3y+1<4\quad\Leftrightarrow\quad 1<\Mvarphi(y)<4.
\]
...
\[
 \function{V}{(0\MIntvlSep \infty)}{\R}{a}{V(a)=a^3.}
\]
...
\[
 \function{I}{(0\MIntvlSep \infty)}{\R}{r}{\frac{c}{r^2}.}
\]
...
Ferner gilt
\[
 \MCases{x>x^2>x^3>x^4>\dots & \text{für}\ x\in(0\MIntvlSep 1) \\ x<x^2<x^3<x^4<\dots & \text{für}\ x\in(1\MIntvlSep \infty).}
\]
...
\[
 W_{f_n}=\MCases{\R\setminus\{0\} & \text{falls}\ n\ \text{ungerade} \\ (0\MIntvlSep \infty) & \text{falls}\ n\ \text{gerade.}}
\]
...
Dabei ist $g\approx \MZahl{9}{81}\frac{\text{m}}{\text{s}^2}$ die physikalische Konstante der Fallbeschleunigung. Trägt man nun diese Werte in einem Diagramm mit $t$ auf der Hochachse und $s$ auf der Querachse auf, erhält man:
...
$$p: [0\MIntvlSep \infty) \longrightarrow (0\MIntvlSep  \infty) \;\text{mit}\; t \longmapsto p(t) = 500 \cdot 2^{(t/13)}$$
...
$$p(75) = 500 \cdot 2^{(75/13)} \approx 500 \cdot \MZahl{54}{539545} \approx 27270$$
...
\MDeclareSiteUXID{VBKM06_Exponentialfunktionen_efkt}
 ...
 $$e = \MZahl{2}{718281828459045235} \dots \; \; \; .$$
 ...
 \begin{MExercise}
  ...
  Wie sieht der Graph der Funktion $h: \R \rightarrow (0\MIntvlSep  \infty)$, $x \Mmapsto h(x) = e^{- x}$ aus, und welche generellen
 ...
 \begin{MExample}
 Bei einer Versuchsreihe mit radioaktiven Jodatomen ($^{131}\text{I}$) ergeben sich im Mittel folgende Daten:
  \begin{center}
  ...
  $$\lambda \approx - \MZahl{0}{0862} \; \Mdfrac{1}{\text{Tage}} \; \; \; .$$
...
\MDeclareSiteUXID{VBKM06_TrigonometrischeFunktionen_Sinus}
 ...
 durch
$$\begin{array}{rcccccl}
  \sin(\alpha) & = & \Mdfrac{\text{Gegenkathete}}{\text{Hypotenuse}}. & & \\[2ex]
 \end{array}$$
 ...
 \begin{MExample}
  ...
  $$\Mvarphi (t) = A \, \sin(\Mdfrac{2 \pi}{T} t + b)$$
...
\MDeclareSiteUXID{VBKM06_TrigonometrischeFunktionen_CosTan}
 ...
 \begin{MExercise}
  ...
  \begin{MHint}{Lösung}
   ...
   $$\cos(\alpha) = 0 \Leftrightarrow \alpha \in \{ \Mtfrac{2k+1}{2} \cdot \pi; k \in \Z \} \;.$$
  \end{MHint}
 \end{MExercise}
 ...
 \begin{MExercise}
  ...
  $$\Mvarphi (t) = \Mdfrac{\pi}{6} \cdot\sin \left(2 t + \Mdfrac{\pi}{2}\right) \;.$$
  ...
  und wenn ja, wie sieht dann $\Mvarphi (t)$ aus?  \begin{MHint}{Lösung}
   ...
   und damit:
   $$\Mvarphi (t) = \Mdfrac{\pi}{6}\cdot\cos(2 t)$$.
...
\begin{itemize}
 ...
 \]
 also die Standardparabel (vgl.~Abschnitt \MNRef{sec:monome}) und die Betragsfunktion (vgl.~Abschnitt \MNRef{sec:betrag}), sind Beispiele für gerade Funktionen. Es gilt $f_1(-x)=(-x)^2=x^2=f_1(x)$ und $f_2(-x)=|-x|=|x|=f_2(x)$ für alle $x\in\R$. Die Graphen weisen die Spiegelsymmetrie an der Hochachse auf:
...
\end{tikzpicture} 
 ...
 \item Die Funktion 
 \[
  \function{h}{(0\MIntvlSep \infty)}{\R}{x}{e^x\frac{1}{x}}
 \]
 ist das \modsemph{Produkt} aus der natürlichen Exponentialfunktion mit Abbildungsvorschrift $e^x$ (vgl.~Abschnitt \MNRef{sec:exp}) und der Hyperbel mit Abbildungvorschrift $\frac{1}{x}$ (vgl.~Abschnitt \MNRef{sec:hyperbel}). Sie besitzt folgenden Graphen:
...
\end{tikzpicture}
 ...
 \[
  \function{w\circ h}{[0\MIntvlSep \pi]}{\R}{x}{\sqrt{\sin(x)}.}
 \]
\end{itemize}
\end{MExample}
...
\[
 \function{u}{(0\MIntvlSep \infty)}{\R}{y}{-\log_2(y).}
\]
...
\begin{tabular}{lll}
\MLCheckbox{0}{EF1}{CHECKBOXEF1} & \ \ & ... kann man kürzer auch als $f(x)=2x+1$ schreiben.\\
\MLCheckbox{1}{EF2}{CHECKBOXEF2} & \ \ & ... ist eine linear-affine Funktion.\\
\MLCheckbox{0}{EF3}{CHECKBOXEF3} & \ \ & ... hat die Wertemenge $\R$.\\
\MLCheckbox{1}{EF4}{CHECKBOXEF4} & \ \ & ... hat die Steigung $2$.\\
\MLCheckbox{1}{EF5}{CHECKBOXEF5} & \ \ & ... kann nur Werte größer oder gleich $1$ und kleiner $7$ annehmen.\\
\MLCheckbox{1}{EF6}{CHECKBOXEF6} & \ \ & ... hat als Graph ein Stück einer Gerade.\\
\MLCheckbox{1}{EF7}{CHECKBOXEF7} & \ \ & ... hat bei $x=0$ den Wert $1$.\\
\MLCheckbox{0}{EF8}{CHECKBOXEF8} & \ \ & ... hat die Definitionsmenge $\R$.
\end{tabular}
\end{MExercise}
