...
\begin{itemize}
\item{dem Abschnitt \MSRef{M06_Abschnitt1}{Grundlegendes zu Funktionen},
\MSRef{M06_Abschnitt2}{Lineare Funktionen und Polynome},
%\MSRef{M06_Abschnitt4}{Rationale Funktionen},
\MSRef{M06_Abschnitt5}{Potenzfunktionen},
\MSRef{M06_Abschnitt6}{Exponential- und Logarithmusfunktionen},
\MSRef{M06_Abschnitt8}{Trigonometrische Funktionen}, \MSRef{M06_Abschnitt9}{Eigenschaften und Konstruktion elementarer Funktionen},}
\item{dem \MSRef{M06_Abschlusstest}{Abschlusstest}.}
\end{itemize}
\end{MSectionStart}

\MSubsection{Grundlegendes zu Funktionen}
\MLabel{M06_Abschnitt1}\label{grundlagen}
...
\begin{MExercise}
Wie sehen die Intervalle $(-\infty;\pi)$ und $(8;8,5]$ auf der Zahlengeraden aus?
\begin{MHint}{Lösung}
\MTikzAuto{%
\begin{tikzpicture}
% reelle Achse
...
aufschreiben können. Dazu erinnern wir uns an den Beginn dieses Beispiels. Wir haben haben uns überlegt, jeder natürlichen Zahl $n$ ihre Hälfte $\frac{n}{2}$ zuzuordnen.
...
\begin{itemize}
 ...
 \item Eine Funktion $\varphi$ soll jeder reellen Zahl $y$ zwischen $0$ und $1$ ihren dreifachen Wert plus $1$ zuordnen. Dies ist ein Beispiel für eine sogenannte linear-affine Funktion (siehe \MNRef{sec:linear-affin}):
 \[
  \function{\varphi}{(0;1)}{\R}{y}{3y+1.}
 \]
 Die Abbildungsvorschrift von $\varphi$ lautet damit $\varphi(y)=3y+1$. Somit errechnet man beispielsweise $\varphi(\frac{1}{3})=3\cdot\frac{1}{3}+1=2$, usw. Allerdings kann man in diesem Fall $\varphi(8)$ oder auch $\varphi(1)$ \modsemph{nicht angeben}, da $8$ und $1$ keine Elemente der Menge $(0,1)$ sind.  %Dies wird unten im Rest dieses Abschnitts nochmal genauer erörtert werden. 
\end{itemize}
...
\begin{itemize}
 ...
 \[
  \function{w}{[4;9]}{\R}{\alpha}{\sqrt{\alpha}.}
 \]
...
\begin{MHint}{Lösung}
 \begin{itemize}
  \item[(i)] Der Kehrwert von $x$ ist $\frac{1}{x}$. Die positiven reellen Zahlen sind die Menge $(0;\infty)$. Damit kann man die Funktion $h$ schreiben als
  \[
   \function{h}{(0;\infty)}{\R}{x}{\frac{1}{x}.}
  \]
  Dies ist ein Beispiel für eine Funktion vom hyperbolischen Typ und wird in Abschnitt \MNRef{M06_Abschnitt5} genauer behandelt. Die Abbildungsvorschrift von $h$ ist $h(x)=\frac{1}{x}$, womit gilt $h(2)=\frac{1}{2}$ und $h(1)=\frac{1}{1}=1$. Außerdem berechnet man $h(3)=\frac{1}{3}$, womit $3\longmapsto\frac{1}{3}$ gilt. Weiterhin führt die Überlegung $h(\frac{1}{2})=\frac{1}{\frac{1}{2}}=2$ auf $\frac{1}{2}\longmapsto2$.
...
\[
   \function{h}{(0;\infty)}{\R}{x}{\frac{1}{x}}
\]
aus Aufgabe \MNRef{ex1:grundlagen}, die Menge $D_h=(0;\infty)$. Auch für die Elemente des Definitionsbereichs gibt es eine spezielle Bezeichnung.
...
\[
  \function{g}{\R}{\R}{x}{x^2,}
\]
womit man $D_w=[4,9]$ und $D_g=\R$ erhält.
\end{MHint}
\end{MExercise}
...
einer Funktion, die größtmögliche Teilmenge der reellen Zahlen $\R$, die man als Definitionsmenge einer Funktion mit bekannter Abbildungsvorschrift benutzen
kann.     

\begin{MExample}
Der größtmögliche Definitionsbereich $D_h\subset\R$ der Funktion
\[
 \function{h}{D_h}{\R}{x}{\frac{1}{x},}
\]
...
an.
\begin{MHint}{Lösung}
Die Wurzel liefert für alle nicht-negativen reellen Zahlen ein sinnvolles reelles Ergebnis. Somit gilt $D_w=[0,\infty)$.  
\end{MHint}
\end{MExercise}
...
\[
 W_\varphi=(1;4).
\]
Dies sieht man ein, indem man einige Werte aus $D_\varphi=(0;1)$ in die Abbildungsvorschrift einsetzt und die Ergebnisse berechnet.
Dies führt auf eine sogenannte \MEntry{Wertetabelle}{Wertetabelle}:

\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
$y$ & $0,1$ & $0,3$ & $0,5$ & $0,7$ & $0,9$ \\\hline 
$\varphi(y)$ & $1,3$ & $1,9$ & $2,5$ & $3,1$ & $3,7$ \\ \hline
\end{tabular}
\end{center}
\end{MExample}
...
\[
  \function{\varphi}{(0;1)}{\R}{y}{3y+1}
\]
gilt aufgrund des Definitionsbereichs $D_\varphi=(0,1)$ für die Veränderliche:
\[
 0<y<1.
\]
Nun benutzen wir Äquivalenzumformungen um in diesen Ungleichungen die Abbildungsvorschrift $\varphi(y)=3y+1$ zu erzeugen:
\[
 0<y<1\,|\cdot 3\quad\Leftrightarrow\quad 0<3y<3\,|+1\quad\Leftrightarrow\quad 1<3y+1<4\quad\Leftrightarrow\quad 1<\varphi(y)<4.
\]
Somit gilt für die Werte der Funktion $\varphi(y)\in(1;4)$ und deshalb $W_\varphi=(1;4)$.
\end{MExample}
...
\[
 \function{V}{(0,\infty)}{\R}{a}{V(a)=a^3.}
\]
...
Die Intensität radioaktiver Strahlung ist \highlight{umgekehrt proportional} zum Quadrat des Abstands von der Quelle. Dies wird auch als \modsemph{Abstandsgesetz} bezeichnet. Unter Benutzung einer physikalischen \highlight{Proportionalitätskonstante} $c>0$ kann man den Zusammenhang zwischen Intensität $I$ der Strahlung und Abstand $r>0$ von der Quelle folgendermaßen als mathematische Funktion formulieren:
\[
 \function{I}{(0,\infty)}{\R}{r}{\frac{c}{r^2}.}
\]
...
a)
\[
 \function{P}{(0,\infty)}{\R}{v}{P(v)=\frac{\rho}{v^3}}
\]
b)
\[
...
c)
\[
 \function{P}{[0,\infty)}{\R}{v}{P(v)=\rho v^3}
\]
d)
\[
 \function{x}{[0,\infty)}{\R}{f}{x(f)=\rho f^3}
\]
\begin{MHint}{Lösung}
...
\[
  \function{\varphi}{(0,1)}{\R}{y}{3y+1}
\]
mit dem Definitionsbereich $D_\varphi=(0,1)$ und dem Wertebereich $W_\varphi=(1,4)$ aus Abschnitt \MNRef{sec:zuordnungen} ins Gedächtnis. Wenn wir ihren Graphen zeichnen, so erkennen wir, dass der Definitionsbereich und der Wertebereich auf der Quer- bzw.~Hochachse auftauchen:
...
\draw (-1,5) node[anchor=north west] {$\varphi(y)$};
%Graph: x \mapsto 3x+1
...
Auf der Querachse in diesem Graphen erkennen wir zwei Bereiche, in denen der Graph unterschiedliches \modsemph{Wachstumsverhalten} zeigt. Im Bereich von $x$-Werten mit $x\in(-\infty,0)$ \modsemph{fällt} der Graph. Das heißt, werden die $x$-Werte größer, so werden die zugehörigen Funktionswerte auf der Hochachse kleiner. Im Bereich von $x$-Werten mit $x\in(0,\infty)$ stellen wir das gegenteilige Verhalten fest. Bei größer werdenden $x$-Werten werden auch die zugehörigen Funktionswerte größer. Der Graph \modsemph{wächst}. Beim Wert $0\in\R$ geht der fallende Bereich in den wachsenden Bereich über. Solche Werte werden bei der Untersuchung von Scheitelpunkten in Abschnitt \MNRef{sec:polynome} und beim Bestimmen von \MSRef{VBKM07_Kurvendiskussion}{Extremwerten} in Modul \MNRef{VBKM07} besonders wichtig werden.
...
\begin{MInfo}
\begin{itemize}
 \item Für $x_1<x_2$ aus einer Teilmenge des Definitionsbereich einer Funktion $f$ gilt $f(x_1)>f(x_2)$. Dann heißt $f$ in dieser Teilmenge streng monoton fallend.
...
\end{tikzpicture}  
 ...
 \item Für $x_1<x_2$ aus einer Teilmenge des Definitionsbereich einer Funktion $f$ gilt $f(x_1)<f(x_2)$. Dann heißt $f$ in dieser Teilmenge streng monoton wachsend.
...
\[
 x_1<x_2\,|\cdot(-\frac{1}{2})\quad\Leftrightarrow\quad -\frac{1}{2}x_1>-\frac{1}{2}x_2.
\]
Weiterhin wird in der Abbildungsvorschrift $+1$ addiert. Wir erhalten also
\[
 -\frac{1}{2}x_1>-\frac{1}{2}x_2\,|+1\quad\Leftrightarrow\quad -\frac{1}{2}x_1+1>-\frac{1}{2}x_2+1\quad\Leftrightarrow\quad h(x_1)>h(x_2),
\]
...
Es gilt
\[
 x_1<x_2\,|\cdot2\quad\Leftrightarrow\quad2x_1<2x_2\,|+2\quad\Leftrightarrow\quad2x_1+2<2x_2+2\quad\Leftrightarrow\quad\eta(x_1)<\eta(x_2),
\]
...
\MLabel{M06_Abschnitt2}\label{linear}
...
Woher kommt der Begriff Steigung für eine lineare Funktion? Teilt man die Differenz, um welche der Graph in vertikaler Richtung anwächst, duch die entsprechende Längeneinheit in horizontaler Richtung, so erhält man die Steigung $m$. 
...
\begin{MXContent}{Linear-affine Funktionen}{Affin}{STD}\MLabel{sec:linear-affin}
...
die Steigung $m=-2$ und der Achsenabschnnitt $c=-1$. Der Achsenabschnnitt ergibt sich als Funktionswert bei $x=0$ und somit durch
\[
 c=f(0)=-2\cdot 0-1 = -1.
\]

\begin{MExercise}
Was sind die Steigung und der Achsenabschnnitt von 
\[
 \function{f}{\R}{\R}{x}{\pi x-42?}
\]
\begin{MHint}{Lösung}
...
\begin{MExercise}
Welche Funktionen ergeben sich als linear-affine Funktionen mit Steigung $m=0$ und welche mit Achsenabschnnitt $c=0$ ?
\begin{MHint}{Lösung}
Ist $m=0$, so gilt $f(x)=0\cdot x+c=c$. Damit ergeben sich die konstanten Funktionen als diejenigen mit Steigung $0$. Ein verschwindender Achsenabschnitt $c=0$ impliziert $f(x)=mx+0=mx$. Somit ergeben sich in diesem Fall genau die linearen Funktionen.
\end{MHint}
\end{MExercise}
...
\begin{MXContent}{Betragsfunktionen}{Betrag}{STD}\MLabel{sec:betrag}
...
\[
 |x| = \begin{MCaseEnv} x & \text{falls}\;x\geq 0 \\ -x & \text{falls}\;x<0. \end{MCaseEnv}
\]
...
\begin{MHint}{Lösung}
Da für alle Zahlen $x$ aus $D_b=\R$ gilt $b(x)=|x|\geq0$, ergibt sich $W_b=[0,\infty)$. 
\end{MHint}
\end{MExercise}
...
Wir berechnen:
\[
 f(x)=|2x-1|=\begin{MCaseEnv} 2x-1 & \text{falls}\ 2x-1\geq0 \\ -(2x-1) & \text{falls}\ 2x-1<0\end{MCaseEnv}=\begin{MCaseEnv} 2x-1 & \text{falls}\ x\geq\frac{1}{2} \\ -2x+1 & \text{falls}\ x<\frac{1}{2}.\end{MCaseEnv}
\]
...
\draw[color=red] (2.5,4) node[anchor=south] {$G_{f_1}$};
\end{tikzpicture}
}%
\end{MExample}
...
Es gilt
\[
 \alpha(x)=|-8x+1|-1=\begin{MCaseEnv} -8x+1-1 & \text{falls}\ -8x+1\geq0 \\ -(-8x+1)-1 & \text{falls}\ -8x+1<0 \end{MCaseEnv}=\begin{MCaseEnv} -8x & \text{falls}\ x\leq\frac{1}{8} \\ 8x-2 & \text{falls}\ x>\frac{1}{8},\end{MCaseEnv}
\]
folglich: 
...
Da $|-8x+1|\geq0$ gilt, folgt $|-8x+1|-1\geq -1$ und damit ist $W_\alpha=[-1,\infty)$.
\end{MHint}
\end{MExercise}
...
\]
mit $n\in\N$ und bezeichnet diese Funktionen als \highlight{Monome}. Der Exponent $n$ eines Monoms wird als \highlight{Grad} des Monoms bezeichnet. So ist etwa die Funktion $g$ vom Anfang dieses Abschnitts ein Monom vom Grad $2$, usw.
...
und
\[
 f(-1)=\MCases{1 & \text{falls}\ n\ \text{gerade} \\ -1 & \text{falls}\ n\ \text{ungerade.} }
\]
Ferner gilt
\[
 \MCases{x>x^2>x^3>x^4>\dots & \text{für}\ x\in(0,1) \\ x<x^2<x^3<x^4<\dots & \text{für}\ x\in(1,\infty).}
\]

\begin{MExercise}
Wie ergeben sich diese Erkenntnisse über Monome unmittelbar aus den Potenzrechengesetzen?
\begin{MHint}{Lösung}
Nach den Potenzrechengesetzen gilt stets $1^n=1$ und $0^n=0$ für beliebige natürliche Zahlen $n$ und $(-1)^n=1$ für gerade $n$ sowie $(-1)^n=-1$ für ungerade $n$. Dadurch ergeben sich die beschriebenen Punkte die alle Graphen der Monome gemeinsam haben. Auch die Ungleichungen $x>x^2>x^3>x^4>\dots$ für $x$ kleiner $1$ und $x<x^2<x^3<x^4<\dots$ für $x$ größer $1$ folgen aus den Potenzrechengesetzen, da höhere werdende Potenzen für Zahlen kleiner $1$ stets kleiner werdende Ergebnisse liefern und für Zahlen größer $1$ umgekehrt stets größer werdende Ergebnisse.
\end{MHint}
\end{MExercise}
...
Möchte man allgemein ein unspezfiziertes Polynom vom Grad $n\in\N$ angeben, so schreibt man dies folgendermaßen:
\[
 \function{f}{\R}{\R}{x}{a_nx^n+a_{n-1}x^{n-1}+a_{n-2}x^{n-2}\dots +a_{2}x^{2}+a_1x+a_0.}
\]
...
\]
und besitzt den Wertebereich $W_f=[-4,\infty)$, da die geraden Potenzen von $x$ nur nichtnegative Werte annehmen können.
\end{MHint}
\end{MExercise}
...
\[
 y^2-4y+3=y^2-4y+4-1=(y-2)^2-1.
\]
Folglich läßt sich die Abbildungsvorschrift als
\[
 \zeta(y)=2(y-2)^2-2
\]
schreiben. Wir erkennen, dass die Parabel gegenüber der Standardparabel um $2$ Längeneinheiten nach rechts und nach unten verschoben ist.  Der Scheitelpunkt läßt sich bei $(2|-2)$ ablesen. Die Nullstellen lassen sich berechnen:
\[
 \zeta(y)=2((y-2)^2-1)=0\quad\Leftrightarrow\quad (y-2)^2=1\quad\Leftrightarrow\quad y_{1,2}-2=\MCases{1\\-1}\quad\Leftrightarrow\quad y_{1,2}=\MCases{3\\1}
\]
...
% \MSubsection{Rationale Funktionen}
% \MLabel{M06_Abschnitt4}%\label{rational}
% 
% \begin{MIntro}
% 
% In diesem Kapitel betrachten wir Funktionen, in deren Abbildungsvorschriften auch \modsemph{Quotienten der Veränderlichen} vorkommen, insbesondere solche, in denen die Veränderliche im Nenner eines Quotienten auftaucht. Ein einfaches Beispiel ist etwa die Funktion
% \[
%  \function{f}{\R}{\R}{x}{\frac{x}{x^2+1}.}
% \]
% \end{MIntro}
...
Wir betrachten Funktionen, die als Abbildungsvorschrift einen \highlight{reziproken Zusammenhang} besitzen. Darunter versteht man das Vorkommen von \highlight{Kehrwerten} in der Abbildungsvorschrift. Zu beachten ist bei der Bestimmung des \modsemph{größtmöglichen Definitionsbereich} solcher Funktionen, dass der Nenner nicht $0$ werden darf.  
...
\]
\[
 \function{f_2}{\R\setminus\{0\}}{\R}{x}{\frac{1}{x^2},}
\]
\[
 \function{f_3}{\R\setminus\{0\}}{\R}{x}{\frac{1}{x^3},}
\]
...
\draw[color = orange](-1,1) node[left]{$G_{f_3}$};
%1/x^3
...
\[
 \function{f_n}{\R\setminus\{0\}}{\R}{x}{\frac{1}{x^n}.}
\]
\begin{MExercise}
...
\[
 W_{f_n}=\MCases{\R\setminus\{0\} & \text{falls}\ n\ \text{ungerade} \\ (0,\infty) & \text{falls}\ n\ \text{gerade.}}
\]
...
\[
 \function{f}{\R}{\R}{x}{\frac{8}{x^2+1},}
\]
\[
 \function{g}{\R\setminus\{-4,\frac{3}{2}\}}{\R}{x}{\frac{-18x+3}{2x^2+5x-12},}
\]
\[
 \function{h}{\R\setminus\{-1\}}{\R}{x}{\frac{x^3-x^2+x}{x+1}.}
\] 
...
\[
 x^2-\pi=0\quad\Leftrightarrow\quad x^2=\pi\quad\Leftrightarrow\quad x=\pm\sqrt{\pi}.
\]
...
\begin{MExercise}
Bestimmen Sie für die gebrochen rationalen Funktionen im einführenden Beispiel \MNRef{einf_bsp_gebr_rat} jeweils den Zähler- sowie den Nennergrad und berechnen Sie die Nullstellen des Zählers und des Nenners.
\begin{MHint}{Lösung}
...
3$, so dass keine weitere reelle Lösung -- und damit keine weitere Nennernullstelle -- existiert.
\end{MHint}
\end{MExercise}
...
\[
 \function{j}{\R\setminus\{-1,3\}}{\R}{x}{\frac{x-1}{x^2-2x-3}}
\]
...
\[
 2x-1=0\quad\Leftrightarrow\quad 2x=1\quad\Leftrightarrow\quad x=\frac{1}{2}.
\]
Aber für den Zähler gilt:
\[
 x^4-1=0\quad\Leftrightarrow\quad x^4=1\quad\Leftrightarrow\quad x=\pm 1,
\]
...
Ein weitere Unterschied wird zwischen den beiden Polstellen von $f_1$ und $f_2$ deutlich. Bei der Polstelle $x=2$ von $f_1$ findet ein \modsemph{Vorzeichenwechsel} der Funktion statt. Der Graph von $f_1$ fällt links der Polstelle unbeschränkt ins Negative und wächst rechts der Polstelle unbeschränkt ins Positive.//
Der Graph von $f_2$ wächst auf beiden Seiten der Polstelle $x=-3$ ins positive, es findet also \modsemph{kein Vorzeichenwechsel} statt. 
...
\[
 x_{1,2}=\frac{1\pm\sqrt{1^2-4\cdot(-6)\cdot1}}{2}=\frac{1\pm 5}{2}=\MCases{3\\-2.}
\]
Folglich ist der größtmögliche Definitionsbereich
\[
 D_\gamma=\R\setminus\{-2,3\}.
\]
Die Zählernullstelle ergibt sich durch $3x+6=0$, liegt also ebenfalls bei $x=-2$. Wir können damit die Abbildungsvorschrift von $\gamma$ also für $x\neq-2$ folgendermaßen umformen:
\[
 \gamma(x)=\frac{3x+6}{x^2+x-6}=\frac{3(x+2)}{(x-3)(x+2)}=\frac{3}{x-3}.
\]
Damit läßt sich die Funktion auch als
\[
 \function{\gamma}{\R\setminus\{-2,3\}}{\R}{x}{\frac{3}{x-3}}
\]
...
\[
 \function{f}{\R\setminus\{-\pi\}}{\R}{x}{\frac{x}{x+\pi}.}
\]
In $f$ ist der Zählergrad $1$ und der Nennergrad $1$. Solche Funktionen werden als \highlight{echt gebrochenrational} bezeichnet. Beispiele hierfür haben wir im vorhergehenden Abschnitt \MNRef{sec:gebrochen} betrachtet. 
...
\]
und stellen fest, dass deren Abbildungsvorschrift in der Form einer Summe aus einem Polynom (vom Grad $0$) und einem echt gebrochenrationalen Term vorliegt. Durch Bilden des Hauptnenners ist es nun einfach $g(x)$ auf eine unecht gebrochenrationale Form zu bringen, in der der Zählergrad und der Nennergrad gleich sind:
\[
 g(x)=1+\frac{1}{x-1}=\frac{x-1}{x-1}+\frac{1}{x-1}=\frac{x-1+1}{x-1}=\frac{x}{x-1}.
\]
...
Dies erkennt man in der Abbildungsvorschrift $g(x)=1+\frac{1}{x-1}$ folgendermaßen. Für "`sehr große'' ($5$, $10$, $100$, usw.) oder "`sehr kleine'' ($-5$, $-10$, $-100$, usw.) Werte für $x$ nähert sich der echt gebrochenrationale Anteil $\frac{1}{x-1}$ immer mehr der $0$ an, da $x$ dort im Nenner vorkommt. Tendenziell bleibt also für solche Werte nur noch der polynomielle Anteil $1$ aus der Abbildungsvorschrift übrig. Dieser Anteil kann nun durch eine -- in diesem Fall konstante -- Funktion beschrieben werden, die als \highlight{Asymptote} $g_{As}$ der Funktion $g$ bezeichnet wird:
\[
 \function{g_{As}}{\R}{\R}{x}{1}
\]
...
\[
 i(x)=3-\frac{6}{x+2}
\]
mit echt gebrochenrationalem Anteil $\frac{6}{x+2}$. Folglich hat die waagrechte Asymptote von $i$ die Abbildungsvorschrift $i_{As}(x)=3$. Die Hyperbel
\[
...
\[
 f(x)=0+\frac{1}{x},
\]
womit für die Asymptote gilt $f_{As}(x)=0$. Hier ist die Asymptote also diejenige Funktion, die konstant $0$ ist: die Nullfunktion, oder -- anders gesagt -- die Querachse des Koordinatensystems. 
\end{MHint}
\end{MExercise}
...
\]
hat als Asymptote eine konstante Funktion -- also eine waagrechte Asymptote -- falls $z\leq n$ gilt (insbesondere ist die Nullfunktion die Asymptote im Fall $z<n$).  
\end{MInfo}
...
\MLabel{M06_Abschnitt5}\label{potenz}
\begin{MIntro}
...
\begin{tabular}{c|c|c|c|c|c}
Fallzeit $t$ in Sekunden & $0$ & $\sqrt{\frac{2}{g}}$ & $\sqrt{\frac{2}{g}}\cdot1.5$ & $\sqrt{\frac{2}{g}}\cdot 2$ & $\sqrt{\frac{2}{g}}\cdot 3$ \\\hline
zurückgelegter Weg $s$ in Metern & $0$ & $1$ & $2.25$ & $4$ & $9$
\end{tabular}

Dabei ist $g\approx9.81\frac{\text{m}}{\text{s}^2}$ die physikalische Konstante der Fallbeschleunigung. Trägt man nun diese Werte in einem Diagramm mit $t$ auf der Hochachse und $s$ auf der Querachse auf erhält man:
...
\[
 \function{t}{[0,\infty)}{\R}{s}{\sqrt{\frac{2}{g}}\cdot \sqrt{s}}
\]
...
\[
 f_2(x)=\sqrt{x}=x^{\frac{1}{2}}, \quad f_3(x)=\sqrt[3]{x}=x^{\frac{1}{3}}, \quad f_4(x)=\sqrt[4]{x}=x^{\frac{1}{4}}, \dots
\]
\end{MHint}
\end{MExercise}
...
\[
 f_1(x)=\sqrt[1]{x}=x^{\frac{1}{1}}=x=\Id(x).
\]
...
\]
mit $n\in\N$, $n>1$ gelten die folgenden größtmöglichen Definitionsbereiche:
\[
D_{f_n}= %\MCases{{\R} & \text{falls}\ n\ \text{ungerade} \\ [0,\infty) & \text{falls}\ n\ \text{gerade.}} 
%\MCases{\R &  \text{falls $n$ ungerade} \\ [0,\infty) & \text{falls $n$ gerade.} }
\]
\end{MInfo}
...
\begin{MHint}{Lösung}
Für die geradzahligen Wurzelfunktionen ergeben sich offenbar als Werte nur nicht-negative reelle Zahlen, denn nach den Potenzrechengesetzen kann $\sqrt{x}$, $\sqrt[4]{x}$, $\sqrt[6]{x}$, usw.~für $x\geq 0$ nie negativ werden. Die ungeradzahligen Wurzelfunktionen können auch alle negativen reellen Zahlen als Werte liefern. Tatsächlich gilt $\sqrt[3]{x}<0$, $\sqrt[5]{x}<0$, usw.~genau dann wenn $x<0$ ist. Zusammenfassend gilt dann also unter Berücksichtigung der strengen Monotonie der Wurzelfunktionen:
%\[
 %W_{f_n}=\MCases{\R & \text{falls }n\text{ ungerade} \\ [0,\infty) & \text{falls }n\text{ gerade.}}
%\]
\end{MHint}
...
$$x \longmapsto 2^x\; \text{oder}\; x \longmapsto 10^x$$
...
$$p: [0;\infty) \longrightarrow (0\infty) \;\text{mit}\; t \longmapsto p(t) = 500 \cdot 2^{(t/13)}$$
...
der Wertebereich nur aus den positiven reellen Zahlen besteht ($W_f = \R^+$), da jedwede Potenz einer
...
\begin{MExercise}
 Warum setzt man bei den Exponentialfunktionen voraus, dass die Basis $a$ größer Null sein soll? \begin{MHint}{Lösung}
  ...
  Zum Beispiel sind Quadratwurzeln aus negativen Zahlen nicht definiert, vergleiche auch Abschnitt \MRef{M06_Abschnitt5}.
 \end{MHint}
\end{MExercise}
...
\begin{MXContent}{Eulersche Funktion}{Eulersche Funktion}{STD}\MLabel{sec:exp}
\MLabel{M06_e_fkt}
\MDeclareSiteUXID{VBKM06_Exponentialfunktionen_efkt}
 ...
 \end{array}$$
 wegen der Basis $e$ auch \MEntry{$e$-Funktion}{e-Funktion} oder \MEntry{natürliche Exponentialfunktion}{Exponentialfunktion (natürlich)} genannt:
 \begin{center}
 ...
 Insbesondere halten wir nochmals fest, dass die $e$-Funktion \modstextbf{streng monoton wachsend} ist, sich für große
 ...
 \MSRef{VBKM06_LNDEF}{natürlichen Logarithmus}, dessen Funktionsgestalt im folgenden Abschnitt
 \MRef{M06_Abschnitt7} noch ausgiebig beschäftigen wird. 
 \begin{MExercise}
  Begründen Sie die Identität $a^x = e^{x \cdot \ln(a)}$. \begin{MHint}{Lösung}
   Nach dem Potenzgesetz $(b^r)^s = b^{r \cdot s}$ kann die rechte Seite der in Frage stehenden Identität als
   $e^{x \cdot\ln(a)} = (e^{\ln(a)})^x$ umgeschrieben werden. Da $\ln$ die Umkehrung zu \textit{$e$ hoch}
   ist, folgt $e^{\ln(a)} = a$. Damit folgt $(e^{\ln(a)})^x = a^x$, was in der Tat die linke Seite der Identität darstellt.
  \end{MHint}
 \end{MExercise}
 
 
 \modstextbf{Allgemeine $e$-Funktionen} enthalten die bereits in Unterabschnitt \MRef{M06_allgem_exp_fkt} eingeführten
 ...
 \begin{MExample}
  ...
  \begin{center}
   \begin{tabular}{|c|r|r|r|r|r|}
    ...
    Anzahl Tage seit Beginn & 0 & 8,04 & 16,08 & 24,12 & usw.\\ \hline
   \end{tabular}
  \end{center}
  Mit anderen Worten: Alle $8,04$ Tage halbiert sich die Anzahl der Jodatome aufgrund radioaktiven Zerfalls; man
  spricht daher in diesem Zusammenhang davon, dass die \modstextbf{Halbwertszeit} $h$ von Jod-$131$ $h = 8,04$ Tage beträgt.
  ...
  \MRef{M06_Abschnitt7}):
  $$\ln \left( \Mdfrac12 \right ) = \ln(e^{\lambda \cdot h})$$
  ...
  wir, dass Logarithmieren die Umkehrung zum Exponentieren darstellt, $\ln(e^{\lambda \cdot h}) = \lambda \cdot h$;
  damit folgt:
  $$\begin{array}{crcl} & - \ln(2) & = & \lambda \cdot h \\[.5ex]
  \Leftrightarrow & \lambda & = & - \Mdfrac{\ln(2)}{h} \end{array}$$
  Mit $h = 8,04$ Tage für Jod-$131$ folgt für den Parameter $\lambda$ im vorliegenden Fall
  $$\lambda \approx - 0,0862 \; \Mdfrac{1}{\text{Tage}} \; \; \; .$$
...
\MLabel{M06_Abschnitt7}\MLabel{M06_Logarithmus}
\MDeclareSiteUXID{VBKM06_Logarithmusfunktion}

In Abschnitt \MRef{M06_e_fkt} haben wir beim Studium der $e$-Funktion,
...
\begin{MInfo}
\MLabel{VBKM06_LNDEF}
Die über die Gleichung $e^{\ln(x)}=x$ erklärte Funktion
$$
\function{\ln}{(0;\infty)}{\R}{x}{\ln(x)}
$$
heißt die \MEntry{natürliche Logarithmusfunktion}{Logarithmusfunktion (natürlich)}.
\end{MInfo}

Die Gleichung ist dabei so zu lesen, dass $\ln(x)=a$ derjenige Wert $a$ ist mit $e^a=x$.
...
$$
\function{\log_b}{(0;\infty)}{\R}{x}{\log_b(x)}
$$
...
$$
\ln(e^5) \;=\; 5\;\; , \;\;\ln(\sqrt{e}) \;=\; \ln(e^{\frac12}) \;=\; \frac12
$$
sowie für den allgemeinen Logarithmus
$$
\log_5(25) \;=\; \log_5(5^2) \;=\; 2 \;\; , \;\; \log_3(81) \;=\; \log_3(3^4) \;=\; 4\: .
$$
\end{MExample}

Dabei muss man auf die Basis des Logarithmus achten, beispielsweise ist
$$
\log_2(64) \;=\; \log_2(2^6) \;=\; 6 \;\; \text{aber}\;\;
...
\item{\MEquationItem{$\displaystyle\ln(\sqrt[3]{e})$}{\MLSimplifyQuestion{10}{1/3}{5}{}{5}{1}{LNA1}}. \begin{MHint}{Lösung}Es ist $\ln(\sqrt[3]{e})=\ln(e^{\frac13})=\frac13$.\end{MHint}}
...
\item{Logarithmus zur Basis $e$: $\log_e(x)=\ln(e)$, der natürliche Logarithmus ist für praktische Rechnungen meist ungeeignet (es sei denn,
der Ausdruck ist eine $e$-Potenz). Er wird als natürlich bezeichnet, weil die Exponentialfunktion zur Basis $e$ aus mathematischer Sicht einfacher ist
als die allgemeinen Exponentialfunktionen (z.B. weil $e^x$ seine eigene Ableitung ist, $b^x$ aber nicht $b\not=e$).}
\end{itemize}
...
\MDeclareSiteUXID{VBKM06_Logarithmusgesetze}
 ...
 \begin{MInfo}
  ...
  $$\begin{array}{rcll}
   \log (u \cdot v) & = & \log(u) + \log(v) & (u, v >0) \\[.5ex]
   \log \left( \Mdfrac{u}{v} \right) & = & \log (u) - \log (v) & (u, v >0) \\[.5ex]
   \log (u^x) & = & x \cdot \log(u) & (u>0, x \in \R)\: .
  \end{array}$$
 \end{MInfo}
 ...
 $$
 \ld(8^5) \;=\; \log_2(8^5) \;=\;5\cdot \log_2(8) \;=\; 5\cdot \log_2(2^3) \;=\; 5\cdot 3 \;=\; 15\: .
 $$
 Produkte in Logarithmen kann man in Summen außerhalb der Logarithmen zerlegen:
 $$
 \lg\left({100\cdot \sqrt{10}\cdot \frac1{10}}\right) \;=\; \lg(100)+\lg(\sqrt{10})-\lg(10) \;=\; 2+\frac12-1 \;=\; \frac32\: .
 $$
 \end{MExample}
...
\begin{MXContent}{Die Sinusfunktion}{Die Sinusfunktion}{STD}\MLabel{sec:sinus}
 \MLabel{M06_Sinusfkt}
\MDeclareSiteUXID{VBKM06_TrigonometrischeFunktionen_Sinus}
 ...
 durch
$$\begin{array}{rcccccl}
  \sin(\alpha) & = & \Mdfrac{\text{Gegenkathete}}{\text{Hypothenuse}}. & & \\[2ex]
 \end{array}$$
 ...
 $$\begin{array}{rrcl}
  \sin : & \R & \longrightarrow & [- 1, + 1] \\ & \alpha & \longmapsto & \sin(\alpha) \; \; \; ,
 \end{array}$$
 ...
 \begin{itemize}
  \item{Die Sinusfunktion ist auf ganz $\R$ definiert, $D_{\sin} = \R$; der Wertebereich besteht dagegen nur aus dem
   Intervall von $- 1$ bis $+ 1$, diese beiden Endpunkte eingeschlossen: $W_{\sin} = [- 1, + 1]$}
 ...
 \begin{MExample}
  ...
  $\varphi$ aus der \modstextbf{Ruhelage} (der \modstextbf{Lotrechten}) hängt $\varphi$ von der Veränderlichen $t$, der Zeit, über
  eine allgemeine Sinusfunktion ab:
  $$\varphi (t) = A \, \sin(\Mdfrac{2 \pi}{T} t + b)$$
...
\MDeclareSiteUXID{VBKM06_TrigonometrischeFunktionen_CosTan}
 ...
 \begin{itemize}
  ...
  $+ 1$, die Endpunkte inbegriffen, $W_{\cos} = [- 1, + 1]$.
 ...
 \begin{MExercise}
  ...
  \begin{MHint}{Lösung}
   ...
   $$\cos(\alpha) = 1 \Leftrightarrow \alpha \in \{ 2 k \cdot \pi; k \in \Z \} $$
   ...
   $$\cos(\alpha) = - 1 \Leftrightarrow \alpha \in \{ (2 k + 1) \cdot \pi; k \in \Z \}$$
   ...
   $$\cos(\alpha) = 0 \Leftrightarrow \alpha \in \{ \Mtfrac{k}{2} \cdot \pi; k \in \Z \}$$
  \end{MHint}
 \end{MExercise}
 ...
 \begin{MExercise}
  ...
  der Pendelauslenkung $\varphi$ unter den Voraussetzungen bestimmen, dass die Schwingungsdauer $T$ gerade $\pi$ Sekunden beträgt,
  und dass zum Zeitpunkt $t = 0$ das Pendel bei einer Auslenkung von $30^\circ$ losgelassen wird:
  $$\varphi (t) = \Mdfrac{\pi}{6} \cdot\sin(2 t + \Mdfrac{\pi}{2}).$$
  Kann man diese Situation auch mit Hilfe der (allgemeinen) Kosinusfunktion (anstelle der Sinusfunktion) beschreiben,
  und wenn ja, wie sieht dann $\varphi (t)$ aus?  \begin{MHint}{Lösung}
   ...
   $$\sin(2 t + \Mdfrac{\pi}{2}) = \cos(2 t) \; \; \; ,$$
   und damit:
   $$\varphi (t) = \Mdfrac{\pi}{6}\cdot\cos(2 t)$$.
 ...
 Tangensfunktion $D_{\tan} = \R \setminus \{ \Mtfrac{k}{2} \cdot \pi; k \in \Z \}$.
 ...
 der Tangensfunktion
 $$\begin{array}{rrcl}
  \tan : & \R \setminus \{ \Mtfrac{k}{2} \cdot \pi; k \in \Z \} & \longrightarrow & \R \\
 ...
 $\cot (\alpha) = \frac1{\tan(\alpha)}= \Mdfrac{\cos(\alpha)}{\sin(\alpha)} \; \; \; .$
  ...
  \begin{MHint}{Lösung}
   ...
   $$D_{\cot} = \R \setminus \{ k \cdot \pi ; k \in \Z \}$$
...
\MLabel{M06_Abschnitt9}\label{eigenschaften}

\begin{MIntro}
\MDeclareSiteUXID{VBKM06_Eigenschaften_Intro}
In diesem Abschnitt werden wir eine weitere Eigenschaften elementarer Funktionen betrachten, die in den vorhergehenden Abschnitten noch nicht behandelt wurde.
...
\begin{itemize}
 ...
 und
 \[
  \function{f_2}{\R}{\R}{x}{|x|},
 \]
 also die Standardparabel (vgl.~Abschnitt \MNRef{sec:monome}) und die Betragsfunktion (vgl.~Abschnitt \MNRef{sec:betrag}), sind Beispiele für gerade Funktionen. Da gilt $f_1(-x)=(-x)^2=x^2=f_1(x)$ und $f_2(-x)=|-x|=|x|=f_2(x)$ für alle $x\in\R$. Die Graphen weisen die Spiegelsymmetrie an der Hochachse auf:
...
\begin{itemize}
 \item[a)] \[\function{f}{\R}{\R}{x}{e^x}\]
 \item[b)] \[\function{g}{\R}{\R}{y}{\sin(y)}\]
 \item[c)] \[\function{h}{(-\frac{\pi}{2},\frac{\pi}{2})}{\R}{\alpha}{\tan(\alpha)}\]
...
\begin{itemize}
 ...
 \]
 ist die \modsemph{Summe} aus der Identität, (vgl.~Abschnitt \MNRef{sec:linear}), und der Sinusfunktion (vgl.~Abschnitt \MNRef{sec:sinus}). Sie besitzt den folgenden Graphen:
...
\end{tikzpicture} 
 ...
 \item Die Funktion
 \[
  \function{g}{[1,2]}{\R}{y}{y^2-\ln(y)}
 \]
 ist die \modsemph{Differenz} aus der Standardparabel (vgl.~Abschnitt \MNRef{sec:monome}) und der natürlichen Logarithmusfunktion (vgl.~Abschnitt \MNRef{M06_Abschnitt7}). Sie besitzt den folgenden Graphen:
  %GRAPH: summ2
\MTikzAuto{%
\begin{tikzpicture}
%Koordinatensystem
...
\end{tikzpicture} 
 ...
 \item Die Funktion 
 \[
  \function{h}{(0,\infty)}{\R}{x}{e^x\frac{1}{x}}
 \]
 ist das \modsemph{Produkt} aus der natürlichen Exponentialfunktion mit Abbildungsvorschrift $e^x$ (und der Hyperbel mit Abbildungvorschrift $\frac{1}{x}$ (vgl.~Abschnitt \MNRef{sec:hyperbel}). Sie besitzt folgenden Graphen:
...
\end{tikzpicture} 
 ...
 \item Die Funktion
 \[
  \function{\varphi}{\R}{\R}{z}{\frac{cos(z)}{z^2+1}}
 \]
...
\draw (yMAX) node[anchor=east] {$\varphi(z)$};
%Graph
\draw[color=red,samples=100,domain=-7:7] plot(\x,{cos(\x r)/(\x*\x + 1)}); 
\draw[color=red] (1,0.7) node[anchor=south] {$G_{\varphi}$};
\end{tikzpicture}
}%
\end{itemize}
...
\begin{itemize}
 ...
 \[
  (f\circ g)(x)=f(g(x))=f(e^x)=(e^x)^2+1=e^{2x}+1,
 \]
 also
 \[
  \function{f\circ g}{\R}{\R}{x}{e^{2x}+1}
 \]
 und
 \[
  (g\circ f)(x)=g(f(x))=g(x^2+1)=e^{x^2+1},
 \]
 also
 \[
  \function{g\circ f}{\R}{\R}{x}{e^{x^2+1}.}
 \]
...
\end{tikzpicture}
 ...
 \[
  (w\circ h)(x)=w(h(x))=w(\sin(x))=\sqrt{\sin(x)}.
 \]
 Da die Funktionswerte des Sinus aber auch negativ werden können, man aber in die Quadratwurzelfunktion nur nicht-negative Werte einsetzen darf, muss also der Definitionsbereich der Sinusfunktion entsprechend eingeschränkt werden, so dass sich nicht-negative Werte ergeben, zum Beispiel mittels $x\in[0,\pi]=D_{w\circ h}$. Wir erhalten also
 \[
  \function{w\circ h}{[0,\pi]}{\R}{x}{\sqrt{\sin(x)}.}
 \]
\end{itemize}
\end{MExample}
...
\]
\begin{itemize}
 \item \MCheckbox{0}{EF1}... kann man kürzer auch als $f(x)=2x+1$ schreiben. 
 \item \MCheckbox{1}{EF2}... ist eine linear-affine Funktion.
 \item \MCheckbox{0}{EF3}... hat die Wertemenge $\R$.
 \item \MCheckbox{1}{EF4}... hat die Steigung $2$.
 \item \MCheckbox{1}{EF5}... kann nur Werte größer oder gleich $1$ und kleiner $7$ annehmen.
 \item \MCheckbox{1}{EF6}... hat als Graph ein Stück einer Gerade.
 \item \MCheckbox{1}{EF7}... hat bei $x=0$ den Wert $1$.
 \item \MCheckbox{0}{EF8}... hat die Definitionsmenge $\R$.
\end{itemize}
\end{MExercise}
