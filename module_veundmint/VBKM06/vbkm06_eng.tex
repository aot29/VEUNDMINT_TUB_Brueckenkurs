...
For doing and applying mathematics, it is not sufficient to consider only sets as well as 
equations and inequalities for numbers of these sets, as done in the previous modules (for example, 
in \MSRef{VBKM01}{Module 1}). Additionally, we need so called \highlight{functions} 
(which are often also denoted as \modsemph{maps}).

\begin{MInfo}
\MLabel{VBKM06_Abbildungen}
\MEntry{Functions}{function} (or \MEntry{maps}{map}) are assignments between elements of two sets.
\end{MInfo}
\end{MIntro}
...
mathematical term of a function as a tool for the formalisation of relations between 
dependant quantities is. Finally, we will study the graphical representation of functions 
...
purpose, we consider the set of natural numbers $\N$ as well as the set of rational numbers
...
Now, we like to do an assignment between the elements of these two sets as follows. 
To each number $n\in\N$ half of this number $\frac{n}{2}\in\Q$ is assigned, i.e. 
to the number $1\in\N$ the number $\frac{1}{2}\in\Q$ is assigned, to the number 
$2\in\N$ the number $1\in\Q$ is assigned, etc. In the figure below, this 
fact is illustrated by arrows that indicate which numbers in $\Q$ are assigned to
which numbers in $\N$.
...
We use for the \modsemph{assignment of single elements} of the sets, described above in words,
the so called \highlight{assignment arrow}. This is an arrow that has a bar at its tail: $\longmapsto$.
...
and \modsemph{to which} the assignment shall be done must be noted. In our case, to the elements of the set of natural 
...
\]
This reads as ``$f$ of $n$ equals $\frac{n}{2}$''. Altogether, we can thus describe the function $f$
...
Throughout this module, we will continue to use this summarising notation for functions. 
...
since $8$ and $1$ do not belong to the set $(0\MIntvlSep 1)$. 
 %Dies wird unten im Rest dieses Abschnitts nochmal genauer erörtert werden. 
\end{itemize}
...
\begin{MHint}{Solution}
 \begin{itemize}
  ...
  Furthermore, the observation $h(\frac{1}{2})=\frac{1}{\frac{1}{2}}=2$ results in $\frac{1}{2}\longmapsto2$.
...
The examples above show some basic properties of functions for which we will now introduce specific terms.
\begin{MInfo}
For the definition of a function, a set of numbers is specified whose elements are to be assigned to 
other numbers by the function. This set is called \MEntry{domain}{domain} or set of definition of the function.
...
\]
in Exercise~\MNRef{ex1:grundlagen} is the set $D_h=(0\MIntvlSep \infty)$. For the elements of the 
domain, a specific term also exists. In this exercise, the numbers $x\in D_h$ are assigned by 
...
be used as the domain of a function with known mapping rule.
...
For the definition of a function, beside the domain another second set is required, namely the set that
...
between domain and target set of a function. The domain contains all numbers, and only those numbers, that are allowed 
...
the results. In this way, a so called \MEntry{lookup table}{lookup table} is obtained:
...
Such lookup tables are useful to get an overview of the values of a function. However, from a 
...
For example, the volume $V$ of a cube depends on the side length $a$ of the cube. Then, the volume 
...
a function of time in physics, the reaction velocity as a function of concentration in chemistry, the amount 
of flour as a function of the desired amount of dough in a cake recipe, etc. 
...
source. This is also called \modsemph{inverse square law}. Using a physical \highlight{proportionality factor} $c>0$,
...
>From the construction of wind turbines it is known that the wind turbine power is \highlight{proportional} to the 
third power of the wind velocity. Under the condition that the proportionality factor satisfies the relation $\rho>0$, 
...
letters are \modsemph{generally} used to denote certain quantities. So, here it is more common to denote the velocity by $v$ 
(from the English word \modsemph{velocity}) and the power by $P$ (from the English word \modsemph{power}).
\end{MHint}
...
Now, from the graph many basic properties of the function can be seen. Recall the function 
\[
  \function{\Mvarphi}{(0\MIntvlSep 1)}{\R}{y}{3y+1}
\]
...
appear on the horizontal and on the vertical axis, respectively:
...
\[
 \function{f}{\R}{\R}{x}{x^2\MDFPSpace,} 
\]
indicate domain and range on the the horizontal axis and on the vertical axis and specify them.
...
can hardly ever be a graph of a function. For a single $x$-value in the domain, several values 
$y_1,y_2,y_3$  had to exist in the range. Graphs of functions indicate 
the uniqueness by the fact that they ``cannot reverse in horizontal direction''.
...
observe the contrary behaviour. For increasing $x$-values the corresponding function values also increase. 
The graph \modsemph{rises}. At the $x$-value $0\in\R$ the region of decreasing growing behaviour merges into 
...
\begin{MInfo}
\begin{itemize}
 \item For $x_1<x_2$ in a subset of the domain of a function $f$, we have $f(x_1)>f(x_2)$. Then the function $f$ is 
...
the case of the standard parabola. However, also functions do exist that have only one of 
the monotonicity properties for the entire domain (see Example~\MNRef{bsp1:monoton} below). In this 
case the entire function is called strictly increasing or strictly decreasing. Furthermore, 
a function that is either strictly decreasing or strictly increasing is simply called 
\highlight{strictly monotonic}.

A further example shows how strictly monotonicity can be checked explicitly for a function by solving 
...
\]
be given. Determine whether the function $h$ is either strictly increasing or 
strictly decreasing?

First, we chose two arbitrary numbers $x_1,x_2\in D_h=\R$ for which
\[
 x_1<x_2 \MDFPeriod
\]
...
functions as well as monomials and polynomials.
\end{MIntro}
...
So called constant functions assign to each number in the domain $\R$ \modsemph{exactly one} 
...
The identity function on $\R$ is the function that assigns to each real number exactly the identical 
real number. This is written as follows:
\[
...
of real numbers ($W_{\Id}=\R$). Furthermore, the identity function is obliviously a strictly 
increasing function.
\end{MXContent}
...
Starting from the identity function, more complex functions, so called \highlight{linear functions},
can be constructed. So, for example, one can think of a function that assigns to each 
...
Hence, all linear functions (except for the zero function, see below) have also the entire set 
...
\begin{MXContent}{Linear Affine Functions}{Affin}{STD}
...
\begin{MXContent}{Absolute Value Functions}{Betrag}{STD}
...
the following \MEntry{absolute value function}{absolute value function}
\[
...
\]
the absolute value function is an example for a \modsemph{piecewise defined} function. If absolute values are 
...
a function involving an absolute value. More complicated examples of functions can be constructed, involving one absolute 
value or several absolute values, e.g.
\[
 \function{f}{\R}{\R}{x}{|2x-1| \MDFPeriod}
\]

For such functions, it is a relevant task to get an idea of how the graph of the function looks like. 
To this, the piecewise definition of the absolute value is used, and the approach is similar to 
...
Let the function
\[\function{f}{\R}{\R}{x}{|2x-1|}\]
be given. How does the graph look like?
...
\begin{enumerate}
 ...
 \item For the case $<0$ the \modsemph{entire} expression gets a minus sign. Here, care has to be taken that
  ...
  $-2x-1$. Also this is always that way if absolute values are resolved.
\end{enumerate}
\end{MInfo}
...
Beside the linear affine functions studied in the previous section one now can 
also think of functions that assign to all real numbers non-negative integer powers 
of itself. An example is the function
\[
 \function{g}{\R}{\R}{x}{x^2 \MDFPeriod}
\]
...
\end{MHint}
\end{MExercise}
The monomial of degree $2$ is also called \highlight{standard parabola}. The monomial 
of degree $3$ is also called \highlight{cubic standard parabola}. The figure below shows 
...
\begin{MExercise}
How this conclusions concerning monomials can be seen immediately from the exponent rules?

\begin{MHint}{Solution}

>From the exponent rules, we know that $1^n=1$ and $0^n=0$ for arbitrary non-negative 
integers $n$, and we have $(-1)^n=1$  if $n$ is an even number as well as $(-1)^n=-1$ if $n$ is an odd number. 
>From this, the described conclusions result, which all monomials have in common. Also, the inequalities
...
While the monomials considered until now always involved only exactly one power of the 
independent variable, from these monomials more complex functions can be constructed 
easily involving several different powers of the independent variable. These are sums 
of multiples of monomials. They are then called \highlight{polynomials}. 
...
can be found by solving equations of $n$th degree. In case of degree $n=2$, i.e. for polynomials 
of degree $2$ (which are also called general parabolas),  this is possible by solving a 
...
\]
be given. We find the roots and the vertex and finally sketch the graph.
...
We consider the functions that have a \highlight{reciprocal relation} in their mapping rule, i.e. in the
mapping rule \highlight{reciprocals} occur. For the determination of the \modsemph{maximum domain} of such a function,
it has to be noted that the denominator must be non-zero. 
...
\]
is called \highlight{hyperbola}.

Generally, for the reciprocal of an arbitrary monomial of degree $n\in\N$
also a corresponding function of hyperbolic type can be specified.
\[
...
does never contain $0\in\R$. Since $x^n\geq0$ for even $n\in\N$, we have $\frac{1}{x^n}>0$  for even $n\in\N$. 
...
General rational functions have mapping rules that consists of a quotient of two polynomials. 
Some examples with their graphs are given below. Of course, also for this functions numbers must be 
excluded from the domain for which the denominator in the mapping rule equals zero.
...
\]
be given. Find the maximum domain $D_\psi\subset\R$ of $\psi$.
...
\begin{MHint}{Solution}
The function $f$ has the numerator's degree $0$ and the denominator's degree $2$. The numerator does not have a root ($8\neq0$) as well as the denominator ($x^2+1=0$ has no solution).\\
The function $g$ has the numerator's degree $1$ and the denominator's degree $2$. The root of the numerator is at $x=\frac{1}{6}$ ($-18x+3=0\Leftrightarrow x=\frac{3}{18}$), and the roots of the denominator at $x_{1,2}=-4,\frac{3}{2}$ are obtained by solving the quadratic equation $2x^2+5x-12=0$, e.g. by means of the quadratic formula.\\
The function $h$ has the numerator's degree $3$ and the denominator's degree $1$. The root of the denominator is simply at 
...
$\Delta=1^2-4\cdot1\cdot1=-3$ is negative such that no other real solution of the equation -- and hence 
...
the domain often have to be investigated further. Particularly, it is of interest how the graphs of functions 
...
\modsemph{(proper) poles}, and at $x=1$ the function $f_3$ has a so called \modsemph{removeable singularity}.
Looking at the graphs, the difference between these types of poles becomes clear. For (proper) poles,
the graph rises or falls unboundedly in the neighbourhood of the pole, and for continuously removeable 
...
any numerator's roots at all. In such cases, the roots of the denominator are always (proper) poles.
...
out. For rational functions that have a removeable singularity, this is always possible.
...
\]
and thus, the function has a continuously removeable singularity at $x=-2$ and a (proper) pole with 
...
the \modsemph{degree of the polynomial in the numerator is less or equal to the degree of the 
polynomial in the denominator}. An example of that case is the function
\[
...
denominator. In general tendency, for these values of $x$, only the polynomial part $1$ 
remains of the mapping rule. This part can be described by a -- in this case constant --
function that is called an \highlight{asymptote} $g_{As}$ of the function $g$:
\[
 \function{g_{As}}{\R}{\R}{x}{1 \MDFPeriod}
\]
Since this is a constant function in this case, it is also called 
...
\]
has also an asymptote. We can write the mapping rule as 
\[
...
with $k<0$. In this section, we will allow arbitrary rational numbers to be the exponent in the mapping rule.
This results in so called \modsemph{power functions} that again include monomials and functions of
hyperbolic type as special cases. Also, we will collect their fundamental properties and give some applications.
\end{MIntro}
...
\begin{MExercise}
What is the functions $f_n$ with $n=1$?
...
\[
 \function{f_n}{D_{f_n}}{\R}{x}{\sqrt[n]{x}}
\]
with $n\in\N$, $n>1$, find the range $W_{f_n}$ depending on the fact whether $n$ is even or odd.
...
we have $\sqrt[3]{x}<0$, $\sqrt[5]{x}<0$ if and only if $x<0$. In summary, we then have, regarding the strictly 
...
does not occur in the \modstextbf{basis} of the exponential term but in the \modstextbf{exponent}.
Accordingly, we will consider mapping rules as, for example, 
...
biological growth processes -- including diverse population models --, for the processes of radioactive decay, or 
for a certain kind of interest calculation. Let us consider an example.

\begin{MExample}
\MLabel{M06_bsp_bak_pop}

A bacterial culture starts with $500$ bacteria and doubles in size every $13$ minutes. We like to know: How many
bacteria will be in the culture after $1$ hour and $15$ minutes (i.e. after $75$ minutes)?

As a first try we can create a simple lookup table that lists the number of bacteria in the population at 
...
for the exact specification we need the mapping rule underlying the graph, which we will first only state here:
$$p: [0\MIntvlSep \infty) \longrightarrow (0\MIntvlSep  \infty) \;\text{with}\; t \longmapsto p(t) = 500 \cdot 2^{(t/13)} \MDFPeriod$$
(In Exercise~\MRef{M06_bsp_bak_pop_exercise} we will reason this functional relation.)\\
Thus, for $t = 75$ (measured in minutes) we obtain the function value
...
\begin{MHint}{Solution}
  An exponential function shall be defined not only for certain, specific, or isolated values of the 
  ...
  a^{1/12}$, etc. For example, square roots of negative numbers are not defined, see also 
...
\end{center}
\begin{itemize}
 \item All these exponential functions pass trough the point $(x = 0, y = 1)$: This is true since 
 ...
 \item If $a > 1$, then the graph of $g$ rises from left to right (i.e. for increasing $x$-values); 
  ...
  the steeper the graph of $g$ rises for \modstextbf{positive} values of $x$. Towards ever larger 
 ...
 \item If $a < 1$, then the graph of $g$ falls from left to right (i.e. for increasing $x$-values);
  ...
  the slower the graph of $g$ falls for \modstextbf{negative} values of $x$. Towards ever larger 
  positive values of $x$ (i.e. approaching from left to right) the positive $x$-axis is an asymptote 
  of the graph.
\end{itemize}
And what is it about the parameters $f_0$ and $\lambda$? The parameter $f_0$ is easily explained: 
...
determines how strong the exponential function -- for one and the same base -- increases (for $\lambda > 0$) 
...
\MDeclareSiteUXID{VBKM06_Exponentialfunktionen_efkt}
  ...
  \end{array}
  $$
  that is because of its base $\MEU$ also called \MEntry{$\MEU$ function}{e function} or 
  \MEntry{natural exponential function}{exponential function (natural)}:
  \begin{center}
  \MUGraphicsSolo{e_fkt.png}{scale=1}{width:400px}
  \end{center}
  Unsurprisingly, also the natural exponential function shows the typical behaviour of exponential 
  ...
  At the beginning of this subsection we have claimed that the exponential functions described above 
 ...
 \begin{MExercise}
  Give reasons why the identity $a^x = \MEU^{x \cdot \ln(a)}$ is valid. 
  \begin{MHint}{Solution}
 ...
 \end{MExercise}
  ...
  parameter $\lambda$ to be negative since the exponential function shall describe a decay
  process, i.e. a process with a negative growth rate. In the sequel, we will determine $\lambda$
...
In Section~\MRef{M06_e_fkt} we studied the natural exponential function
 ...
 \end{array}
 $$
In particular, we pointed out there a very important property of the natural exponential function, namely
that this function is strictly increasing. If the graph of this function is reflected over the angle 
...
Besides the natural logarithmic function other logarithmic functions exist that each corresponds to a certain 
exponent.
...
$$
is called the \MEntry{general logarithmic function}{logarithmic function (general)} to the base $b$.
\end{MInfo}
...
$$
as well as for the general logarithmic function
$$
...
For the logarithmic function several calculation rules exist that will be explained in the next section.
\end{MXContent}
...
the \MEntry{trigonometric functions}{trigonometric functions}, as \modstextbf{sine function}, \modstextbf{cosine function} 
and \modstextbf{tangent function}, play a central role. 

However, the field of applications of sine, cosine, tangent and et cetera is not restricted 
to ``simple'' triangle calculations. In fact, the trigonometric functions develop their real potential 
only in the manifold fields of applications. The most relevant of those are probably in the description of 
...
In Module~\MRef{VBKM05} the trigonometric functions were introduced elementary in Section~\MRef{M05_Trigonometrie}
on right triangles, for example by the relation
$$\begin{array}{rcccccl}
  \sin(\alpha) & = & \Mdfrac{\text{opposite side}}{\text{hypotenuse}\MDFPSpace ,} & & \\[2ex]
\end{array}$$
and explained at the \MSRef{VBKM05_Trigonometrie_Einheitskreis}{unit circle}. Starting from this 
...
$0$ again. At $\alpha = 360^\circ$ the game is starting again.
 \begin{center}
...
The figure above shows the graph of the sine function
 ...
 \end{itemize}
  ...
  Just a short look at the graph of the simple sine function suggests to use this function for the description 
  of wave phenomena. However, to be able to use up the full potential of the sine function a few parameters will be 
  introduced previously. For example, the amplitude of the sine function can be amplified or damped by a 
...
\MDeclareSiteUXID{VBKM06_TrigonometrischeFunktionen_CosTan}
 ...
 \end{center}
  The figure above shows, besides the graph of the cosine (solid line), once again the graph 
  of the sine function (dotted line) for comparison. We see a very strong relationship that we will
  discuss later.
 ...
 \begin{itemize}
  ...
  for all real values of $\alpha$. Likewise true, but not as obvious, is the relation
    $$\cos(\alpha) = - \sin \left( \alpha - \Mdfrac{\pi}{2} \right) \MDFPeriod$$
 \end{itemize}
 \begin{MExercise}
  \MLabel{M06_cos_diskussion}
  At which points the cosine function takes its maximum value $1$, and at which points it takes its maximum negative value $- 1$?
  What are its roots (at which points the function equals $0$)?
  \begin{MHint}{Solution}
...
to different situations (in application examples):
 ...
 $$
 \begin{MExercise}
  In Example~\MRef{M06_Fadenpendel} we have shortly discussed the simple pendulum. In particular, 
  ...
  and if so, which form has $\Mvarphi (t)$ in this case?
...
\begin{MHint}{Solution}
 ...
 \begin{MExercise}
 The so called cotangent function (abbreviated by $\cot$) is defined by
 $\cot (\alpha) = \frac1{\tan(\alpha)}= \Mdfrac{\cos(\alpha)}{\sin(\alpha)}$.
  ...
  And this is the case if $\alpha$ is an integer multiple of $\pi$. Thus, we have to exclude these points 
...
discussed in previous sections yet. This concerns the symmetry of functions. Furthermore, 
we will investigate how to construct new functions from known elementary functions. For this
purpose, among others, sums, products and compositions of functions are introduced.
\end{MIntro}
...
These two symmetry conditions for functions allow conclusions on the behaviour of the graphs. 
...
\end{tikzpicture}  
 ...
 \]
  i.e. the cubic parabola (see Section~\MNRef{sec:monome}), is an example for an odd function. We have 
...
rule. The example below lists again a few of such combined functions.
...
\end{tikzpicture}
 ...
 \]
  ...
  like to consider the composed function $w\circ h$, then we have
 \[
...
Restrict the domains, if necessary, such that the composition is allowed. However, use for the composed function 
always the maximum domain.
