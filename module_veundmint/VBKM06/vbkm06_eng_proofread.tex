...
to the number $1\in\N$ assign the number $\frac{1}{2}\in\Q$, to the number 
$2\in\N$ assign the number $1\in\Q$, etc. In the figure below, this 
...
Starting from the identity function, more complex functions - so called \highlight{linear functions} - can be constructed. So, for example, one can think of a function that assigns to each 
...
As well as the linear affine functions studied in the previous section one now can 
...
the graph rises or falls unboundedly in the neighbourhood of the pole, and for continuously removable 
