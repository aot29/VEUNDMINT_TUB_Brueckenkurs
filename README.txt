...
Folgend findet sich die Anleitung zur Einrichtung einer lokalen Kopie des VE&MINT-Onlinebrückenkurses. Zum Ausführen wird **Python3** benötigt.
...
`virtualenv -p python3 venv` aus. Dadurch wird eine Ordner `venv` im Verzeichnis erstellt in den alle python3 Libraries installiert werden können.
...
php-cli (php-cli ist nötig, um die dynamischen stylesheets zu bauen. 
Um den Feedbackserver zu benutzen, soll ausserdem php in vollem Umfang installiert sein).
