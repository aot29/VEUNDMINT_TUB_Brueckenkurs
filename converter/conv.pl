...
our $dopdf = 1;     # Sollen PDFs automatisch verlinkt werden? (betrifft hier nur die Verlinkungsmechanismen in split.pm)
...
our $outputfolder = "../out/tmp/";   # in mconvert.pl noch fest verdrahtet!
...
for ($i=0; $i<=$#ARGV; $i++) {
  print("Verarbeite Parameter " . $ARGV[$i] . ":\n");
  if ($ARGV[$i] eq "-nopdf") {
    print "PDF-Verlinkung wird deaktiviert\n";
    $dopdf = 0;
  }
...
logtext("Starte Konvertierung am " . ($year+1900 ) . "-" . ($mon+1) . "-$mday um $hour:$min:$sec Uhr");

#Alte Daten loeschen
print "Alte Daten loeschen und Standard-Dateien kopieren . . .";
system "rm -rf $outputfolder";
#system "rm -R $outputfolder/xml";
#system "rm -R $outputfolder/mpl";
#Ordner neu erstellen
system "mkdir -p $outputfolder";
system "cp -R files/* $outputfolder";
print "ok.\n\n";


# DirectHTML-Datei einlesen
my $direct_all = read_file('directhtml.txt') or die "Fehler beim Lesen der DirectHTML-Datei.\n";
my $k = 0;
...
$mints_open = open(MINTS, "> $outputfolder/convinfo.js") or die "Fehler beim Erstellen der Datei convinfo.js.\n";
...
if ($text =~ s/<!-- mmainlogo;;(.*?);; \/\/-->//s ) {
  ...
  $logofile = "";
  print "Keine Logo-Datei angegeben, kein Logo sichtbar\n";
}
...
if ($dofeedback eq 1) { print "----------------------- FEEDBACKVERSION WIRD ERZEUGT ---------------------------------\n";  }
...
if ($text =~ s/<!-- headertext;(.*?);(.*?);(.*?);(.*?);(.*?);(.*?); \/\/-->//s ) {
  ...
  print "Kein Header Pragma gefunden, verwende:\n";
}
...
if ($verde) {
	languagede();
	storelabels($root);
	printpages($root, "$outputfolder" . ($veren ? "/de" : ""));
}
if ($veren) {
	languageen();
	storelabels($rooten);
	printpages($rooten, "$outputfolder" . ($verde ? "/en" : ""));
}
...
if ($config{docollections} eq 1) {
  print "Exportfile fuer die Collections wird erzeugt: ";
  my $nco = $#colexports + 1;
  if ($nco le 0) {
    print "Keine Exports gefunden!\n";
  } else {
    print("$nco Collections werden exportiert\n");
    my $colexportfile = open(MINTS, "> collectionexport.json") or die "ERROR: Fehler beim Schreiben von collectionexport.json";
...
#Rechte setzen
system "chmod -R 777 $outputfolder";

#Rechenzeit anzeigen
$time2=time;
$diff = $time2-$time;
print "\nFertig.\nRechenzeit: $diff Sekunden.\n\n";
logtext("\nRechenzeit: $diff Sekunden");

if ($#converrors ge 0) {
  print "----------------------- Schwerwiegende Konvertierungsfehler aufgetreten ---------------------------------\n";
  my $yi = 0;
  ...
  print "Keine schwerwiegenden Konvertierungsfehler aufgetreten.\n";
}
