% Version 2.9 (Abschlussstand vor Konverterumstellung 03/2014) von Daniel Haase, MINT-Kolleg BW
...
\ifttm
\else
% \usepackage{pdfanim} % Fuer MATLAB-Modul Info1314
%     \PDFAnimLoad[width=.8\linewidth,loop,remember]{Informatik_Matlab_Animation}{Info1314_Matlab/Informatik_Matlab_Animation}{100}
%     \PDFAnimLoad[width=.5\linewidth,loop,remember]{Informatik_Matlab_Aufg_Astroide_Animation}{Info1314_Matlab/Informatik_Matlab_Aufg_Astroide_Animation}{100}
%\ifpdfoutput{\DeclareGraphicsExtensions{.pdf,.png,.jpg}}{}\fi
\fi
...
% Diese Markierung wird im postprocessing (split.pm) durch den physikalischen Dateinamen ersetzt, aber nur den Namen (d.h. \MMaterial gehoert noch davor, vgl Definition von MGraphics)
% Parameter: Pfad/Name der Datei bzw. des Ordners, relativ zur Position des Modul-Tex-Dokuments.
% Fuer png-Dateien wird in split.pm im postprocessing der base64-Inlinestring erzeugt
\ifttm
...
\ifttm
\newcommand{\MGlobalConfTag}{\special{html:<!-- mglobalconftag -->}}
\else
...
\ifttm
\newcommand{\MGlobalFavoTag}{\special{html:<!-- mglobalfavotag -->}}
\else
\newcommand{\MGlobalFavoTag}{}
\fi

% Makro, um in der HTML-Ausgabe die Betaseite zu kennzeichnen
\ifttm
\newcommand{\MGlobalBetaTag}{\special{html:<!-- mglobalbetatag -->}}
\else
\newcommand{\MGlobalBetaTag}{}
\fi
...
\ifttm%
  ...
  \newenvironment{MSolution}{\special{html:<!-- loesung -->}}{\special{html:<!-- endloesung -->}}

  \newenvironment{MHint}[1]{  \special{html:<button name="Name_MHint}\arabic{MHintCounter}\special{html:" id="MHintButton}\arabic{MHintCounter}\special{html:" %
  ...
  \newenvironment{MCOSHZusatz}{  \special{html:<button name="Name_MHint}\arabic{MHintCounter}\special{html:" id="MHintButton}\arabic{MHintCounter}\special{html:" %
  type="button" onclick="toggle_hint('MHint}\arabic{MHintCounter}\special{html:');">}Weiterführende Inhalte\special{html:</button>}
  \special{html:<div class="hintc" style="display:none" id="MHint}\arabic{MHintCounter}\special{html:"> }}{\begin{html}</div>\end{html}\addtocounter{MHintCounter}{1}}

  
  \newenvironment{MDefinition}{\begin{definition}\setcounter{MLastIndex}{\value{definition}}\ \\}{\end{definition}}

  
  % Darstellung wird dann aus dem CSS genommen
  ...
  \special{html:</div><div class="aufgabe" id="ADIV_}\MGenerateExNumber\special{html:">}%
  \textbf{Aufgabe \MGenerateExNumber
  } \ \\%
  ...
  % Imitiert die info-Umgebung aus dem Kasseler xmlbefehle-Package
  % Darstellung wird dann aus dem CSS genommen
  ...
  % Imitiert die info-Umgebung aus dem Kasseler cpxmlbefehle-Package
  % Darstellung wird dann aus dem CSS genommen
...
%  \arabic{MExperimentCounter}\setcounter{MLastIndex}{\value{MExperimentCounter}}
  ...
  % Imitiert die info-Umgebung aus dem Kasseler xmlbefehle-Package
  % Darstellung wird dann aus dem CSS genommen, es wird keine Nummer vergeben
...
\else%
  ...
  \newenvironment{MSolution}{\ \\ \underline{L\"osung:}\\}{}
  \newenvironment{MHint}[1]{\ \\ \underline{#1:}\\}{}
  \newenvironment{MCOSHZusatz}{\ \\ \underline{Weiterführende Inhalte:}\\}{}
...
\newenvironment{MSolutionHint}{\begin{MHint}{L"osung}}{\end{MHint}}
...
% Ein MLabel wird im html codiert durch das tag <!-- mmlabel;;Labelbezeichner;;SubjectArea;;section;;subsection;;Index;;Objekttyp; //-->
\def\MLabel#1{%
...
\label{#1}\begin{html}<!-- mmlabel;;#1;;\end{html}\arabic{MSubjectArea}\special{html:;;}\arabic{section}\special{html:;;}\arabic{subsection}\special{html:;;}\arabic{MLastIndex}\special{html:;;}\arabic{MLastType}\special{html:; //-->}}%
...
\newenvironment{MSContent}[3]{\special{html:<!-- scontent;-;}\arabic{MSCounter};-;#1;-;#2;-;#3\special{html: //-->}\MPageScripts\MSubsubsectionx{#1}}{\MEndScripts\special{html:<!-- endscontent;;}\arabic{MSCounter}\special{html: //-->}\addtocounter{MSCounter}{1}}
...
\newenvironment{MXContent}[3]{\special{html:<!-- xcontent;-;}\arabic{MXCounter};-;#1;-;#2;-;#3\special{html: //-->}\MPageScripts\MSubsubsection{#1}}{\MEndScripts\special{html:<!-- endxcontent;;}\arabic{MXCounter}\special{html: //-->}\addtocounter{MXCounter}{1}}
...
% #1 = Titel des Modulabschnitts, #2 = Kurztitel fuer die Buttons, #3 = Buttonkennung (STD = Defaultbutton, NONE = Ohne Button in der Navigation)
\newenvironment{MUContent}[3]{\special{html:<!-- xcontent;-;}\arabic{MXCounter};-;#1;-;#2;-;#3\special{html: //-->}\MPageScripts\MSubsubsectionx{#1}}{\MEndScripts\special{html:<!-- endxcontent;;}\arabic{MXCounter}\special{html: //-->}\addtocounter{MXCounter}{1}}
...
\ifttm
\newenvironment{MSectionStart}{\MODULGHEADER\MSubsubsectionx{Modul\"ubersicht}}{\MODULGFOOTER\setcounter{MSSEnd}{0}}
% Darf nicht als XContent nummeriert werden, darf nicht als XContent gelabelt werden
\else
\newenvironment{MSectionStart}{\MSubsectionx{Modul\"ubersicht}}{\setcounter{MSSEnd}{0}}
\fi

\newenvironment{MIntro}{\begin{MUContent}{Einf\"uhrung}{Einf\"uhrung}{genetisch}}{\end{MUContent}}
\newenvironment{MContent}{\begin{MUContent}{Inhalt}{Inhalt}{beweis}}{\end{MUContent}}
\newenvironment{MExercises}{\ifttm\else\clearpage\fi\begin{MUContent}{Aufgaben}{Aufgaben}{aufgb}\special{html:<!-- declareexcsymb //-->}}{\end{MUContent}}
...
\ \\ \ \\%
\MInTestFooter
\end{MUContent}\addtocounter{MTestSite}{-1}%
}

\newenvironment{MExtra}{\ifttm\else\clearpage\fi\begin{MUContent}{Zus\"atzliche Inhalte}{Zusatz}{weiterfhrg}}{\end{MUContent}}

\ifttm
\def\MTableOfContents{\special{html:<p><!-- toc //--></p>}}
\else
\def\MTableOfContents{\tableofcontents}
\fi
...
\def\MLastFileName{\special{html:<!-- mfilenameref;;}\arabic{MFileNumber}\special{html:; //-->}}
\def\MLastFilePath{\special{html:<!-- mfilepathref;;}\arabic{MFileNumber}\special{html:; //-->}}
...
<img src="\end{html}\MMaterial/\MLastFile\begin{html}" style="#3" alt="\end{html}\MMaterial/\MLastFile\begin{html}"/>
\end{html}%
...
\newcounter{MWTabCounter}

\ifttm

\newenvironment{MWTabular}[1]{
\special{html:<!-- wtab-start;;}\arabic{MWTabCounter}\special{html:;;}#1\special{html:; //-->}
\begin{tabular}{#1}
}{
\end{tabular}
\special{html:<!-- wtab-end;;}\arabic{MWTabCounter}\special{html:; //-->}
\addtocounter{MWTabCounter}{1}
}
...
% Optionale redefinition der headertexte (sollte nur in Hauptdatei verwendet werden)
\ifttm%
\newcommand{\MHeaders}[6]{\special{html:<!-- headertext;}#1\special{html:;}#2\special{html: ;}#3\special{html:;}#4\special{html:;}#5\special{html:;}#6\special{html:; //-->}}
\else%
\newcommand{\MHeaders}[6]{\relax}
\fi
...
\tikzset%
{
  % Defines a custom style which generates BOTH, .png and .pdf export
  png export/.style=%
  {
    external/system call/.add={}{; convert -density 300 -scale 37.5\% -transparent
    white "\image.pdf" "\image.png"; rm "\image.pdf"; rm "\image.log"; rm "\image.dpth"; rm "\image.idx"},
...
\MGraphicsSolo{\MSectionID\MTIKZautofilename.png}{scale=1}
}
\else%
...
% --------------------------------- Spezialbefehle aus dem Aufgabenpaket vom mkbw ---------------------
...
\newcommand{\msz}[3][\MZXYZhltrennzeichen]{\MZahl[#1]{#2}{#3}}
\newcommand{\mse}[2][\MEinheitenabstXYZnd]{\MEinheit[#1]{#2}}
\newcommand{\msx}[2][\MExponentensymbXYZl]{\MExponent[#1]{#2}}


% % ENTFERNT FUER TU9-Kurs
% % Das Editierfenster
% \ifttm%
% \newcommand{\MCodeEdit}[1]{%
% \MRegisterFile{../../files/ShowField.class}
% \special{html:
% <div id="editor">}#1\special{html:</div>
% <script>
%   var codeeditor}\arabic{MCodeEditCounter}\special{html: = ace.edit("editor");
%   codeeditor}\arabic{MCodeEditCounter}\special{html:.setTheme("ace/theme/eclipse");
%   codeeditor}\arabic{MCodeEditCounter}\special{html:.getSession().setMode("ace/mode/java");
% </script>
% <button name="Name_MCodeButton}\arabic{MCodeEditCounter}\special{html:" id="MCodeCheckButton}\arabic{MCodeEditCounter}\special{html:"
%   type="button" onclick="checkCESyntax(codeeditor}\arabic{MCodeEditCounter}\special{html:,document.JavaView}\arabic{MCodeEditCounter}\special{html:);">}Syntax prüfen\special{html:</button>
% <applet codebase="}\MMaterial\special{html:/}\MLastFilePath\special{html:" code="}\MLastFileName\special{html:" name="JavaView}\arabic{MCodeEditCounter}\special{html:" width="200" height="85">
% }
% \addtocounter{MCodeEditCounter}{1}
% }%
% \else%
% \newcommand{\MCodeEdit}[1]{}
% \fi
...
%Abkürzung für \longrightarrow:
\newcommand{\lto}{\ensuremath{\longrightarrow}}

%Makro für Funktionen:
...
%Zwei Betonungskommandos (können angepasst werden):
...
Schreiben Sie Malpunkte (geschrieben als \texttt{*}) aus und setzen Sie Klammern um Argumente für Funktionen.
...
\arabic{section}\special{html:;;}\arabic{subsection}\special{html:;;}\arabic{MEntryCounter}\special{html:; //-->}%
...
\begin{itemize}
\item{Im Gegensatz zu den offenen Aufgaben werden beim Eingeben keine Hinweise zur Formulierung der mathematischen Ausdrücke gegeben.}
\item{Der Test kann jederzeit neu gestartet oder verlassen werden.}
\item{Der Test kann durch die Buttons am Ende der Seite beendet und abgeschickt, oder zurückgesetzt werden.}
\item{Der Test kann mehrfach probiert werden. Für die Statistik zählt die zuletzt abgeschickte Version.}
\end{itemize}
}
\else
\newcommand{\MInTestHeader}{%
...
\begin{html}
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<button name="Name_TESTRESET" id="TESTRESET" type="button" onclick="reset_button();">Test zurücksetzen</button>
<br />
<br />
<div class="xreply">
...
% Wird beim Postprocessing durch grafische HTML-Modultiteldarstellung ersetzt
\ifttm
%\def\MODULGHEADER{\special{html:<!-- modulgheader //-->}}%
%\def\MODULGFOOTER{\special{html:<!-- modulgfooter //-->}}%
\def\MODULGHEADER{}%
\def\MODULGFOOTER{}%
\else%
%
...
% MSignature wird (nur im HTML) durch conv.pl gefunden und setzt die Signatur des Onlinepakets
\ifttm
\newcommand{\MSignature}[3]{\special{html:<!-- msignature;;}#1\special{html:;;}#2\special{html:;;}#3\special{html:;; //-->}}
\else
\newcommand{\MSignature}[3]{\relax}
% ggf. in PDF-Signatur einbauen?
\fi

\ifttm
\newcommand{\MReplyAdress}[1]{\special{html:<!-- mreplyadress;;}#1\special{html:;; //-->}}
\else
\newcommand{\MReplyAdress}[1]{\relax}
% ggf. in PDF-Signatur einbauen?
\fi

% MDeclareFeedbackServer wird (nur im HTML) durch conv.pl gefunden und setzt die Adresse des Feedback-Servers
% #1 = Vollstaendige Servicebezeichnung (sollte auf .php enden), #2 = Lesbare Serverbezeichnung fuer Benutzer
\ifttm
\newcommand{\MDeclareFeedbackServer}[2]{\special{html:<!-- mfeedbackserverdeclare;;}#1\special{html:;;}#2\special{html:;; //-->}}
\newcommand{\MDeclareDataServer}[1]{\special{html:<!-- mdataserverdeclare;;}#1\special{html:;; //-->}}
\newcommand{\MDeclareExerciseServer}[1]{\special{html:<!-- mexerciseserverdeclare;;}#1\special{html:;; //-->}}
\else
\newcommand{\MDeclareFeedbackServer}[2]{\relax}
\newcommand{\MDeclareDataServer}[1]{\relax}
\newcommand{\MDeclareExerciseServer}[1]{\relax}
\fi

\ifttm
\newcommand{\MMainLogo}[1]{\special{html:<!-- mmainlogo;;}#1\special{html:;; //-->}}%
\else
\newcommand{\MMainLogo}[1]{\relax}%
\fi

\ifttm
\newcommand{\MColorFile}[1]{\special{html:<!-- mcolorfile;;}#1\special{html:;; //-->}}%
\else
\newcommand{\MColorFile}[1]{\relax}%
\fi

\ifttm
\newcommand{\MDoFeedback}[1]{\special{html:<!-- mdofeedback;;}#1\special{html:;; //-->}}%
\else
\newcommand{\MDoFeedback}[1]{\relax}%
\fi

\ifttm
\newcommand{\MDoExport}[1]{\special{html:<!-- mdoexport;;}#1\special{html:;; //-->}}%
\else
\newcommand{\MDoExport}[1]{\relax}%
\fi
...
\MPragma{mintmodversion;3.1}
\MPragma{usercomment;This is file mintmod.tex version 3.1}
...
% ----------------------------------- Am Ende werden von mconvert.pl die lokalen Preambeln aus den Modulen angefuegt -------
