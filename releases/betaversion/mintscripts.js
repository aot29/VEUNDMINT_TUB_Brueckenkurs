...
function notationParser_IN(s) {
    ...
    // console.log("len: " + s.length + ", np_in in: " + s);
...
function CreateQuestionObj(uxid,c,solution,id,type,option,pnts,intest,section) {
  ...
  if ((type == 6) || (type == 7)) {
    ...
    try {
      ...
      console.log("Solution of type " + type + " not compileable: " + solution);
      ob.solcodea = mathJS.compile("0");
      ob.solcodeb = mathJS.compile("0");
    }
    ...
    try {
      ...
      // console.log("Successfully compiled " + solution);
    } catch(e) {
    console.log("Solution of type " + type + " not compileable: " + solution);
    ob.solcode = mathJS.compile("0");
    }
...
function checkSimplification(type, input) {
  ...
  if (( type & 256) == 256) {
    ...
    if ((input.indexOf("^") != -1) | (input.indexOf("/") != -1)) {
      ret.push(new Array("L&#246;sung darf keine Brüche oder Potenzen enthalten" , 1));
    }
  }
...
function handlerChange(id, nocontentcheck) {
  ...
  if (nocontentcheck != 1) {
      ...
      if (FVAR[id].points == FVAR[id].maxpoints) {
        if (intersiteactive == true) {
            ...
            st = "EXSUCCESS: " + "CID:" + signature_CID + ", user:" + currentUser + ", timestamp:" + timestamp + ", pagename:" + fullName + ", uxid:" + FVAR[id].uxid + ", elementid:" + id + ", input:" + FVAR[id].rawinput + ", message:" + FVAR[id].message;
  ...
  if (formula == 1) {
    ...
    if (u != null) {
      ...
      // console.log("UFIDM not available"); // passiert z.B. bei geschlossenen Tests staendig da Messagepart dann nicht angezeigt ist
    }
...
//    console.log("-------------------");
//    console.log("rawparse: input = " + eingabe);
   try {
     // console.log("---------------");
     // console.log("rawparse bekommt: " + eingabe);
     var retobj = mparser.convertMathInput(eingabe,2);
     var mjs = retobj.mathjs;
          // console.log("RawParse: mathjs = " + mjs);
         var z = mparser.evalMathJS(mjs);
          // console.log("RawPrase: eval = " + z);
	 return z;
   } catch(e) {
      // console.log("Exception at rawParse, raw = NaN");
...
function check_group(input_from, input_to) {
    ...
    for (i=input_from; i<=input_to; i++) {
              ...
              case 3: {
                ...
                if ((soluta.length == 1) && (solleer == 0)) {
                     ...
                     if ((tr.indexOf("{") != 0) | (tr.indexOf("}") != (tr.length-1))) {
                         ...
                         if (st.trim() == "") {
                             valuta = {};
                             // console.log("Benutzer hat leere Menge eingegeben");
                         } else {
                           valuta = st.split(",");
			   if (valuta.length==1) {
		...
		// console.log("Using " + varia.length + " variables");
		...
		for (vj=0; vj<varia.length; vj++) vv[vj] = first;
		  ...
		  // console.log("---------- Starting eval for vv = " + vv + " --------------------");
		  ...
		  while (fini == false) {
		    ...
		    for (vj=0; vj<varia.length; vj++) {
		      ...
		      // console.log("Variable " + varia[vj] + " is now " + vv[vj]);
		    }
		  ...
		  // console.log("Exception in eval", e);
		}
		...
		if ((typr == 0) || (typl == 0)) {
		  ...
		  console.log("Loesungsintervall " + FVAR[i].solution + " ist fehlerhaft");
		...
		switch(styp) {
		  ...
		  case "onlyempty": {
		    ...
		    break;
                  ...
                  case "vector": {
                    var n = rawParse(b);
                    // console.log("vector: rawparse liefert " + n);
                    ok = 1;
                    break;
                  }
		  ...
		  case "intervalelement": {
		    ...
		    var s = FVAR[i].solution;
                    // console.log("intervalelement: sol = " + s);
		    ...
		    if ((typr == 0) || (typl == 0)) {
		      console.log("Loesungsintervall " + FVAR[i].solution + " ist fehlerhaft (Aufgabe Typ 7)");
		    } else {
			...
			if (b.trim() == "") {
			  ...
			  ok = 0;
		        ...
		        if ((sl.indexOf(",") != -1) | (sl.indexOf("[") != -1) | (sl.indexOf("]") != -1)) {
                          ...
                          if ((typr == 1) && (h1 <= l)) ok = 0;
                          if ((typr == 2) && (h1 < l)) ok = 0;
                          // console.log("typr=" + typr + ", typl=" + typl + ", h0=" + h0 + ", h1=" + h1 + ", parsedinput = " + l);
                       }
 		      }	
		    }
		    ...
		    break;
                  ...
                  case "exactfraction": {
                    // Bruch muss zur Loesung äquivalent sein, sowie maximal gekürzt und mit positivem Nenner
                    ...
                    if (Math.abs(extround(l,stellen)-extround(sp,stellen)) <= Math.pow(10,(stellen+2)*(-1))) {
                        ...
                        ok = 0;
                        // console.log("Diff = " + Math.abs(extround(l,stellen)-extround(sp,stellen)) + ", stellen = " + stellen);
                    }
                    ...
                    if (b.indexOf("/") == -1) {
                        ...
                        if (fr.length != 2) {
                            ...
                            // console.log("fr[0] = " + fr[0] + ", fr[1] = " + fr[1]);
                            var rex = /(-?\d+)/ ;
                            var u;
                                ...
                                if (u[1] == fr[0]) {
                                  if ((u = rex.exec(fr[1])) != null) {
                                    if (u[1] == fr[1]) {
                                        ...
                                        var b = rawParse(fr[1]);
                                        var g = mygcd(a,b);
                                        if ((g != 1) || (b <= 0)) ok = 0; else {
                                            // console.log("Nicht gekuerzt oder Nenner<0");
                                        }
                                    } else {
                                      ok = 0;
                                      // console.log("Nenner nicht ganzzahlig");
                                    }
                                  } else {
                                    ok = 0;
                                    // console.log("Nenner nicht ganzzahlig!");
                                    ...
                                    ok = 0;
                                    // console.log("Zaehler nicht ganzzahlig, u = " + u + ", ft[0] = " + fr[0]);
                                }
                            } else {
                              ok = 0;
                              // console.log("Zaehler nicht ganzzahlig!");
                            }
                        }
                    }
                  ...
                  default: {
		    console.log("STYP " + styp + " nicht bekannt (MSpecialQuestion)");
		    ok = 0;
		    break;
		  }
		}
...
function GetResult(uxid)
{
  if (intersiteactive == true) {
	...
	// console.log("GetResult: " + uxid + " <-> " + intersiteobj.scores[j].uxid);
	if (intersiteobj.scores[j].uxid == uxid) {
	  // console.log("GetResult returns " + intersiteobj.scores[j].rawinput);
...
function InitResults(empty)
{
  ...
  // VERALTET!!
  // console.log("InitResults: intersiteactive=" + intersiteactive + ", empty=" + empty);
...
function finish_button(name) {
 ...
 var ratio = 100*nPoints/nMaxPoints;
  ...
  if (isTest == true) {
	  ...
	  pushISO();
	  var timestamp = +new Date();
	  var cm = "TESTFINISH: " + "CID:" + signature_CID + ", user:" + currentUser + ", timestamp:" + timestamp + ", testname:" + name + ", nPoints:" + nPoints + ", maxPoints:" + nMaxPoints + ", ratio:" + (nPoints/nMaxPoints) + ", nMinPoints:" + nMinPoints;
...
function notifyPoints(i, points, correct) {
      ...
      if ((intersiteobj.configuration.CF_LOCAL == "1") && (intersiteobj.configuration.CF_TESTS == "1")) {
          ...
          for (j = 0; j<intersiteobj.scores.length; j++) {
              if (intersiteobj.scores[j].uxid == FVAR[i].uxid) {
                  ...
                  // console.log("Points for " + SITE_UXID + "->" + FVAR[i].uxid + " modernized");
              }
          }
          if (f == false) {
            ...
            // console.log("Points for " + FVAR[i].uxid + " ADDED at position " + k);
          }
      }
  }
  ...
  if (img == null) {
      // console.log("img=0, type = " + FVAR[i].type);
  } else {
    switch(correct) {
...
//         var psres = pipwerks.SCORM.init();
//         console.log("SCORM init = " + psres);
//     } else console.log("SCORM already active");
//   } else console.log("SCORM OFF");
...
{
  setupJQuery();
  // console.log("ReadyHandler finished");
}
...
function closeInputContent() {
    ...
    if (activefieldid != "") {
	  ...
	  console.log("activefieldid ohne tooltip!");
	} else {
...
function debork(str, l) {
  ...
  // console.log("Debork: " + str + " -> " + str.slice(0,l));
