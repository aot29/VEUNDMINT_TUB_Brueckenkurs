...
our $macrofile = "$macrofilename.tex"; # Relativ zum src/tex Verzeichnis
...
sub minimizeJS {
  ...
  for ($borkka = 0; $borkka < $borknt; $borkka++) {
    ...
    if ($domini eq 1) { 
      $borkcall = "java -jar $basis/src/yuicompressor-2.4.8.jar $borkfilename -o $borkfilename";
...
sub create_tree {
  ...
  system("cp -R $basis/src $outputdir/.");
  system("cp -R " . $config{source} . "/* $outputdir/src/tex/.");
  ...
  my $call = "find -P " . $outputdir . "/src/tex/. -name \\*.tex";
  ...
  for ($ka = 0; $ka < $nt; $ka++) {
    ...
    while ($textex =~ m/\\MDia\{(.+?)\}/g ) {
      my $dprx = "$outputdir/src/tex/$prx/$1";
      if ($config{diaok} == "0") {
...
#       print "AFTER: \n$after\n\n";
    ...
    if ($dotikzfile eq 1) {
      ...
      system("cp $basis/src/tex/maxpage.sty .");
      system("cp $basis/src/tex/bibgerm.sty .");
  ...
  logMessage($VERBOSEINFO, "Copying configfile $mconfigfile to " . $outputdir . "/src/config.pl");
  system("cp " . $mconfigfile . " " . $outputdir . "/src/config.pl");

  # Create copyright text file
  my $copyrightfile = $outputdir . "/src/tex/copyrightcollection.tex";
  writefile($copyrightfile, "\%---------- Von mconvert.pl generierte Datei ------------\n" . $copyrightcollection);

  # Erstelle Datei mit DirectHTML-Texten
  my $directhtmlfile = "$outputdir/src/directhtml.txt";
  ...
  # Erstelle Datei mit DirectHTML-Texten
  my $directexercisesfile = "$outputdir/src/directexercises.tex";
  writefile($directexercisesfile, $globalexstring);

  # Create main file for converter build
  my $ttminputfile = "$outputdir/src/tex/htmlprepare.tex";
  ...
  my $pdfinputfile = "$outputdir/src/tex/targetpdf.tex";
  ...
  chdir("$ndir/src");
  logMessage($VERBOSEINFO, "Changing to directory $ndir/src");
  ...
  if ($config{dopdf} eq 1) {
    if ($pdfok == 1) {
      system("cp $ndir/src/tex/*.pdf $ndir/.");
  ...
  if ($config{doscorm} eq 1) { system("cp -R $ndir/src/SCORM2004v4/* $ndir/."); }


  if ($config{localjax} eq 1) {
    logMessage($CLIENTINFO, "MathJax 2.6 (full package) is added locally");
    system("mkdir $ndir/MathJax");
    system("tar -xzf $ndir/src/mathjax26complete.tgz --directory=$ndir/MathJax/.");
  }

  system("mv $ndir/src/" . $config{outtmp} . "/* $ndir/.");
  system("rmdir $ndir/src/" . $config{outtmp});

  if ($config{cleanup} == 0) {
    logMessage($CLIENTINFO, "CONVERTER-SUBDIRECTORY IS NOT BEING REMOVED");
  } else {
    system("rm -fr $ndir/src");
