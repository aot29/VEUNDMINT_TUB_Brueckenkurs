...
\ifttm
\relax% ttm can't handle DeclareGraphicsExtension, graphics includes will be done by preprocessing anyway
\else
...
\ifttm
\newcommand{\MGlobalLogoutTag}{\special{html:<!-- mgloballogouttag -->}}
\else
\newcommand{\MGlobalLogoutTag}{}
\fi

\ifttm
\newcommand{\MGlobalLoginTag}{\special{html:<!-- mgloballogintag -->}}
\else
\newcommand{\MGlobalLoginTag}{}
\fi
...
% Parameter: Laenge des Feldes, Loesungsfunktion, Anzahl Stuetzstellen, Funktionsvariable (nicht case-sensitive), Anzahl Nachkommastellen im Vergleich, Vereinfachungsbedingung, UXID
...
<center>
<div>
<video width="95\%" controls="controls"><source src="\end{html}\MMaterial/\MLastFile\begin{html}" type="video/mp4">Ihr Browser kann keine MP4-Videos abspielen!</video>
</div>
<div class="bildtext">
\end{html}
...
Formel zu klein? Mit einem Doppelklick auf eine Formel wird sie vergr\"o{\ss}ert dargestellt.}}
...
Schreiben Sie Malpunkte (geschrieben als \texttt{*}) aus und setzen Sie Klammern um Argumente f\"ur Funktionen.
...
\begin{itemize}
\item{Im Gegensatz zu den offenen Aufgaben werden beim Eingeben keine Hinweise zur Formulierung der mathematischen Ausdr\"ucke gegeben.}
\item{Der Test kann jederzeit neu gestartet oder verlassen werden.}
\item{Der Test kann durch die Buttons am Ende der Seite beendet und abgeschickt, oder zur\"uckgesetzt werden.}
\item{Der Test kann mehrfach probiert werden. F\"ur die Statistik z\"ahlt die zuletzt abgeschickte Version.}
\end{itemize}
}
\else
\newcommand{\MInTestHeader}{%
...
\special{html:<button name="Name_TESTFINISH" id="TESTFINISH" type="button" onclick="finish_button('}\MTestName\special{html:');" class="testsbutton">Test auswerten</button>}%
\begin{html}
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<button name="Name_TESTRESET" id="TESTRESET" type="button" onclick="reset_button();" class="testsbutton">Test l&\#246;schen und neu starten</button>
<br />
<br />
<div class="xreply">
...
\newcommand{\MOptionalEmph}[1]{#1} % optional aktivierbare Hervorhebung wichtiger Stellen im Text
...
\ifttm
\newcommand{\MDirectRouletteExercises}[2]{\relax} % wird durch den Konverter gefunden und ersetzt
\else
% #1 = Dateiname, #2 = eindeutige ID fuer das Roulette im Kurs
\newcommand{\MDirectRouletteExercises}[2]{
\begin{MExercise}
\texttt{In der Onlineversion erscheinen hier zufällige Aufgaben aus einer Aufgabenliste.}
\end{MExercise}
}
\fi
...
% ----------------------------------- Konverter fuegt automatisierte Makros am Schluss an ---------------------------
