...
";
char tth_USAGE[]="\n\
USAGE: tth [-a -c ...] [<]file.tex [>file.xml] [2>err]\n\
...
\\begin\{document\} { /* Check for aux file. If present input. */
  tth_LaTeX=1;
  if(tth_splitfile)strcpy(filechar,"index.xml"); /*sf*/
...
\\printindex { /* Check for file. If present put title and open */
  ...
  argchar[0]=0;
  if(tth_splitfile){ /*sf*/
    strcpy(filenext,"docindex.xml");/*sf*/
    ...
    *(chscratch+strlen(chscratch)-1)=0;
	...
	if(tth_splitfile==2 || *auxflch==0){ /*sf*/
	  ...
	  *(auxflch+strcspn(auxflch,"."))=0;  /*sf*/
	  strcat(auxflch,".xml"); /*sf*/
	} /*sf*/
      }else if(strstr(yytext,"{chapter}")!=NULL){ /*sf*/
	sprintf(auxflch,"chap%s.xml",chscratch);  /*sf*/
      }else if(strstr(yytext,"{table}")!=NULL){  /*sf*/
       if(*(auxflch)=='c')sprintf(auxflch,"chap%s.xml",tchar[tbno++]);/*sf*/
       else sprintf(auxflch,"sec%s.xml",tchar[tbno++]);/*sf*/
      }else if(strstr(yytext,"{figure}")!=NULL){  /*sf*/
       if(*(auxflch)=='c')sprintf(auxflch,"chap%s.xml",fchar[fgno++]);/*sf*/
       else sprintf(auxflch,"sec%s.xml",fchar[fgno++]);/*sf*/
      } /*sf*/
    } /*sf*/
...
/*      ind=ind+i+1; */
    ...
    *(dupstore+ind+js2)=0;
	...
	if(tth_splitfile)sprintf(dupstore2+strlen(dupstore2),/*sf*/
	   "\\special{html:<a href=\"refs.xml#%s\" id=\"CITE%s\" class=\"tth_citation\">}",/*sf*/
	...
	if(tth_splitfile)sprintf(dupstore2+strlen(dupstore2),/*sf*/
	   "\\special{html:<a href=\"refs.xml#%s\" class=\"tth_citeref\">}",/*sf*/
...
\\tth_thebibliography {
  ...
  if(tth_splitfile){ /*sf*/
    if(!bibliogs) strcpy(filenext,"refs.xml"); /*sf*/
    else sprintf(filenext,"refs%d.xml",bibliogs); /*sf*/
    bibliogs++; /*sf*/
    TTH_SCAN_STRING("\\tthsplittail\\tthsplitinv\\tthsplittop\\tthfileupd"); /*sf*/
  }/*sf*/
  TTH_SCAN_STRING("\\par");
...
\\tthchapter {
  ...
  if(tth_splitfile){ /*sf*/
    sprintf(filenext,"chap%s.xml",labelchar);/*sf*/
    TTH_SCAN_STRING("\\tthsplittail\\tthsplitinv\\tthsplittop\\tthfileupd"); /*sf*/
  }/*sf*/
}
...
\\tthsection {
  ...
  if(lbook) {
    ...
    if(tth_splitfile){ /*sf*/
      sprintf(filenext,"sec%s.xml",labelchar);/*sf*/
      TTH_SCAN_STRING("\\tthsplittail\\tthsplitinv\\tthsplittop\\tthfileupd"); /*sf*/
    }/*sf*/
  }
}
...
<psub>\\tthnewlabel {
  ...
  if(tth_splitfile){ /*sf*/
    ...
    if(strlen(schar)){ /* File defined; use it.*/  /*sf*/
      ...
      strcat(scratchstring,".xml"); /*sf*/
    }else if(*(scrstring+1)=='}') strcpy(scratchstring,"index.xml"); /*sf*/
...
\\footnote{SP}*(\[[0-9]*\])?     {  /* Now using embracetok Sep 98*/
  ...
  if(tth_splitfile)sprintf(scratchstring,"<a href=\"footnote.xml#tthFtNt%s\" id=\"tthFref%s\">",ftntcode,ftntcode);else /*sf*/
  sprintf(scratchstring,
...
<<EOF>>		{
  ...
  if ( --tth_stack_ptr < 0){
    ...
    if(ftntno){
      ...
      ftntno=0;
      if(tth_splitfile){ /*sf*/
	strcpy(filenext,"footnote.xml");/*sf*/
	TTH_SCAN_STRING("\\tthsplittail\\tthsplitinv\\tthsplittop\\tthfileupd"); /*sf*/
      }/*sf*/
      }else{
	...
	if(ftntwrap < nkeys){      /* Footnote wrap-up. Search keys. */
	  ...
	  if(tth_splitfile)fprintf(tth_fdout,"<hr /><a href=\"index.xml\">%s</a>",TTH_HEAD);/*sf*/
    ...
    if(eofrmv[tth_stack_ptr] == 11){ /*Index ref in toc*/
      if(tth_splitfile)/*sf*/
	{fprintf(tth_fdout,"<a href=\"docindex.xml#tth_sEcindex\">Index</a><br />");}else/*sf*/
...
char* tth_symbol_point(chsym)
     char *chsym;
{
  /* this table is based on the test/symbols.xml file of MathML*/
