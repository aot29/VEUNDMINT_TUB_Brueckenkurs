...
function CreateQuestionObj(uxid, c, solution, id, type, option, pnts, intest, section) {
  ...
  */
  ...
  } else
  {
    ...
    ob.validate = function() {
      console.log('validating', this.element.value);
    }
  ...
  function GetResult(uxid)
  {
    ...
    return dataService.getUserData().then(function(userData) {
  ...
  function loadUserInput() {
    log.debug('loading user input and FVAR is', FVAR);
    dataService.getUserData().then(function(data) {
      for (i=1; i<FVAR.length; i++) { FVAR[i].prepare(); }
      log.debug('and now FVAR is', FVAR);
      for (i=1; i<FVAR.length; i++) {
        //set the input value to the value from userData.scores
        var userInputForElement = getUserInputForUxid(FVAR[i].uxid, data);
        FVAR[i].element.value = userInputForElement;
        //trigger on change manually
        if (userInputForElement !== "") {
          handlerChange(i, 0);
        }
      }
    });
